"""Parseur ManhwaRaw pour NexusDL.

ManhwaRaw (``https://manhwaraw.com``) est un site de lecture de **manhwas
coréens bruts** (non traduits, en coréen d'origine) — c'est une plateforme
de référence pour les raws, largement utilisée par les traducteurs et les
lecteurs souhaitant accéder aux œuvres dans leur langue d'origine.

Caractéristiques techniques
---------------------------

* **Moteur** : WordPress **Madara** (thème premium Mangabooth) avec des
  **overrides custom** pour la gestion des raws coréens (structure DOM
  similaire, mais sélecteurs partiellement différents).
* **Langue** : Coréen (``ko``) — les manhwas sont en raw (aucune
  traduction, aucune localisation).
* **Protection** : Cloudflare (AS13335, IP ``104.21.16.85`` /
  ``172.67.172.115``) avec challenge modéré.
* **Domaines** :
    - ``manhwaraw.com`` (principal)
    - ``manhwaraw.net`` (miroir)
    - ``manhwaraw.org`` (miroir secondaire)
* **Structure des URLs** (format Madara custom) :
    - Catalogue : ``/manga/`` ou ``/wp-manga/``.
    - Recherche : ``/?s={query}&post_type=wp-manga``.
    - Manga : ``/manga/{slug}/``.
    - Chapitre : ``/manga/{slug}/{chapter-slug}/``.
* **Sélecteurs Madara spécifiques** : ``.page-item-detail.manga``,
  ``.listing-chapters_wrap li``,
  ``.reading-content .page-break img``.
* **Objet JS ``ts_reader``** : le thème Madara de ManhwaRaw expose les
  pages dans un objet JavaScript ``ts_reader`` — méthode d'extraction
  principale utilisée par les extensions Tachiyomi/Mihon.
* **Contenu adulte** : certains manhwas sont classés « mature » (pornhwa,
  ecchi) — la classification est détectée automatiquement.
* **Politique de contenu** : les raws sont publiés en accord avec les
  ayants droit coréens (partenariats officiels avec certains studios).
...
Example:
    ...
    >>> from nexusdl.parsers.kr.manhwa_raw import ManhwaRawParser
    >>>
    >>> parser = ManhwaRawParser(config, session)
...
_BASE_URL: Final[str] = "https://manhwaraw.com"
_FALLBACK_DOMAINS: Final[tuple[str, ...]] = (
    "https://manhwaraw.com",
    "https://manhwaraw.net",
    "https://manhwaraw.org",
)
_SITE_ID: Final[str] = "manhwa_raw"
_LANGUAGE: Final[Language] = Language.KO  # Raws coréens
_ADULT: Final[bool] = False

# Regex de parsing des chapitres (format mixte coréen/anglais).
# Supporte "Chapter 45", "Chapitre 45", "챕터 45", "45화", "45장".
_CHAPTER_NUMBER_RE: Final[re.Pattern[str]] = re.compile(
    r"(?:chapter|chapitre|ch\.?|episode|ep\.?|ep|챕터|화|장)\s*(\d+(?:[.,]\d+)?)"
    r"|(\d+(?:[.,]\d+)?)\s*(?:화|장)",
    re.IGNORECASE,
)
_VOLUME_RE: Final[re.Pattern[str]] = re.compile(
    r"(?:volume|vol\.?|tome|권)\s*(\d+)", re.IGNORECASE
)
_RELATIVE_DATE_RE: Final[re.Pattern[str]] = re.compile(
    r"(\d+)\s*(second|minute|hour|day|week|month|year|초|분|시간|일|주|개월|년)"
    r"s?\s*(ago|전)?",
    re.IGNORECASE,
)
...
# ---------------------------------------------------------------------------
# Sélecteurs Madara spécifiques à ManhwaRaw
# ---------------------------------------------------------------------------

_MR_SEARCH_ITEM_SELECTOR: Final[str] = (
    ".page-item-detail.manga, .page-item-detail, "
    ".c-tabs-item__content, .bs, .bsx, "
    ".manga-item, .item-summary, .row.c-tabs-item, "
    "div.manga__item, div.manga-item"
)
_MR_CHAPTER_SELECTOR: Final[str] = (
...
_MR_PAGE_IMG_SELECTOR: Final[str] = (
...
_MR_PAGES_VAR_NAMES: Final[tuple[str, ...]] = (
    "ts_reader",
    "pages",
    "page_urls",
    "pageUrls",
...
class ManhwaRawParser(
    ...
    """Parseur ManhwaRaw (Madara custom + Cloudflare).
    ...
    Attributes:
        ...
        language: Langue principale (coréen — raws).
    ...
    # --- Sélecteurs Madara (standards + ajustements ManhwaRaw) ---
    madara_search_item_selector: ClassVar[str] = _MR_SEARCH_ITEM_SELECTOR
    ...
    madara_chapter_selector: ClassVar[str] = _MR_CHAPTER_SELECTOR
    madara_page_img_selector: ClassVar[str] = _MR_PAGE_IMG_SELECTOR
    madara_pages_var_names: ClassVar[tuple[str, ...]] = _MR_PAGES_VAR_NAMES
    ...
    def __init__(
        ...
        *,
        ...
        """Initialise le parseur ManhwaRaw.
    ...
    def _madara_text(node: Node | None) -> str:
        ...
        if node is None:
            return ""
        return ManhwaRawParser._madara_clean(
    ...
    def _madara_parse_status(self, raw: str | None) -> MangaStatus:
        ...
        Args:
            raw: Libellé brut (français, anglais ou coréen).
    ...
    def _madara_parse_relative_date(raw: str | None) -> datetime | None:
        """Parse une date relative (anglaise ou coréenne).

        Supporte : ``"3 days ago"``, ``"3일 전"``, ``"2시간 전"``.
    ...
    def _madara_chapter_number(text: str) -> float | str:
        ...
        """
        cleaned = ManhwaRawParser._madara_clean(text)
        match = _CHAPTER_NUMBER_RE.search(cleaned)
        if not match:
            return cleaned
        # Le groupe 1 capture "Chapter 45", le groupe 2 capture "45화".
    ...
    @staticmethod
    def _mr_extract_ts_reader(html: str) -> list[str]:
        """Extrait les URLs d'images depuis l'objet JS ``ts_reader``.

        Le thème Madara de ManhwaRaw expose les pages dans un objet
        ...
        for pattern in patterns:
            for match in pattern.finditer(html):
                ...
                try:
                    ...
                    continue
                urls = ManhwaRawParser._mr_walk_ts_reader(parsed)
                if urls:
                    return urls
    ...
    @staticmethod
    def _mr_walk_ts_reader(payload: Any) -> list[str]:
    ...
    async def search(
        ...
        """Recherche des manhwas sur ManhwaRaw.

        ManhwaRaw utilise ``/?s={query}&post_type=wp-manga`` (paramètre
        WordPress standard pour le CPT ``wp-manga`` de Madara).

        Args:
            query: Terme de recherche (coréen ou anglais).
        ...
        """
        self.logger.debug(
            "ManhwaRaw search: {query} (page {page})",
            query=query,
            page=page,
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec recherche ManhwaRaw pour {query!r}: {err}",
                query=query,
                err=exc,
            )
            raise ParseError(
    ...
    def _madara_parse_search_html(
        ...
        *,
        ...
        self.logger.info(
            "ManhwaRaw search: {n} résultat(s)", n=len(results)
        )
        return results

    async def get_manga(self, url_or_id: str) -> Manga:
        """Récupère la fiche complète d'un manhwa ManhwaRaw.
        ...
        self.logger.debug("ManhwaRaw get_manga: {url}", url=url)
        ...
        try:
            ...
            self.logger.error(
                "Échec get_manga ManhwaRaw pour {url!r}: {err}",
                url=url,
                err=exc,
            )
            raise ParseError(
        ...
        self.logger.info(
            "ManhwaRaw get_manga OK: {title} ({n} chapitres)",
            title=title,
            n=len(chapters),
        )
        return manga
    ...
    async def get_chapters(self, manga: Manga) -> list[Chapter]:
        """Récupère la liste des chapitres d'un manhwa.
        ...
        """
        self.logger.debug(
            "ManhwaRaw get_chapters: {title}", title=manga.title
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_chapters ManhwaRaw pour {title!r}: {err}",
                title=manga.title,
                err=exc,
            )
            raise ParseError(
    ...
    def _madara_parse_chapters_from_html(
        ...
        *,
        ...
        self.logger.debug(
            "ManhwaRaw chapitres: {n}", n=len(chapters)
        )
        return chapters

    async def get_pages(self, chapter: Chapter) -> list[Page]:
        """Récupère les URLs des pages d'un chapitre ManhwaRaw.
        ...
        """
        self.logger.debug(
            "ManhwaRaw get_pages: {url}", url=str(chapter.url)
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_pages ManhwaRaw pour {url!r}: {err}",
                url=chapter_url,
                err=exc,
            )
            raise ParseError(
        ...
        ts_urls = self._mr_extract_ts_reader(html)
        if ts_urls:
            for u in ts_urls:
        ...
        self.logger.info(
            "ManhwaRaw get_pages: {n} page(s) extraite(s)", n=len(pages)
        )
        return pages
    ...
    async def health_check(self) -> bool:
        """Vérifie que ManhwaRaw est accessible.
        ...
        for base in self.fallback_domains:
            try:
                ...
                if rendered.ok:
                    self.logger.info(
                        "Health check ManhwaRaw OK: {base} (status={s})",
                        base=base,
                        s=rendered.status,
                    )
                    return True
                ...
                self.logger.debug(
                    "Health check ManhwaRaw échec sur {base}: {err}",
                    base=base,
                    err=exc,
                )
        self.logger.warning("Health check ManhwaRaw KO (tous domaines)")
        return False
