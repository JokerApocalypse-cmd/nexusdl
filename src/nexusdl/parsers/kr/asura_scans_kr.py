"""Parseur Asura Scans KR pour NexusDL.

Asura Scans KR (``https://asura-scans.com``) est une variante coréenne
d'Asura Scans, agrégateur de **manhwas coréens** en lecture directe
(principalement en anglais avec quelques sections coréennes). Il est
distinct d'Asura Scans EN (``asuracomic.net``) et fait partie du réseau
historique d'Asura.

⚠️ **Site à évolution rapide** : comme tous les sites du réseau Asura, il
change fréquemment de domaine et son architecture alterne entre :

* **MangaThemesia** (thème WordPress) — mode historique.
* **Next.js custom** — mode moderne (post-2023).
* **API dédiée** (``api.asura-scans.com``) — mode actuel (DTOs JSON).

Ce parseur implémente une **stratégie hybride à trois niveaux** :

1. **API dédiée** — priorité 1, rapide et structurée.
2. **Rendu Playwright + MangaThemesia** — priorité 2, fallback HTML.
3. **Extraction ``ts_reader``** — priorité 3, pour les pages chapitres.

Caractéristiques techniques
---------------------------

* **Moteur** : hybride (API + MangaThemesia + Next.js).
* **Langue** : Coréen (``ko``) avec support anglais (``en``).
* **Protection** : Cloudflare (AS13335, IP ``104.21.31.111`` /
  ``172.67.176.61``) avec Managed Challenge.
* **Domaines** : ``asura-scans.com`` (principal),
  ``asurascans.kr`` (miroir coréen), ``asuracomic.net`` (miroir EN).
* **Structure des URLs** :
    - Catalogue : ``/series`` ou ``/comics``.
    - Manga : ``/series/{slug}``.
    - Chapitre : ``/series/{slug}/{chapter-slug}``.
* **CDN d'images** : ``gg.asuracomic.net/storage/media/...``,
  nécessite le header ``Referer``.
* **Rate limiting** : 2 req/2s signalé par la communauté.
* **User-Agent** : strict (UA identifiable obligatoire).
* **Système de slugs** : suffixes aléatoires pour contrer le scraping.

Le parseur combine quatre mixins :

1. :class:`~nexusdl.parsers.mixins.js_rendered.JsRenderedMixin` — rendu
   Playwright (fallback Next.js SPA).
...
3. :class:`~nexusdl.parsers.mixins.api_based.ApiBasedMixin` — client REST
   pour l'API interne ``api.asura-scans.com``.
4. :class:`~nexusdl.parsers.mixins.mangathemesia.MangaThemesiaMixin` —
   fallback HTML pour les versions MangaThemesia.
...
Example:
    ...
    >>> from nexusdl.parsers.kr.asura_scans_kr import AsuraScansKrParser
    >>>
    >>> parser = AsuraScansKrParser(config, session)
...
_BASE_URL: Final[str] = "https://asura-scans.com"
...
_API_URL: Final[str] = "https://api.asura-scans.com/api"
_SITE_ID: Final[str] = "asura_scans_kr"
_LANGUAGE: Final[Language] = Language.KO
...
# ---------------------------------------------------------------------------
# Sélecteurs MangaThemesia (fallback HTML)
# ---------------------------------------------------------------------------
...
class AsuraScansKrParser(
    ...
    """Parseur Asura Scans KR (API + MangaThemesia + Cloudflare).

    Combine le rendu Playwright (fallback), le contournement Cloudflare, un
    client REST pour l'API ``api.asura-scans.com`` et les implémentations
    MangaThemesia pour les versions HTML historiques.

    Attributes:
        site_id: Identifiant interne du site.
        language: Langue principale (coréen — manhwas).
    ...
    locale: ClassVar[str] = "ko-KR"
    timezone_id: ClassVar[str] = "Asia/Seoul"
    viewport_width: ClassVar[int] = 1366
    viewport_height: ClassVar[int] = 900

    # --- Cache de mapping slug → canonical slug ---
    ...
    def __init__(
        ...
        *,
        ...
        """Initialise le parseur Asura Scans KR.
    ...
    def _asura_logger(self) -> Any:
        ...
        Returns:
            Logger loguru bindé avec ``site_id`` et ``mixin="asura_scans_kr"``.
        """
        return self.logger
    ...
    def _asura_text(node: Node | None) -> str:
        ...
        if node is None:
            return ""
        return AsuraScansKrParser._asura_clean(
    ...
    # ------------------------------------------------------------------
    # Résolution des slugs (suffixes aléatoires Asura)
    # ------------------------------------------------------------------

    def _asura_resolve_slug(self, raw_slug: str) -> str:
        """Résout un slug Asura (avec ou sans suffixe aléatoire).

        Asura Scans ajoute des suffixes aléatoires aux slugs (ex.
        ``solo-leveling-a1b2c3``). Cette méthode tente de retrouver le slug
        canonique via le cache ou l'heuristique de retrait.
    ...
    def _asura_parse_status(self, raw: str | None) -> MangaStatus:
        ...
        Args:
            raw: Libellé brut (anglais, coréen).
    ...
    def _asura_chapter_number(text: str) -> float | str:
        """Extrait le numéro de chapitre d'un libellé (support coréen).

        Supporte : ``"Chapter 45"``, ``"Ch. 45"``, ``"챕터 45"``,
        ``"45화"``, ``"45장"``.
        ...
        """
        cleaned = AsuraScansKrParser._asura_clean(text)
    ...
    def _asura_chapter_volume(text: str) -> int | None:
        """Extrait le numéro de tome d'un libellé (support coréen).

        Args:
            text: Libellé contenant éventuellement ``Volume X`` ou ``X권``.
    ...
    def _asura_detect_rating(self, genres: list[str]) -> ContentRating:
        ...
        if lowered & _ADULT_GENRE_MARKERS:
            if {"hentai", "porn", "18+", "explicit", "pornhwa", "성인"} & lowered:
    ...
    # ------------------------------------------------------------------
    # Construction depuis l'API
    # ------------------------------------------------------------------

    def _asura_build_manga_from_api(self, data: dict[str, Any]) -> Manga:
        """Construit un objet :class:`Manga` depuis l'API Asura Scans KR.
        ...
        if isinstance(status_raw, int):
            ...
            status = self._asura_parse_status(
                str(status_raw) if status_raw else None
            )
    ...
    def _asura_build_chapter_from_api(
        ...
        """Construit un objet :class:`Chapter` depuis l'API Asura Scans KR.
        ...
        chapter_number = self._asura_chapter_number(
            str(chapter_num_raw or "")
        )
    ...
    async def search(
        ...
        """Recherche des mangas sur Asura Scans KR.
        ...
        """
        self.logger.debug(
            "AsuraScansKr search: {query} (page {page})",
            query=query,
            page=page,
        )
        ...
        try:
            ...
            if isinstance(payload, dict):
                ...
                if isinstance(items, list) and items:
                    ...
                    if results:
                        self.logger.info(
                            "AsuraScansKr search (API): {n} résultat(s)",
                            n=len(results),
                        )
                        return results
        ...
        try:
            ...
            self.logger.error(
                "Échec recherche AsuraScansKr pour {query!r}: {err}",
                query=query,
                err=exc,
            )
            raise ParseError(
        ...
        self.logger.info(
            "AsuraScansKr search (DOM): {n} résultat(s)", n=len(filtered)
        )
        return filtered
    ...
    async def get_manga(self, url_or_id: str) -> Manga:
        """Récupère la fiche complète d'un manga Asura Scans KR.
        ...
        self.logger.debug(
            "AsuraScansKr get_manga: {slug} (canonical: {cs})",
            slug=slug,
            cs=canonical_slug,
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_manga AsuraScansKr pour {url!r}: {err}",
                url=url,
                err=exc,
            )
            raise ParseError(
    ...
    def _asura_extract_series_slug(url: str) -> str:
        """Extrait un identifiant de série depuis une URL Asura Scans KR.
    ...
    async def get_chapters(self, manga: Manga) -> list[Chapter]:
        ...
        """
        self.logger.debug(
            "AsuraScansKr get_chapters: {title}", title=manga.title
        )
            ...
            if isinstance(payload, dict):
                ...
                if isinstance(items, list) and items:
                    ...
                    if chapters:
                        ...
                        self.logger.info(
                            "AsuraScansKr get_chapters (API): {n} chapitre(s)",
                            n=len(chapters),
                        )
                        return chapters
        except Exception as exc:  # noqa: BLE001
        ...
        try:
            ...
            self.logger.error(
                "Échec get_chapters AsuraScansKr pour {title!r}: {err}",
                title=manga.title,
                err=exc,
            )
            raise ParseError(
    ...
    async def get_pages(self, chapter: Chapter) -> list[Page]:
        """Récupère les URLs des pages d'un chapitre Asura Scans KR.

        Le parser tente trois stratégies :

        1. API dédiée ``/api/chapter/{id}/pages``.
        2. Extraction depuis l'objet JS ``ts_reader``.
        3. Sélecteurs CSS MangaThemesia.
        ...
        """
        self.logger.debug(
            "AsuraScansKr get_pages: {url}", url=str(chapter.url)
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_pages AsuraScansKr pour {url!r}: {err}",
                url=chapter_url,
                err=exc,
            )
            raise ParseError(
        ...
        for pattern in patterns:
            for match in pattern.finditer(html):
                ...
                try:
                    ...
                    continue
                urls = AsuraScansKrParser._asura_walk_ts_reader(parsed)
                if urls:
                    return urls
    ...
    def _asura_extract_page_urls_from_api(payload: dict[str, Any]) -> list[str]:
        """Extrait les URLs de pages depuis la réponse API Asura Scans KR.
    ...
    async def health_check(self) -> bool:
        """Vérifie que Asura Scans KR est accessible.
        ...
        for base in self.fallback_domains:
            try:
                ...
                if rendered.ok:
                    self.logger.info(
                        "Health check AsuraScansKr OK: {base} (status={s})",
                        base=base,
                        s=rendered.status,
                    )
                    return True
                ...
                self.logger.debug(
                    "Health check AsuraScansKr échec sur {base}: {err}",
                    base=base,
                    err=exc,
                )
        self.logger.warning("Health check AsuraScansKr KO (tous domaines)")
        return False
