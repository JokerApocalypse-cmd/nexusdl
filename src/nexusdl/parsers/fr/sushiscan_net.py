"""Parseur SushiScan.net pour NexusDL.

SushiScan (``https://sushiscan.net``) est l'un des plus importants sites de
scanlation **francophone**, spécialisé dans les mangas, manhwas et manhuas
traduits en français. Il fait partie de la même famille que
**SushiScan.fr** (``sushiscan.fr``) et **SushiScan.net** — deux instances
distinctes gérées par des équipes différentes, mais partageant le même
thème WordPress **Madara** (Mangabooth).
...
* **Protection** : Cloudflare (AS13335, IP ``188.114.96.3`` /
  ``104.21.16.85``) avec challenge modéré.
* **Domaines** :
    - ``sushiscan.net`` (principal, référence historique)
    - ``sushiscan.fr`` (instance sœur, parser distinct)
    - ``sushiscans.net`` (miroir avec "s")
    - ``sushi-scan.net`` (miroir avec tiret)
...
* **Objet JS ``ts_reader``** : le thème Madara de SushiScan expose les
...
* **Contenu adulte** : certains titres sont classés « mature » ou
  ...
  genres.
* **Migration SushiDL** : ce parser reflète la migration documentée dans
  le CHANGELOG de SushiDL (source : projet original devenu NexusDL).
...
Example:
    ...
    >>> from nexusdl.parsers.fr.sushiscan_net import SushiScanNetParser
    >>>
    >>> parser = SushiScanNetParser(config, session)
...
_BASE_URL: Final[str] = "https://sushiscan.net"
_FALLBACK_DOMAINS: Final[tuple[str, ...]] = (
    "https://sushiscan.net",
    "https://sushiscans.net",
    "https://sushi-scan.net",
    "https://sushiscan.fr",
)
_SITE_ID: Final[str] = "sushiscan_net"
...
# ---------------------------------------------------------------------------
# Sélecteurs Madara spécifiques à SushiScan
# ---------------------------------------------------------------------------

_SUSHI_SEARCH_ITEM_SELECTOR: Final[str] = (
    ".page-item-detail.manga, .page-item-detail, "
    ".c-tabs-item__content, .bs, .bsx, "
    ".manga-item, .item-summary, .row.c-tabs-item"
)
_SUSHI_CHAPTER_SELECTOR: Final[str] = (
...
_SUSHI_PAGE_IMG_SELECTOR: Final[str] = (
...
_SUSHI_PAGES_VAR_NAMES: Final[tuple[str, ...]] = (
    "ts_reader",
    "pages",
    "page_urls",
    "pageUrls",
...
class SushiScanNetParser(
    JsRenderedMixin,
    CloudflareMixin,
    MadaraMixin,
    BaseParser,
):
    """Parseur SushiScan.net (Madara + Cloudflare).
    ...
    # --- Sélecteurs Madara (standards + ajustements SushiScan) ---
    madara_search_item_selector: ClassVar[str] = _SUSHI_SEARCH_ITEM_SELECTOR
    ...
    madara_chapter_selector: ClassVar[str] = _SUSHI_CHAPTER_SELECTOR
    madara_page_img_selector: ClassVar[str] = _SUSHI_PAGE_IMG_SELECTOR
    madara_pages_var_names: ClassVar[tuple[str, ...]] = _SUSHI_PAGES_VAR_NAMES
    ...
    def __init__(
        ...
        *,
        ...
        """Initialise le parseur SushiScan.net.
    ...
    def _madara_text(node: Node | None) -> str:
        ...
        if node is None:
            return ""
        return SushiScanNetParser._madara_clean(
    ...
    def _madara_chapter_number(text: str) -> float | str:
        ...
        """
        cleaned = SushiScanNetParser._madara_clean(text)
    ...
    @staticmethod
    def _sushi_extract_ts_reader(html: str) -> list[str]:
        """Extrait les URLs d'images depuis l'objet JS ``ts_reader``.

        Le thème Madara de SushiScan expose les pages dans un objet
        ...
        for pattern in patterns:
            for match in pattern.finditer(html):
                ...
                try:
                    ...
                    continue
                urls = SushiScanNetParser._sushi_walk_ts_reader(parsed)
                if urls:
                    return urls
    ...
    @staticmethod
    def _sushi_walk_ts_reader(payload: Any) -> list[str]:
    ...
    async def search(
        ...
        """Recherche des mangas sur SushiScan.net.

        SushiScan utilise ``/?s={query}&post_type=wp-manga`` (paramètre
        ...
        """
        self.logger.debug(
            "SushiScanNet search: {query} (page {page})",
            query=query,
            page=page,
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec recherche SushiScanNet pour {query!r}: {err}",
                query=query,
                err=exc,
            )
            raise ParseError(
    ...
    def _madara_parse_search_html(
        ...
        *,
        ...
        self.logger.info(
            "SushiScanNet search: {n} résultat(s)", n=len(results)
        )
        return results

    async def get_manga(self, url_or_id: str) -> Manga:
        """Récupère la fiche complète d'un manga SushiScan.net.
        ...
        self.logger.debug("SushiScanNet get_manga: {url}", url=url)
        ...
        try:
            ...
            self.logger.error(
                "Échec get_manga SushiScanNet pour {url!r}: {err}",
                url=url,
                err=exc,
            )
            raise ParseError(
        ...
        self.logger.info(
            "SushiScanNet get_manga OK: {title} ({n} chapitres)",
            title=title,
            n=len(chapters),
        )
        return manga
    ...
    async def get_chapters(self, manga: Manga) -> list[Chapter]:
        ...
        """
        self.logger.debug(
            "SushiScanNet get_chapters: {title}", title=manga.title
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_chapters SushiScanNet pour {title!r}: {err}",
                title=manga.title,
                err=exc,
            )
            raise ParseError(
    ...
    def _madara_parse_chapters_from_html(
        ...
        *,
        ...
        self.logger.debug(
            "SushiScanNet chapitres: {n}", n=len(chapters)
        )
        return chapters

    async def get_pages(self, chapter: Chapter) -> list[Page]:
        """Récupère les URLs des pages d'un chapitre SushiScan.net.
        ...
        """
        self.logger.debug(
            "SushiScanNet get_pages: {url}", url=str(chapter.url)
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_pages SushiScanNet pour {url!r}: {err}",
                url=chapter_url,
                err=exc,
            )
            raise ParseError(
        ...
        ts_urls = self._sushi_extract_ts_reader(html)
        if ts_urls:
            for u in ts_urls:
        ...
        self.logger.info(
            "SushiScanNet get_pages: {n} page(s) extraite(s)", n=len(pages)
        )
        return pages
    ...
    async def health_check(self) -> bool:
        """Vérifie que SushiScan.net est accessible.
        ...
        for base in self.fallback_domains:
            try:
                ...
                if rendered.ok:
                    self.logger.info(
                        "Health check SushiScanNet OK: {base} (status={s})",
                        base=base,
                        s=rendered.status,
                    )
                    return True
                ...
                self.logger.debug(
                    "Health check SushiScanNet échec sur {base}: {err}",
                    base=base,
                    err=exc,
                )
        self.logger.warning("Health check SushiScanNet KO (tous domaines)")
        return False
