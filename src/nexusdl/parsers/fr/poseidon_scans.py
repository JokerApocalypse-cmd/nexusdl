"""Parseur Poseidon Scans pour NexusDL.

Poseidon Scans (``https://poseidonscans.com``) est une équipe de scanlation
**francophone** spécialisée dans les mangas, manhwas et manhuas traduits en
français. Le site utilise le thème WordPress **Madara** (Mangabooth),
également utilisé par Toonily, ToonGod, Galaxy Scans, CrunchyScan, Urano
Scans, Taisei Scans, SushiScan.net, SushiScan.fr, Shinra Scans, Rimuscan et
Raijin Scans.
...
* **Domaines** :
    - ``poseidonscans.com`` (principal)
    - ``poseidon-scans.com`` (miroir avec tiret)
    - ``poseidonscans.fr`` (miroir francophone)
...
* **Objet JS ``ts_reader``** : le thème Madara de Poseidon Scans expose les
...
Example:
    ...
    >>> from nexusdl.parsers.fr.poseidon_scans import PoseidonScansParser
    >>>
    >>> parser = PoseidonScansParser(config, session)
...
_BASE_URL: Final[str] = "https://poseidonscans.com"
_FALLBACK_DOMAINS: Final[tuple[str, ...]] = (
    "https://poseidonscans.com",
    "https://poseidon-scans.com",
    "https://poseidonscans.fr",
)
_SITE_ID: Final[str] = "poseidon_scans"
...
# ---------------------------------------------------------------------------
# Sélecteurs Madara spécifiques à Poseidon Scans
# ---------------------------------------------------------------------------

_POSEIDON_SEARCH_ITEM_SELECTOR: Final[str] = (
    ".page-item-detail.manga, .page-item-detail, "
    ".c-tabs-item__content, .bs, .bsx, "
    ".manga-item, .item-summary, .row.c-tabs-item"
)
_POSEIDON_CHAPTER_SELECTOR: Final[str] = (
...
_POSEIDON_PAGE_IMG_SELECTOR: Final[str] = (
...
_POSEIDON_PAGES_VAR_NAMES: Final[tuple[str, ...]] = (
    "ts_reader",
    "pages",
    "page_urls",
    "pageUrls",
...
class PoseidonScansParser(
    JsRenderedMixin,
    CloudflareMixin,
    MadaraMixin,
    BaseParser,
):
    """Parseur Poseidon Scans (Madara + Cloudflare).
    ...
    # --- Sélecteurs Madara (standards + ajustements Poseidon Scans) ---
    madara_search_item_selector: ClassVar[str] = _POSEIDON_SEARCH_ITEM_SELECTOR
    ...
    madara_chapter_selector: ClassVar[str] = _POSEIDON_CHAPTER_SELECTOR
    madara_page_img_selector: ClassVar[str] = _POSEIDON_PAGE_IMG_SELECTOR
    madara_pages_var_names: ClassVar[tuple[str, ...]] = _POSEIDON_PAGES_VAR_NAMES
    ...
    def __init__(
        ...
        *,
        ...
        """Initialise le parseur Poseidon Scans.
    ...
    def _madara_text(node: Node | None) -> str:
        ...
        if node is None:
            return ""
        return PoseidonScansParser._madara_clean(
    ...
    def _madara_chapter_number(text: str) -> float | str:
        ...
        """
        cleaned = PoseidonScansParser._madara_clean(text)
    ...
    @staticmethod
    def _poseidon_extract_ts_reader(html: str) -> list[str]:
        """Extrait les URLs d'images depuis l'objet JS ``ts_reader``.

        Le thème Madara de Poseidon Scans expose les pages dans un objet
        ...
        for pattern in patterns:
            for match in pattern.finditer(html):
                ...
                try:
                    ...
                    continue
                urls = PoseidonScansParser._poseidon_walk_ts_reader(parsed)
                if urls:
                    return urls
    ...
    @staticmethod
    def _poseidon_walk_ts_reader(payload: Any) -> list[str]:
    ...
    async def search(
        ...
        """Recherche des mangas sur Poseidon Scans.

        Poseidon Scans utilise ``/?s={query}&post_type=wp-manga`` (paramètre
        ...
        """
        self.logger.debug(
            "PoseidonScans search: {query} (page {page})",
            query=query,
            page=page,
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec recherche PoseidonScans pour {query!r}: {err}",
                query=query,
                err=exc,
            )
            raise ParseError(
    ...
    def _madara_parse_search_html(
        ...
        *,
        ...
        self.logger.info(
            "PoseidonScans search: {n} résultat(s)", n=len(results)
        )
        return results

    async def get_manga(self, url_or_id: str) -> Manga:
        """Récupère la fiche complète d'un manga Poseidon Scans.
        ...
        self.logger.debug("PoseidonScans get_manga: {url}", url=url)
        ...
        try:
            ...
            self.logger.error(
                "Échec get_manga PoseidonScans pour {url!r}: {err}",
                url=url,
                err=exc,
            )
            raise ParseError(
        ...
        self.logger.info(
            "PoseidonScans get_manga OK: {title} ({n} chapitres)",
            title=title,
            n=len(chapters),
        )
        return manga
    ...
    async def get_chapters(self, manga: Manga) -> list[Chapter]:
        ...
        """
        self.logger.debug(
            "PoseidonScans get_chapters: {title}", title=manga.title
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_chapters PoseidonScans pour {title!r}: {err}",
                title=manga.title,
                err=exc,
            )
            raise ParseError(
    ...
    def _madara_parse_chapters_from_html(
        ...
        *,
        ...
        self.logger.debug(
            "PoseidonScans chapitres: {n}", n=len(chapters)
        )
        return chapters

    async def get_pages(self, chapter: Chapter) -> list[Page]:
        """Récupère les URLs des pages d'un chapitre Poseidon Scans.
        ...
        """
        self.logger.debug(
            "PoseidonScans get_pages: {url}", url=str(chapter.url)
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_pages PoseidonScans pour {url!r}: {err}",
                url=chapter_url,
                err=exc,
            )
            raise ParseError(
        ...
        ts_urls = self._poseidon_extract_ts_reader(html)
        if ts_urls:
            for u in ts_urls:
        ...
        self.logger.info(
            "PoseidonScans get_pages: {n} page(s) extraite(s)", n=len(pages)
        )
        return pages
    ...
    async def health_check(self) -> bool:
        """Vérifie que Poseidon Scans est accessible.
        ...
        for base in self.fallback_domains:
            try:
                ...
                if rendered.ok:
                    self.logger.info(
                        "Health check PoseidonScans OK: {base} (status={s})",
                        base=base,
                        s=rendered.status,
                    )
                    return True
                ...
                self.logger.debug(
                    "Health check PoseidonScans échec sur {base}: {err}",
                    base=base,
                    err=exc,
                )
        self.logger.warning("Health check PoseidonScans KO (tous domaines)")
        return False
