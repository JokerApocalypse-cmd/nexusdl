"""Parseur CrunchyScan FR pour NexusDL.

CrunchyScan FR (``https://crunchyscan.fr``) est l'instance **francophone**
de CrunchyScan — l'un des plus importants sites de scanlation francophone,
spécialisé dans les mangas, manhwas et manhuas traduits en français. Il
utilise le thème WordPress **Madara** (Mangabooth), le même moteur que
Toonily, ToonGod, Galaxy Scans, Urano Scans, Taisei Scans, SushiScan.net,
SushiScan.fr, Shinra Scans, Rimuscan, Raijin Scans, Poseidon Scans,
Phenix Scans, Ortegascans, Karma Scans, FanDub Scans et Epsilon Scan.

⚠️ **Note importante** : ce parser est **distinct** de
:class:`~nexusdl.parsers.en.crunchyscan_org.CrunchyScanParser` qui cible
l'instance ``crunchyscan.org``. Les deux instances partagent le même
thème Madara et le même opérateur historique, mais ont des catalogues
**partiellement distincts** :

* ``crunchyscan.fr`` — instance francophone historique (ce parser).
* ``crunchyscan.org`` — instance moderne (parser distinct dans `en/`).

⚠️ **Blocage en France** : ``crunchyscan.fr`` a été bloqué par les
principaux FAI français suite à des décisions judiciaires. L'accès
nécessite un VPN ou un DNS alternatif pour les utilisateurs basés en
France.

Caractéristiques techniques
---------------------------

* **Moteur** : WordPress **Madara** (thème premium Mangabooth).
* **Langue** : Français (``fr``).
* **Protection** : Cloudflare (AS13335, IP ``104.21.16.85`` /
  ``172.67.172.115``) avec challenge modéré. Le site exige également un
  **captcha interceptor** intégré aux chapitres (signalé par la
  communauté Tachiyomi/Mihon) et un **age gate** cookie pour le contenu
  adulte.
* **Domaines** :
    - ``crunchyscan.fr`` (principal, instance FR)
    - ``crunchyscan.org`` (instance moderne, fallback)
    - ``crunchyscan.st`` (miroir)
* **Structure des URLs** (format Madara standard) :
    - Catalogue : ``/liste-manga/`` ou ``/manga/``.
    - Recherche : ``/?s={query}&post_type=wp-manga``.
    - Manga : ``/liste-manga/{slug}/`` ou ``/manga/{slug}/``.
    - Chapitre : ``/lecture-en-ligne/{slug}/{chapter-slug}/``.
* **Contenu adulte** : 18+/NSFW (partiel) — certains titres sont
  classifiés `mature` ou `ecchi`. L'age gate cookie est requis pour
  accéder au contenu NSFW.
* **Objet JS ``ts_reader``** : le thème Madara de CrunchyScan expose les
  pages dans un objet JavaScript ``ts_reader`` — méthode d'extraction
  principale utilisée par les extensions Tachiyomi/Mihon.
...
Example:
    ...
    >>> from nexusdl.parsers.fr.crunchyscan_fr import CrunchyScanFrParser
    >>>
    >>> parser = CrunchyScanFrParser(config, session)
...
_BASE_URL: Final[str] = "https://crunchyscan.fr"
_FALLBACK_DOMAINS: Final[tuple[str, ...]] = (
    "https://crunchyscan.fr",
    "https://crunchyscan.org",
    "https://crunchyscan.st",
)
_SITE_ID: Final[str] = "crunchyscan_fr"
_LANGUAGE: Final[Language] = Language.FR
_ADULT: Final[bool] = True  # Contenu adulte partiel (age gate requis)

# Cookie age gate (obligatoire pour accéder au contenu NSFW).
_AGE_GATE_COOKIE_NAME: Final[str] = "adult"
_AGE_GATE_COOKIE_VALUE: Final[str] = "1"

# Regex de parsing des chapitres (format Madara FR).
...
# ---------------------------------------------------------------------------
# Sélecteurs Madara spécifiques à CrunchyScan FR
# ---------------------------------------------------------------------------

_CS_FR_SEARCH_ITEM_SELECTOR: Final[str] = (
    ".page-item-detail.manga, .page-item-detail, "
    ".c-tabs-item__content, .bs, .bsx, "
    ".manga-item, .item-summary, .row.c-tabs-item"
)
_CS_FR_CHAPTER_SELECTOR: Final[str] = (
...
_CS_FR_PAGE_IMG_SELECTOR: Final[str] = (
...
_CS_FR_PAGES_VAR_NAMES: Final[tuple[str, ...]] = (
    "ts_reader",
    "pages",
    "page_urls",
    "pageUrls",
...
class CrunchyScanFrParser(
    JsRenderedMixin,
    CloudflareMixin,
    MadaraMixin,
    BaseParser,
):
    """Parseur CrunchyScan FR (Madara + Cloudflare).
    ...
    Attributes:
        ...
        adult: Contenu adulte (``True`` — age gate NSFW partiel).
    ...
    madara_series_path: ClassVar[str] = "/liste-manga"
    madara_read_path: ClassVar[str] = "/lecture-en-ligne"

    # --- Sélecteurs Madara (standards + ajustements CrunchyScan FR) ---
    madara_search_item_selector: ClassVar[str] = _CS_FR_SEARCH_ITEM_SELECTOR
    ...
    madara_chapter_selector: ClassVar[str] = _CS_FR_CHAPTER_SELECTOR
    madara_page_img_selector: ClassVar[str] = _CS_FR_PAGE_IMG_SELECTOR
    madara_pages_var_names: ClassVar[tuple[str, ...]] = _CS_FR_PAGES_VAR_NAMES
    ...
    def __init__(
        ...
        *,
        ...
        """Initialise le parseur CrunchyScan FR.
    ...
    def _inject_age_gate_cookie(self) -> None:
        ...
        CrunchyScan FR exige un cookie ``adult=1`` pour accéder au
        contenu NSFW (certains titres classifiés mature/ecchi).
        """
    ...
    def _madara_text(node: Node | None) -> str:
        ...
        if node is None:
            return ""
        return CrunchyScanFrParser._madara_clean(
    ...
    def _madara_chapter_number(text: str) -> float | str:
        ...
        """
        cleaned = CrunchyScanFrParser._madara_clean(text)
    ...
    def _madara_detect_rating(self, genres: list[str]) -> ContentRating:
        ...
        CrunchyScan FR héberge un mélange de titres généralistes et de
        titres matures. La classification est détectée automatiquement.
    ...
    @staticmethod
    def _cs_fr_extract_ts_reader(html: str) -> list[str]:
        """Extrait les URLs d'images depuis l'objet JS ``ts_reader``.

        Le thème Madara de CrunchyScan FR expose les pages dans un objet
        JavaScript ``ts_reader`` de la forme
        ...
        for pattern in patterns:
            for match in pattern.finditer(html):
                ...
                try:
                    ...
                    continue
                urls = CrunchyScanFrParser._cs_fr_walk_ts_reader(parsed)
                if urls:
                    return urls
    ...
    @staticmethod
    def _cs_fr_walk_ts_reader(payload: Any) -> list[str]:
    ...
    async def search(
        ...
        """Recherche des mangas sur CrunchyScan FR.

        CrunchyScan FR utilise ``/?s={query}&post_type=wp-manga``
        (paramètre WordPress standard pour le CPT ``wp-manga`` de Madara).
        ...
        """
        self.logger.debug(
            "CrunchyScanFr search: {query} (page {page})",
            query=query,
            page=page,
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec recherche CrunchyScanFr pour {query!r}: {err}",
                query=query,
                err=exc,
            )
            raise ParseError(
    ...
    def _madara_parse_search_html(
        ...
        *,
        ...
        for node in self._madara_all(
            ...
            if node.tag == "a" and (
                "/liste-manga/" in self._madara_attr(node, "href")
                or "/manga/" in self._madara_attr(node, "href")
            ):
                link_node = node
            else:
                for candidate in node.css("a"):
                    href = self._madara_attr(candidate, "href")
                    if "/liste-manga/" in href or "/manga/" in href:
                        link_node = candidate
                        break
        ...
        self.logger.info(
            "CrunchyScanFr search: {n} résultat(s)", n=len(results)
        )
        return results

    async def get_manga(self, url_or_id: str) -> Manga:
        """Récupère la fiche complète d'un manga CrunchyScan FR.
        ...
        self.logger.debug("CrunchyScanFr get_manga: {url}", url=url)
        ...
        try:
            ...
            self.logger.error(
                "Échec get_manga CrunchyScanFr pour {url!r}: {err}",
                url=url,
                err=exc,
            )
            raise ParseError(
        ...
        self.logger.info(
            "CrunchyScanFr get_manga OK: {title} ({n} chapitres)",
            title=title,
            n=len(chapters),
        )
        return manga
    ...
    def _madara_extract_series_slug(url: str) -> str:
        ...
        Supporte les préfixes ``/liste-manga/`` et ``/manga/``.
    ...
    async def get_chapters(self, manga: Manga) -> list[Chapter]:
        ...
        """
        self.logger.debug(
            "CrunchyScanFr get_chapters: {title}", title=manga.title
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_chapters CrunchyScanFr pour {title!r}: {err}",
                title=manga.title,
                err=exc,
            )
            raise ParseError(
    ...
    def _madara_parse_chapters_from_html(
        ...
        *,
        ...
        self.logger.debug(
            "CrunchyScanFr chapitres: {n}", n=len(chapters)
        )
        return chapters

    async def get_pages(self, chapter: Chapter) -> list[Page]:
        """Récupère les URLs des pages d'un chapitre CrunchyScan FR.

        Le site utilise Cloudflare (avec captcha interceptor) et du
        lazy-loading : le rendu Playwright est nécessaire. Les pages sont
        extraites en priorité depuis l'objet JS ``ts_reader`` (méthode
        Tachiyomi/Mihon), avec fallback sur les sélecteurs CSS Madara.
        ...
        """
        self.logger.debug(
            "CrunchyScanFr get_pages: {url}", url=str(chapter.url)
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_pages CrunchyScanFr pour {url!r}: {err}",
                url=chapter_url,
                err=exc,
            )
            raise ParseError(
        ...
        ts_urls = self._cs_fr_extract_ts_reader(html)
        if ts_urls:
            for u in ts_urls:
        ...
        if not urls:
            for node in self._madara_all(tree, self.madara_page_img_selector):
                ...
                if not src:
                    continue
                # Filtre les images non-chapitre (age gate, captcha).
                if any(
                    marker in src.lower()
                    for marker in (
                        "logo",
        ...
        self.logger.info(
            "CrunchyScanFr get_pages: {n} page(s) extraite(s)", n=len(pages)
        )
        return pages
    ...
    async def health_check(self) -> bool:
        """Vérifie que CrunchyScan FR est accessible.
        ...
        for base in self.fallback_domains:
            try:
                ...
                if rendered.ok:
                    self.logger.info(
                        "Health check CrunchyScanFr OK: {base} (status={s})",
                        base=base,
                        s=rendered.status,
                    )
                    return True
                ...
                self.logger.debug(
                    "Health check CrunchyScanFr échec sur {base}: {err}",
                    base=base,
                    err=exc,
                )
        self.logger.warning(
            "Health check CrunchyScanFr KO (tous domaines)"
        )
        return False
