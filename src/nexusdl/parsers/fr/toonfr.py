"""Parseur ToonFR pour NexusDL.

ToonFR (``https://toonfr.com``) est une plateforme de scanlation
**francophone** spécialisée dans les **manhwas**, **manhuas** et
**webtoons** traduits en français. Le site utilise le thème WordPress
**MangaThemesia** (thème moderne dérivé de Madara), également utilisé
par Asura Scans, Flame Comics, Void Scans, Luminous Scans, Zenith Scans
et Xanadu Scans.
...
* **Domaines** :
    - ``toonfr.com`` (principal)
    - ``toon-fr.com`` (miroir avec tiret)
    - ``toonfr.fr`` (miroir francophone)
* **Structure des URLs** :
    - Catalogue : ``/series/``.
...
Example:
    ...
    >>> from nexusdl.parsers.fr.toonfr import ToonFRParser
    >>>
    >>> parser = ToonFRParser(config, session)
...
_BASE_URL: Final[str] = "https://toonfr.com"
_FALLBACK_DOMAINS: Final[tuple[str, ...]] = (
    "https://toonfr.com",
    "https://toon-fr.com",
    "https://toonfr.fr",
)
_SITE_ID: Final[str] = "toonfr"
...
# ---------------------------------------------------------------------------
# Sélecteurs MangaThemesia spécifiques à ToonFR
# ---------------------------------------------------------------------------

_TOONFR_SEARCH_ITEM_SELECTOR: Final[str] = (
    ".listupd .bs, .listupd .bsx, .bs, .bsx, .uta, "
    ".list-item, .manga-item, .manga__item"
)
_TOONFR_SEARCH_LINK_SELECTOR: Final[str] = "a"
_TOONFR_SEARCH_COVER_SELECTOR: Final[str] = "img"
_TOONFR_SEARCH_TITLE_SELECTOR: Final[str] = (
    ".tt, .title, h3, h4, .entry-title, .ntitle"
)

_TOONFR_MANGA_TITLE_SELECTOR: Final[str] = (
    "h1.entry-title, h1, .entry-title, .manga-title, .post-title, "
    ".ts-breadcrumb li:last-child"
)
_TOONFR_MANGA_COVER_SELECTOR: Final[str] = (
    ".thumb img, .manga-cover img, .summary_image img, "
    ".cover img, .thumbook img"
)
_TOONFR_MANGA_DESCRIPTION_SELECTOR: Final[str] = (
    ".summary__content, .manga-summary, .description, "
    ".entry-content p, .desc"
)
_TOONFR_MANGA_AUTHOR_SELECTOR: Final[str] = (
    ".author, .manga-author, a[href*='/author/'], "
    ".tsinfo .imptdt:contains('Auteur') a"
)
_TOONFR_MANGA_ARTIST_SELECTOR: Final[str] = (
    ".artist, .manga-artist, a[href*='/artist/'], "
    ".tsinfo .imptdt:contains('Artiste') a"
)
_TOONFR_MANGA_GENRES_SELECTOR: Final[str] = (
    ".genres a, .manga-genres a, a[href*='/genre/'], "
    "a[href*='/tag/'], .mgen a"
)
_TOONFR_MANGA_STATUS_SELECTOR: Final[str] = (
    ".status, .manga-status, .post-status, "
    ".tsinfo .imptdt:contains('Statut') i"
)
_TOONFR_MANGA_YEAR_SELECTOR: Final[str] = (
    ".year, .manga-year, .post-year, "
    ".tsinfo .imptdt:contains('Année') i"
)

_TOONFR_CHAPTER_SELECTOR: Final[str] = (
...
class ToonFRParser(
    ...
    """Parseur ToonFR (MangaThemesia + Cloudflare).
    ...
    # --- Sélecteurs MangaThemesia (standards + ajustements ToonFR) ---
    mt_search_item_selector: ClassVar[str] = _TOONFR_SEARCH_ITEM_SELECTOR
    mt_search_link_selector: ClassVar[str] = _TOONFR_SEARCH_LINK_SELECTOR
    mt_search_cover_selector: ClassVar[str] = _TOONFR_SEARCH_COVER_SELECTOR
    mt_search_title_selector: ClassVar[str] = _TOONFR_SEARCH_TITLE_SELECTOR

    mt_manga_title_selector: ClassVar[str] = _TOONFR_MANGA_TITLE_SELECTOR
    mt_manga_cover_selector: ClassVar[str] = _TOONFR_MANGA_COVER_SELECTOR
    mt_manga_description_selector: ClassVar[str] = _TOONFR_MANGA_DESCRIPTION_SELECTOR
    mt_manga_author_selector: ClassVar[str] = _TOONFR_MANGA_AUTHOR_SELECTOR
    mt_manga_artist_selector: ClassVar[str] = _TOONFR_MANGA_ARTIST_SELECTOR
    mt_manga_genres_selector: ClassVar[str] = _TOONFR_MANGA_GENRES_SELECTOR
    mt_manga_status_selector: ClassVar[str] = _TOONFR_MANGA_STATUS_SELECTOR
    mt_manga_year_selector: ClassVar[str] = _TOONFR_MANGA_YEAR_SELECTOR

    mt_chapter_selector: ClassVar[str] = _TOONFR_CHAPTER_SELECTOR
    mt_page_img_selector: ClassVar[str] = _TOONFR_PAGE_IMG_SELECTOR
    mt_pages_var_names: ClassVar[tuple[str, ...]] = _TOONFR_PAGES_VAR_NAMES
    ...
    def _mt_chapter_number(text: str) -> float | str:
        ...
        """
        cleaned = ToonFRParser._mt_clean(text)
    ...
    @staticmethod
    def _toonfr_extract_ts_reader(html: str) -> list[str]:
        ...
        for pattern in patterns:
            for match in pattern.finditer(html):
                ...
                try:
                    ...
                    continue
                urls = ToonFRParser._toonfr_walk_ts_reader(parsed)
                if urls:
                    return urls
    ...
    @staticmethod
    def _toonfr_walk_ts_reader(payload: Any) -> list[str]:
    ...
    async def search(
        ...
        """Recherche des mangas sur ToonFR.

        ToonFR utilise ``/?s={query}`` (paramètre WordPress standard).
        ...
        try:
            ...
            self.logger.error(
                "Échec recherche ToonFR pour {query!r}: {err}",
                query=query,
                err=exc,
            )
            raise ParseError(
    ...
    def _mt_parse_search_html(
        ...
        *,
        ...
        self.logger.info(
            "ToonFR search: {n} résultat(s)", n=len(results)
        )
        return results

    async def get_manga(self, url_or_id: str) -> Manga:
        """Récupère la fiche complète d'un manga ToonFR.
        ...
        self.logger.debug("ToonFR get_manga: {url}", url=url)
        ...
        try:
            ...
            self.logger.error(
                "Échec get_manga ToonFR pour {url!r}: {err}",
                url=url,
                err=exc,
            )
            raise ParseError(
        ...
        self.logger.info(
            "ToonFR get_manga OK: {title} ({n} chapitres)",
            title=title,
            n=len(chapters),
        )
        return manga
    ...
    def _mt_extract_series_slug(url: str) -> str:
        """Extrait un identifiant de série depuis une URL ToonFR.
    ...
    async def get_chapters(self, manga: Manga) -> list[Chapter]:
        ...
        """
        self.logger.debug(
            "ToonFR get_chapters: {title}", title=manga.title
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_chapters ToonFR pour {title!r}: {err}",
                title=manga.title,
                err=exc,
            )
            raise ParseError(
    ...
    def _mt_parse_chapters_from_html(
        ...
        *,
        ...
        self.logger.debug(
            "ToonFR chapitres: {n}", n=len(chapters)
        )
        return chapters

    async def get_pages(self, chapter: Chapter) -> list[Page]:
        """Récupère les URLs des pages d'un chapitre ToonFR.
        ...
        """
        self.logger.debug(
            "ToonFR get_pages: {url}", url=str(chapter.url)
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_pages ToonFR pour {url!r}: {err}",
                url=chapter_url,
                err=exc,
            )
            raise ParseError(
        ...
        ts_urls = self._toonfr_extract_ts_reader(html)
        if ts_urls:
            for u in ts_urls:
        ...
        self.logger.info(
            "ToonFR get_pages: {n} page(s) extraite(s)", n=len(pages)
        )
        return pages
    ...
    async def health_check(self) -> bool:
        """Vérifie que ToonFR est accessible.
        ...
        for base in self.fallback_domains:
            try:
                ...
                if rendered.ok:
                    self.logger.info(
                        "Health check ToonFR OK: {base} (status={s})",
                        base=base,
                        s=rendered.status,
                    )
                    return True
                ...
                self.logger.debug(
                    "Health check ToonFR échec sur {base}: {err}",
                    base=base,
                    err=exc,
                )
        self.logger.warning("Health check ToonFR KO (tous domaines)")
        return False
