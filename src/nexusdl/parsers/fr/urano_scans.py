"""Parseur Urano Scans pour NexusDL.

Urano Scans (``https://uranoscans.com``) est une équipe de scanlation
...
Toonily, ToonGod, Galaxy Scans et CrunchyScan.
...
* **Domaines** :
    - ``uranoscans.com`` (principal)
    - ``urano-scans.com`` (miroir avec tiret)
    - ``uranoscans.fr`` (miroir francophone)
...
* **Objet JS ``ts_reader``** : le thème Madara d'Urano Scans expose les
...
Example:
    ...
    >>> from nexusdl.parsers.fr.urano_scans import UranoScansParser
    >>>
    >>> parser = UranoScansParser(config, session)
...
_BASE_URL: Final[str] = "https://uranoscans.com"
_FALLBACK_DOMAINS: Final[tuple[str, ...]] = (
    "https://uranoscans.com",
    "https://urano-scans.com",
    "https://uranoscans.fr",
)
_SITE_ID: Final[str] = "urano_scans"
...
# ---------------------------------------------------------------------------
# Sélecteurs Madara spécifiques à Urano Scans
# ---------------------------------------------------------------------------

_URANO_SEARCH_ITEM_SELECTOR: Final[str] = (
    ".page-item-detail.manga, .page-item-detail, "
    ".c-tabs-item__content, .bs, .bsx, "
    ".manga-item, .item-summary, .row.c-tabs-item"
)
_URANO_CHAPTER_SELECTOR: Final[str] = (
...
_URANO_PAGE_IMG_SELECTOR: Final[str] = (
...
_URANO_PAGES_VAR_NAMES: Final[tuple[str, ...]] = (
    "ts_reader",
    "pages",
    "page_urls",
    "pageUrls",
...
class UranoScansParser(
    JsRenderedMixin,
    CloudflareMixin,
    MadaraMixin,
    BaseParser,
):
    """Parseur Urano Scans (Madara + Cloudflare).
    ...
    # --- Sélecteurs Madara (standards + ajustements Urano Scans) ---
    madara_search_item_selector: ClassVar[str] = _URANO_SEARCH_ITEM_SELECTOR
    ...
    madara_chapter_selector: ClassVar[str] = _URANO_CHAPTER_SELECTOR
    madara_page_img_selector: ClassVar[str] = _URANO_PAGE_IMG_SELECTOR
    madara_pages_var_names: ClassVar[tuple[str, ...]] = _URANO_PAGES_VAR_NAMES
    ...
    def __init__(
        ...
        *,
        ...
        """Initialise le parseur Urano Scans.
    ...
    def _madara_text(node: Node | None) -> str:
        ...
        if node is None:
            return ""
        return UranoScansParser._madara_clean(
    ...
    def _madara_chapter_number(text: str) -> float | str:
        ...
        """
        cleaned = UranoScansParser._madara_clean(text)
    ...
    @staticmethod
    def _urano_extract_ts_reader(html: str) -> list[str]:
        """Extrait les URLs d'images depuis l'objet JS ``ts_reader``.

        Le thème Madara d'Urano Scans expose les pages dans un objet
        ...
        for pattern in patterns:
            for match in pattern.finditer(html):
                ...
                try:
                    ...
                    continue
                urls = UranoScansParser._urano_walk_ts_reader(parsed)
                if urls:
                    return urls
    ...
    @staticmethod
    def _urano_walk_ts_reader(payload: Any) -> list[str]:
    ...
    async def search(
        ...
        """Recherche des mangas sur Urano Scans.

        Urano Scans utilise ``/?s={query}&post_type=wp-manga`` (paramètre
        ...
        """
        self.logger.debug(
            "UranoScans search: {query} (page {page})",
            query=query,
            page=page,
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec recherche UranoScans pour {query!r}: {err}",
                query=query,
                err=exc,
            )
            raise ParseError(
    ...
    def _madara_parse_search_html(
        ...
        *,
        ...
        self.logger.info(
            "UranoScans search: {n} résultat(s)", n=len(results)
        )
        return results

    async def get_manga(self, url_or_id: str) -> Manga:
        """Récupère la fiche complète d'un manga Urano Scans.
        ...
        self.logger.debug("UranoScans get_manga: {url}", url=url)
        ...
        try:
            ...
            self.logger.error(
                "Échec get_manga UranoScans pour {url!r}: {err}",
                url=url,
                err=exc,
            )
            raise ParseError(
        ...
        self.logger.info(
            "UranoScans get_manga OK: {title} ({n} chapitres)",
            title=title,
            n=len(chapters),
        )
        return manga
    ...
    async def get_chapters(self, manga: Manga) -> list[Chapter]:
        ...
        """
        self.logger.debug(
            "UranoScans get_chapters: {title}", title=manga.title
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_chapters UranoScans pour {title!r}: {err}",
                title=manga.title,
                err=exc,
            )
            raise ParseError(
    ...
    def _madara_parse_chapters_from_html(
        ...
        *,
        ...
        self.logger.debug(
            "UranoScans chapitres: {n}", n=len(chapters)
        )
        return chapters

    async def get_pages(self, chapter: Chapter) -> list[Page]:
        """Récupère les URLs des pages d'un chapitre Urano Scans.
        ...
        """
        self.logger.debug(
            "UranoScans get_pages: {url}", url=str(chapter.url)
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_pages UranoScans pour {url!r}: {err}",
                url=chapter_url,
                err=exc,
            )
            raise ParseError(
        ...
        ts_urls = self._urano_extract_ts_reader(html)
        if ts_urls:
            for u in ts_urls:
        ...
        self.logger.info(
            "UranoScans get_pages: {n} page(s) extraite(s)", n=len(pages)
        )
        return pages
    ...
    async def health_check(self) -> bool:
        """Vérifie que Urano Scans est accessible.
        ...
        for base in self.fallback_domains:
            try:
                ...
                if rendered.ok:
                    self.logger.info(
                        "Health check UranoScans OK: {base} (status={s})",
                        base=base,
                        s=rendered.status,
                    )
                    return True
                ...
                self.logger.debug(
                    "Health check UranoScans échec sur {base}: {err}",
                    base=base,
                    err=exc,
                )
        self.logger.warning("Health check UranoScans KO (tous domaines)")
        return False
