"""Parseur Raijin Scans pour NexusDL.

Raijin Scans (``https://raijinscans.com``) est une équipe de scanlation
...
Scans, Taisei Scans, SushiScan.net, SushiScan.fr, Shinra Scans et Rimuscan.
...
* **Domaines** :
    - ``raijinscans.com`` (principal)
    - ``raijin-scans.com`` (miroir avec tiret)
    - ``raijinscans.fr`` (miroir francophone)
...
* **Objet JS ``ts_reader``** : le thème Madara de Raijin Scans expose les
...
Example:
    ...
    >>> from nexusdl.parsers.fr.raijin_scans import RaijinScansParser
    >>>
    >>> parser = RaijinScansParser(config, session)
...
_BASE_URL: Final[str] = "https://raijinscans.com"
_FALLBACK_DOMAINS: Final[tuple[str, ...]] = (
    "https://raijinscans.com",
    "https://raijin-scans.com",
    "https://raijinscans.fr",
)
_SITE_ID: Final[str] = "raijin_scans"
...
# ---------------------------------------------------------------------------
# Sélecteurs Madara spécifiques à Raijin Scans
# ---------------------------------------------------------------------------

_RAIJIN_SEARCH_ITEM_SELECTOR: Final[str] = (
    ".page-item-detail.manga, .page-item-detail, "
    ".c-tabs-item__content, .bs, .bsx, "
    ".manga-item, .item-summary, .row.c-tabs-item"
)
_RAIJIN_CHAPTER_SELECTOR: Final[str] = (
...
_RAIJIN_PAGE_IMG_SELECTOR: Final[str] = (
...
_RAIJIN_PAGES_VAR_NAMES: Final[tuple[str, ...]] = (
    "ts_reader",
    "pages",
    "page_urls",
    "pageUrls",
...
class RaijinScansParser(
    JsRenderedMixin,
    CloudflareMixin,
    MadaraMixin,
    BaseParser,
):
    """Parseur Raijin Scans (Madara + Cloudflare).
    ...
    # --- Sélecteurs Madara (standards + ajustements Raijin Scans) ---
    madara_search_item_selector: ClassVar[str] = _RAIJIN_SEARCH_ITEM_SELECTOR
    ...
    madara_chapter_selector: ClassVar[str] = _RAIJIN_CHAPTER_SELECTOR
    madara_page_img_selector: ClassVar[str] = _RAIJIN_PAGE_IMG_SELECTOR
    madara_pages_var_names: ClassVar[tuple[str, ...]] = _RAIJIN_PAGES_VAR_NAMES
    ...
    def __init__(
        ...
        *,
        ...
        """Initialise le parseur Raijin Scans.
    ...
    def _madara_text(node: Node | None) -> str:
        ...
        if node is None:
            return ""
        return RaijinScansParser._madara_clean(
    ...
    def _madara_chapter_number(text: str) -> float | str:
        ...
        """
        cleaned = RaijinScansParser._madara_clean(text)
    ...
    @staticmethod
    def _raijin_extract_ts_reader(html: str) -> list[str]:
        """Extrait les URLs d'images depuis l'objet JS ``ts_reader``.

        Le thème Madara de Raijin Scans expose les pages dans un objet
        ...
        for pattern in patterns:
            for match in pattern.finditer(html):
                ...
                try:
                    ...
                    continue
                urls = RaijinScansParser._raijin_walk_ts_reader(parsed)
                if urls:
                    return urls
    ...
    @staticmethod
    def _raijin_walk_ts_reader(payload: Any) -> list[str]:
    ...
    async def search(
        ...
        """Recherche des mangas sur Raijin Scans.

        Raijin Scans utilise ``/?s={query}&post_type=wp-manga`` (paramètre
        ...
        """
        self.logger.debug(
            "RaijinScans search: {query} (page {page})",
            query=query,
            page=page,
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec recherche RaijinScans pour {query!r}: {err}",
                query=query,
                err=exc,
            )
            raise ParseError(
    ...
    def _madara_parse_search_html(
        ...
        *,
        ...
        self.logger.info(
            "RaijinScans search: {n} résultat(s)", n=len(results)
        )
        return results

    async def get_manga(self, url_or_id: str) -> Manga:
        """Récupère la fiche complète d'un manga Raijin Scans.
        ...
        self.logger.debug("RaijinScans get_manga: {url}", url=url)
        ...
        try:
            ...
            self.logger.error(
                "Échec get_manga RaijinScans pour {url!r}: {err}",
                url=url,
                err=exc,
            )
            raise ParseError(
        ...
        self.logger.info(
            "RaijinScans get_manga OK: {title} ({n} chapitres)",
            title=title,
            n=len(chapters),
        )
        return manga
    ...
    async def get_chapters(self, manga: Manga) -> list[Chapter]:
        ...
        """
        self.logger.debug(
            "RaijinScans get_chapters: {title}", title=manga.title
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_chapters RaijinScans pour {title!r}: {err}",
                title=manga.title,
                err=exc,
            )
            raise ParseError(
    ...
    def _madara_parse_chapters_from_html(
        ...
        *,
        ...
        self.logger.debug(
            "RaijinScans chapitres: {n}", n=len(chapters)
        )
        return chapters

    async def get_pages(self, chapter: Chapter) -> list[Page]:
        """Récupère les URLs des pages d'un chapitre Raijin Scans.
        ...
        """
        self.logger.debug(
            "RaijinScans get_pages: {url}", url=str(chapter.url)
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_pages RaijinScans pour {url!r}: {err}",
                url=chapter_url,
                err=exc,
            )
            raise ParseError(
        ...
        ts_urls = self._raijin_extract_ts_reader(html)
        if ts_urls:
            for u in ts_urls:
        ...
        self.logger.info(
            "RaijinScans get_pages: {n} page(s) extraite(s)", n=len(pages)
        )
        return pages
    ...
    async def health_check(self) -> bool:
        """Vérifie que Raijin Scans est accessible.
        ...
        for base in self.fallback_domains:
            try:
                ...
                if rendered.ok:
                    self.logger.info(
                        "Health check RaijinScans OK: {base} (status={s})",
                        base=base,
                        s=rendered.status,
                    )
                    return True
                ...
                self.logger.debug(
                    "Health check RaijinScans échec sur {base}: {err}",
                    base=base,
                    err=exc,
                )
        self.logger.warning("Health check RaijinScans KO (tous domaines)")
        return False
