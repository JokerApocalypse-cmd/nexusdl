"""Parseur Blossom Scans pour NexusDL.

Blossom Scans (``https://blossom-scans.com``) est une équipe de scanlation
...
Karma Scans, FanDub Scans, Epsilon Scan et CrunchyScan FR.
...
* **Domaines** :
    - ``blossom-scans.com`` (principal)
    - ``blossomscans.com`` (miroir sans tiret)
    - ``blossom-scans.fr`` (miroir francophone)
...
* **Objet JS ``ts_reader``** : le thème Madara de Blossom Scans expose les
...
Example:
    ...
    >>> from nexusdl.parsers.fr.blossom_scans import BlossomScansParser
    >>>
    >>> parser = BlossomScansParser(config, session)
...
_BASE_URL: Final[str] = "https://blossom-scans.com"
_FALLBACK_DOMAINS: Final[tuple[str, ...]] = (
    "https://blossom-scans.com",
    "https://blossomscans.com",
    "https://blossom-scans.fr",
)
_SITE_ID: Final[str] = "blossom_scans"
...
# ---------------------------------------------------------------------------
# Sélecteurs Madara spécifiques à Blossom Scans
# ---------------------------------------------------------------------------

_BLOSSOM_SEARCH_ITEM_SELECTOR: Final[str] = (
    ".page-item-detail.manga, .page-item-detail, "
    ".c-tabs-item__content, .bs, .bsx, "
    ".manga-item, .item-summary, .row.c-tabs-item"
)
_BLOSSOM_CHAPTER_SELECTOR: Final[str] = (
...
_BLOSSOM_PAGE_IMG_SELECTOR: Final[str] = (
...
_BLOSSOM_PAGES_VAR_NAMES: Final[tuple[str, ...]] = (
    "ts_reader",
    "pages",
    "page_urls",
    "pageUrls",
...
class BlossomScansParser(
    JsRenderedMixin,
    CloudflareMixin,
    MadaraMixin,
    BaseParser,
):
    """Parseur Blossom Scans (Madara + Cloudflare).
    ...
    # --- Sélecteurs Madara (standards + ajustements Blossom Scans) ---
    madara_search_item_selector: ClassVar[str] = _BLOSSOM_SEARCH_ITEM_SELECTOR
    ...
    madara_chapter_selector: ClassVar[str] = _BLOSSOM_CHAPTER_SELECTOR
    madara_page_img_selector: ClassVar[str] = _BLOSSOM_PAGE_IMG_SELECTOR
    madara_pages_var_names: ClassVar[tuple[str, ...]] = _BLOSSOM_PAGES_VAR_NAMES
    ...
    def __init__(
        ...
        *,
        ...
        """Initialise le parseur Blossom Scans.
    ...
    def _madara_text(node: Node | None) -> str:
        ...
        if node is None:
            return ""
        return BlossomScansParser._madara_clean(
    ...
    def _madara_chapter_number(text: str) -> float | str:
        ...
        """
        cleaned = BlossomScansParser._madara_clean(text)
    ...
    @staticmethod
    def _blossom_extract_ts_reader(html: str) -> list[str]:
        """Extrait les URLs d'images depuis l'objet JS ``ts_reader``.

        Le thème Madara de Blossom Scans expose les pages dans un objet
        ...
        for pattern in patterns:
            for match in pattern.finditer(html):
                ...
                try:
                    ...
                    continue
                urls = BlossomScansParser._blossom_walk_ts_reader(parsed)
                if urls:
                    return urls
    ...
    @staticmethod
    def _blossom_walk_ts_reader(payload: Any) -> list[str]:
    ...
    async def search(
        ...
        """Recherche des mangas sur Blossom Scans.

        Blossom Scans utilise ``/?s={query}&post_type=wp-manga`` (paramètre
        ...
        """
        self.logger.debug(
            "BlossomScans search: {query} (page {page})",
            query=query,
            page=page,
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec recherche BlossomScans pour {query!r}: {err}",
                query=query,
                err=exc,
            )
            raise ParseError(
    ...
    def _madara_parse_search_html(
        ...
        *,
        ...
        self.logger.info(
            "BlossomScans search: {n} résultat(s)", n=len(results)
        )
        return results

    async def get_manga(self, url_or_id: str) -> Manga:
        """Récupère la fiche complète d'un manga Blossom Scans.
        ...
        self.logger.debug("BlossomScans get_manga: {url}", url=url)
        ...
        try:
            ...
            self.logger.error(
                "Échec get_manga BlossomScans pour {url!r}: {err}",
                url=url,
                err=exc,
            )
            raise ParseError(
        ...
        self.logger.info(
            "BlossomScans get_manga OK: {title} ({n} chapitres)",
            title=title,
            n=len(chapters),
        )
        return manga
    ...
    async def get_chapters(self, manga: Manga) -> list[Chapter]:
        ...
        """
        self.logger.debug(
            "BlossomScans get_chapters: {title}", title=manga.title
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_chapters BlossomScans pour {title!r}: {err}",
                title=manga.title,
                err=exc,
            )
            raise ParseError(
    ...
    def _madara_parse_chapters_from_html(
        ...
        *,
        ...
        self.logger.debug(
            "BlossomScans chapitres: {n}", n=len(chapters)
        )
        return chapters

    async def get_pages(self, chapter: Chapter) -> list[Page]:
        """Récupère les URLs des pages d'un chapitre Blossom Scans.
        ...
        """
        self.logger.debug(
            "BlossomScans get_pages: {url}", url=str(chapter.url)
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_pages BlossomScans pour {url!r}: {err}",
                url=chapter_url,
                err=exc,
            )
            raise ParseError(
        ...
        ts_urls = self._blossom_extract_ts_reader(html)
        if ts_urls:
            for u in ts_urls:
        ...
        self.logger.info(
            "BlossomScans get_pages: {n} page(s) extraite(s)", n=len(pages)
        )
        return pages
    ...
    async def health_check(self) -> bool:
        """Vérifie que Blossom Scans est accessible.
        ...
        for base in self.fallback_domains:
            try:
                ...
                if rendered.ok:
                    self.logger.info(
                        "Health check BlossomScans OK: {base} (status={s})",
                        base=base,
                        s=rendered.status,
                    )
                    return True
                ...
                self.logger.debug(
                    "Health check BlossomScans échec sur {base}: {err}",
                    base=base,
                    err=exc,
                )
        self.logger.warning("Health check BlossomScans KO (tous domaines)")
        return False
