"""Parseur Taisei Scans pour NexusDL.

Taisei Scans (``https://taiseiscans.fr``) est une équipe de scanlation
...
Toonily, ToonGod, Galaxy Scans, CrunchyScan et Urano Scans.
...
* **Domaines** :
    - ``taiseiscans.fr`` (principal)
    - ``taisei-scans.fr`` (miroir avec tiret)
    - ``taiseiscans.com`` (miroir secondaire)
...
* **Objet JS ``ts_reader``** : le thème Madara de Taisei Scans expose les
...
Example:
    ...
    >>> from nexusdl.parsers.fr.taisei_scans import TaiseiScansParser
    >>>
    >>> parser = TaiseiScansParser(config, session)
...
_BASE_URL: Final[str] = "https://taiseiscans.fr"
_FALLBACK_DOMAINS: Final[tuple[str, ...]] = (
    "https://taiseiscans.fr",
    "https://taisei-scans.fr",
    "https://taiseiscans.com",
)
_SITE_ID: Final[str] = "taisei_scans"
...
# ---------------------------------------------------------------------------
# Sélecteurs Madara spécifiques à Taisei Scans
# ---------------------------------------------------------------------------

_TAISEI_SEARCH_ITEM_SELECTOR: Final[str] = (
    ".page-item-detail.manga, .page-item-detail, "
    ".c-tabs-item__content, .bs, .bsx, "
    ".manga-item, .item-summary, .row.c-tabs-item"
)
_TAISEI_CHAPTER_SELECTOR: Final[str] = (
...
_TAISEI_PAGE_IMG_SELECTOR: Final[str] = (
...
_TAISEI_PAGES_VAR_NAMES: Final[tuple[str, ...]] = (
    "ts_reader",
    "pages",
    "page_urls",
    "pageUrls",
...
class TaiseiScansParser(
    JsRenderedMixin,
    CloudflareMixin,
    MadaraMixin,
    BaseParser,
):
    """Parseur Taisei Scans (Madara + Cloudflare).
    ...
    # --- Sélecteurs Madara (standards + ajustements Taisei Scans) ---
    madara_search_item_selector: ClassVar[str] = _TAISEI_SEARCH_ITEM_SELECTOR
    ...
    madara_chapter_selector: ClassVar[str] = _TAISEI_CHAPTER_SELECTOR
    madara_page_img_selector: ClassVar[str] = _TAISEI_PAGE_IMG_SELECTOR
    madara_pages_var_names: ClassVar[tuple[str, ...]] = _TAISEI_PAGES_VAR_NAMES
    ...
    def __init__(
        ...
        *,
        ...
        """Initialise le parseur Taisei Scans.
    ...
    def _madara_text(node: Node | None) -> str:
        ...
        if node is None:
            return ""
        return TaiseiScansParser._madara_clean(
    ...
    def _madara_chapter_number(text: str) -> float | str:
        ...
        """
        cleaned = TaiseiScansParser._madara_clean(text)
    ...
    @staticmethod
    def _taisei_extract_ts_reader(html: str) -> list[str]:
        """Extrait les URLs d'images depuis l'objet JS ``ts_reader``.

        Le thème Madara de Taisei Scans expose les pages dans un objet
        ...
        for pattern in patterns:
            for match in pattern.finditer(html):
                ...
                try:
                    ...
                    continue
                urls = TaiseiScansParser._taisei_walk_ts_reader(parsed)
                if urls:
                    return urls
    ...
    @staticmethod
    def _taisei_walk_ts_reader(payload: Any) -> list[str]:
    ...
    async def search(
        ...
        """Recherche des mangas sur Taisei Scans.

        Taisei Scans utilise ``/?s={query}&post_type=wp-manga`` (paramètre
        ...
        """
        self.logger.debug(
            "TaiseiScans search: {query} (page {page})",
            query=query,
            page=page,
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec recherche TaiseiScans pour {query!r}: {err}",
                query=query,
                err=exc,
            )
            raise ParseError(
    ...
    def _madara_parse_search_html(
        ...
        *,
        ...
        self.logger.info(
            "TaiseiScans search: {n} résultat(s)", n=len(results)
        )
        return results

    async def get_manga(self, url_or_id: str) -> Manga:
        """Récupère la fiche complète d'un manga Taisei Scans.
        ...
        self.logger.debug("TaiseiScans get_manga: {url}", url=url)
        ...
        try:
            ...
            self.logger.error(
                "Échec get_manga TaiseiScans pour {url!r}: {err}",
                url=url,
                err=exc,
            )
            raise ParseError(
        ...
        self.logger.info(
            "TaiseiScans get_manga OK: {title} ({n} chapitres)",
            title=title,
            n=len(chapters),
        )
        return manga
    ...
    async def get_chapters(self, manga: Manga) -> list[Chapter]:
        ...
        """
        self.logger.debug(
            "TaiseiScans get_chapters: {title}", title=manga.title
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_chapters TaiseiScans pour {title!r}: {err}",
                title=manga.title,
                err=exc,
            )
            raise ParseError(
    ...
    def _madara_parse_chapters_from_html(
        ...
        *,
        ...
        self.logger.debug(
            "TaiseiScans chapitres: {n}", n=len(chapters)
        )
        return chapters

    async def get_pages(self, chapter: Chapter) -> list[Page]:
        """Récupère les URLs des pages d'un chapitre Taisei Scans.
        ...
        """
        self.logger.debug(
            "TaiseiScans get_pages: {url}", url=str(chapter.url)
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_pages TaiseiScans pour {url!r}: {err}",
                url=chapter_url,
                err=exc,
            )
            raise ParseError(
        ...
        ts_urls = self._taisei_extract_ts_reader(html)
        if ts_urls:
            for u in ts_urls:
        ...
        self.logger.info(
            "TaiseiScans get_pages: {n} page(s) extraite(s)", n=len(pages)
        )
        return pages
    ...
    async def health_check(self) -> bool:
        """Vérifie que Taisei Scans est accessible.
        ...
        for base in self.fallback_domains:
            try:
                ...
                if rendered.ok:
                    self.logger.info(
                        "Health check TaiseiScans OK: {base} (status={s})",
                        base=base,
                        s=rendered.status,
                    )
                    return True
                ...
                self.logger.debug(
                    "Health check TaiseiScans échec sur {base}: {err}",
                    base=base,
                    err=exc,
                )
        self.logger.warning("Health check TaiseiScans KO (tous domaines)")
        return False
