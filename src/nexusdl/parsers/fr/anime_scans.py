"""Parseur Anime Scans pour NexusDL.

Anime Scans (``https://anime-scan.fr``) est une équipe de scanlation
...
également utilisé par Toonily, ToonGod, Galaxy Scans, CrunchyScan,
Urano Scans, Taisei Scans, SushiScan.net, SushiScan.fr, Shinra Scans,
Rimuscan, Raijin Scans, Poseidon Scans, Phenix Scans, Ortegascans,
Karma Scans, FanDub Scans, Epsilon Scan, CrunchyScan FR et Blossom Scans.

⚠️ **Note importante** : malgré son nom « Anime Scans », ce site est un
site de **scanlation** (traduction écrite de mangas/manhwas), pas un site
d'anime (streaming vidéo). Le nom est historique et provient d'un ancien
projet multimédia de l'équipe.
...
* **Domaines** :
    - ``anime-scan.fr`` (principal)
    - ``animescan.fr`` (miroir sans tiret)
    - ``anime-scan.com`` (miroir secondaire)
...
* **Objet JS ``ts_reader``** : le thème Madara d'Anime Scans expose les
...
Example:
    ...
    >>> from nexusdl.parsers.fr.anime_scans import AnimeScansParser
    >>>
    >>> parser = AnimeScansParser(config, session)
...
_BASE_URL: Final[str] = "https://anime-scan.fr"
_FALLBACK_DOMAINS: Final[tuple[str, ...]] = (
    "https://anime-scan.fr",
    "https://animescan.fr",
    "https://anime-scan.com",
)
_SITE_ID: Final[str] = "anime_scans"
...
# ---------------------------------------------------------------------------
# Sélecteurs Madara spécifiques à Anime Scans
# ---------------------------------------------------------------------------

_ANIME_SEARCH_ITEM_SELECTOR: Final[str] = (
    ".page-item-detail.manga, .page-item-detail, "
    ".c-tabs-item__content, .bs, .bsx, "
    ".manga-item, .item-summary, .row.c-tabs-item"
)
_ANIME_CHAPTER_SELECTOR: Final[str] = (
...
_ANIME_PAGE_IMG_SELECTOR: Final[str] = (
...
_ANIME_PAGES_VAR_NAMES: Final[tuple[str, ...]] = (
    "ts_reader",
    "pages",
    "page_urls",
    "pageUrls",
...
class AnimeScansParser(
    JsRenderedMixin,
    CloudflareMixin,
    MadaraMixin,
    BaseParser,
):
    """Parseur Anime Scans (Madara + Cloudflare).
    ...
    # --- Sélecteurs Madara (standards + ajustements Anime Scans) ---
    madara_search_item_selector: ClassVar[str] = _ANIME_SEARCH_ITEM_SELECTOR
    ...
    madara_chapter_selector: ClassVar[str] = _ANIME_CHAPTER_SELECTOR
    madara_page_img_selector: ClassVar[str] = _ANIME_PAGE_IMG_SELECTOR
    madara_pages_var_names: ClassVar[tuple[str, ...]] = _ANIME_PAGES_VAR_NAMES
    ...
    def __init__(
        ...
        *,
        ...
        """Initialise le parseur Anime Scans.
    ...
    def _madara_text(node: Node | None) -> str:
        ...
        if node is None:
            return ""
        return AnimeScansParser._madara_clean(
    ...
    def _madara_chapter_number(text: str) -> float | str:
        ...
        """
        cleaned = AnimeScansParser._madara_clean(text)
    ...
    @staticmethod
    def _anime_extract_ts_reader(html: str) -> list[str]:
        """Extrait les URLs d'images depuis l'objet JS ``ts_reader``.

        Le thème Madara d'Anime Scans expose les pages dans un objet
        ...
        for pattern in patterns:
            for match in pattern.finditer(html):
                ...
                try:
                    ...
                    continue
                urls = AnimeScansParser._anime_walk_ts_reader(parsed)
                if urls:
                    return urls
    ...
    @staticmethod
    def _anime_walk_ts_reader(payload: Any) -> list[str]:
    ...
    async def search(
        ...
        """Recherche des mangas sur Anime Scans.

        Anime Scans utilise ``/?s={query}&post_type=wp-manga`` (paramètre
        ...
        """
        self.logger.debug(
            "AnimeScans search: {query} (page {page})",
            query=query,
            page=page,
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec recherche AnimeScans pour {query!r}: {err}",
                query=query,
                err=exc,
            )
            raise ParseError(
    ...
    def _madara_parse_search_html(
        ...
        *,
        ...
        self.logger.info(
            "AnimeScans search: {n} résultat(s)", n=len(results)
        )
        return results

    async def get_manga(self, url_or_id: str) -> Manga:
        """Récupère la fiche complète d'un manga Anime Scans.
        ...
        self.logger.debug("AnimeScans get_manga: {url}", url=url)
        ...
        try:
            ...
            self.logger.error(
                "Échec get_manga AnimeScans pour {url!r}: {err}",
                url=url,
                err=exc,
            )
            raise ParseError(
        ...
        self.logger.info(
            "AnimeScans get_manga OK: {title} ({n} chapitres)",
            title=title,
            n=len(chapters),
        )
        return manga
    ...
    async def get_chapters(self, manga: Manga) -> list[Chapter]:
        ...
        """
        self.logger.debug(
            "AnimeScans get_chapters: {title}", title=manga.title
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_chapters AnimeScans pour {title!r}: {err}",
                title=manga.title,
                err=exc,
            )
            raise ParseError(
    ...
    def _madara_parse_chapters_from_html(
        ...
        *,
        ...
        self.logger.debug(
            "AnimeScans chapitres: {n}", n=len(chapters)
        )
        return chapters

    async def get_pages(self, chapter: Chapter) -> list[Page]:
        """Récupère les URLs des pages d'un chapitre Anime Scans.
        ...
        """
        self.logger.debug(
            "AnimeScans get_pages: {url}", url=str(chapter.url)
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_pages AnimeScans pour {url!r}: {err}",
                url=chapter_url,
                err=exc,
            )
            raise ParseError(
        ...
        ts_urls = self._anime_extract_ts_reader(html)
        if ts_urls:
            for u in ts_urls:
        ...
        self.logger.info(
            "AnimeScans get_pages: {n} page(s) extraite(s)", n=len(pages)
        )
        return pages
    ...
    async def health_check(self) -> bool:
        """Vérifie que Anime Scans est accessible.
        ...
        for base in self.fallback_domains:
            try:
                ...
                if rendered.ok:
                    self.logger.info(
                        "Health check AnimeScans OK: {base} (status={s})",
                        base=base,
                        s=rendered.status,
                    )
                    return True
                ...
                self.logger.debug(
                    "Health check AnimeScans échec sur {base}: {err}",
                    base=base,
                    err=exc,
                )
        self.logger.warning("Health check AnimeScans KO (tous domaines)")
        return False
