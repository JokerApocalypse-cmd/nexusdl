"""Parseur Ortegascans COM pour NexusDL.

Ortegascans (``https://ortegascans.com``) est un site de scanlation
**multilingue** (espagnol, anglais, français) géré par une équipe
hispanophone. Cette instance ``.com`` est l'**instance principale
espagnole** — elle est référencée comme fallback par le parser
:class:`~nexusdl.parsers.fr.ortegascans_fr.OrtegascansFrParser` en raison
du partage de catalogue partiel entre les deux instances.

Le site utilise le thème WordPress **Madara** (Mangabooth), également
utilisé par Toonily, ToonGod, Galaxy Scans, CrunchyScan, Urano Scans,
Taisei Scans, SushiScan.net, SushiScan.fr, Shinra Scans, Rimuscan, Raijin
Scans, Poseidon Scans, Phenix Scans et Ortegascans FR.

⚠️ **Note importante** : Ortegascans existe en plusieurs variantes
linguistiques :

* ``ortegascans.com`` — instance principale espagnole (ce parser).
* ``ortegascans.fr`` — instance française (parser distinct
  :class:`~nexusdl.parsers.fr.ortegascans_fr.OrtegascansFrParser`).
* ``ortegascans.net`` — miroir multi-langue.

Les trois instances partagent le même thème Madara mais ont des catalogues
**partiellement distincts** (chaque instance a ses propres traductions).

Caractéristiques techniques
---------------------------

* **Moteur** : WordPress **Madara** (thème premium Mangabooth).
* **Langue principale** : Espagnol (``es``) — avec support multilingue.
* **Protection** : Cloudflare (AS13335, IP ``104.21.16.85`` /
  ``172.67.172.115``) avec challenge modéré.
* **Domaines** :
    - ``ortegascans.com`` (principal, instance ES)
    - ``ortega-scans.com`` (miroir avec tiret)
    - ``ortegascans.net`` (miroir multi-langue)
    - ``ortegascans.fr`` (instance FR, fallback)
...
Example:
    ...
    >>> from nexusdl.parsers.fr.ortegascans_com import OrtegascansComParser
    >>>
    >>> parser = OrtegascansComParser(config, session)
...
_BASE_URL: Final[str] = "https://ortegascans.com"
_FALLBACK_DOMAINS: Final[tuple[str, ...]] = (
    "https://ortegascans.com",
    "https://ortega-scans.com",
    "https://ortegascans.net",
    "https://ortegascans.fr",
)
_SITE_ID: Final[str] = "ortegascans_com"
_LANGUAGE: Final[Language] = Language.ES
_ADULT: Final[bool] = False

# Regex de parsing des chapitres (format Madara multi-langue).
# Supporte "Chapitre 47", "Chapter 47", "Capítulo 47", "Cap. 47".
_CHAPTER_NUMBER_RE: Final[re.Pattern[str]] = re.compile(
    r"(?:chapitre|chapter|chap\.?|ch\.?|cap[ií]tulo|cap\.?|"
    r"episode|ep\.?|ep)\s*(\d+(?:[.,]\d+)?)",
    re.IGNORECASE,
)
_VOLUME_RE: Final[re.Pattern[str]] = re.compile(
    r"(?:tome|volume|vol\.?|tomo)\s*(\d+)", re.IGNORECASE
)
_RELATIVE_DATE_RE: Final[re.Pattern[str]] = re.compile(
    r"(\d+)\s*(seconde|minute|heure|jour|semaine|mois|an|"
    r"segundo|minuto|hora|d[ií]a|semana|mes|a[ñn]o|"
    r"second|minute|hour|day|week|month|year)s?\s*(ago|avant|hace)?",
    re.IGNORECASE,
)

# Statuts → énumération NexusDL (espagnol + anglais + français).
_STATUS_MAP: Final[dict[str, MangaStatus]] = {
    # Espagnol (langue principale)
    "en curso": MangaStatus.ONGOING,
    "emisión": MangaStatus.ONGOING,
    "emision": MangaStatus.ONGOING,
    "publicándose": MangaStatus.ONGOING,
    "publicandose": MangaStatus.ONGOING,
    "completado": MangaStatus.COMPLETED,
    "finalizado": MangaStatus.COMPLETED,
    "terminado": MangaStatus.COMPLETED,
    "en pausa": MangaStatus.HIATUS,
    "pausado": MangaStatus.HIATUS,
    "cancelado": MangaStatus.CANCELLED,
    "abandonado": MangaStatus.CANCELLED,
    "descontinuado": MangaStatus.CANCELLED,
    # Anglais
...
# ---------------------------------------------------------------------------
# Sélecteurs Madara spécifiques à Ortegascans COM
# ---------------------------------------------------------------------------

_ORTEGA_SEARCH_ITEM_SELECTOR: Final[str] = (
    ".page-item-detail.manga, .page-item-detail, "
    ".c-tabs-item__content, .bs, .bsx, "
    ".manga-item, .item-summary, .row.c-tabs-item"
)
_ORTEGA_CHAPTER_SELECTOR: Final[str] = (
...
_ORTEGA_PAGE_IMG_SELECTOR: Final[str] = (
...
_ORTEGA_PAGES_VAR_NAMES: Final[tuple[str, ...]] = (
    "ts_reader",
    "pages",
    "page_urls",
    "pageUrls",
...
class OrtegascansComParser(
    JsRenderedMixin,
    CloudflareMixin,
    MadaraMixin,
    BaseParser,
):
    """Parseur Ortegascans COM (Madara + Cloudflare).
    ...
    Attributes:
        ...
        language: Langue principale (espagnol).
    ...
    # --- Sélecteurs Madara (standards + ajustements Ortegascans COM) ---
    madara_search_item_selector: ClassVar[str] = _ORTEGA_SEARCH_ITEM_SELECTOR
    ...
    madara_chapter_selector: ClassVar[str] = _ORTEGA_CHAPTER_SELECTOR
    madara_page_img_selector: ClassVar[str] = _ORTEGA_PAGE_IMG_SELECTOR
    madara_pages_var_names: ClassVar[tuple[str, ...]] = _ORTEGA_PAGES_VAR_NAMES

    # --- Comportement Madara ---
    madara_requires_js: ClassVar[bool] = False
    madara_api_pages_endpoint: ClassVar[str | None] = None
    madara_default_rating: ClassVar[ContentRating] = ContentRating.SAFE
    ...
    locale: ClassVar[str] = "es-ES"
    timezone_id: ClassVar[str] = "Europe/Madrid"
    ...
    def __init__(
        ...
        *,
        ...
        """Initialise le parseur Ortegascans COM.
    ...
    def _madara_text(node: Node | None) -> str:
        ...
        if node is None:
            return ""
        return OrtegascansComParser._madara_clean(
    ...
    def _madara_parse_status(self, raw: str | None) -> MangaStatus:
        ...
        Args:
            raw: Libellé brut (espagnol, anglais ou français).
    ...
    def _madara_parse_relative_date(raw: str | None) -> datetime | None:
        """Parse une date relative (espagnol, français ou anglais).

        Supporte : ``"hace 3 días"``, ``"il y a 3 jours"``,
        ``"3 days ago"``.
        ...
        table: dict[str, timedelta] = {
            # Espagnol (langue principale)
            "segundo": timedelta(seconds=amount),
            "minuto": timedelta(minutes=amount),
            "hora": timedelta(hours=amount),
            "día": timedelta(days=amount),
            "dia": timedelta(days=amount),
            "semana": timedelta(weeks=amount),
            "mes": timedelta(days=amount * 30),
            "año": timedelta(days=amount * 365),
            "ano": timedelta(days=amount * 365),
            # Français
    ...
    def _madara_chapter_number(text: str) -> float | str:
        ...
        Supporte : ``"Capítulo 47"``, ``"Chapter 47"``, ``"Chapitre 47"``.
        ...
        """
        cleaned = OrtegascansComParser._madara_clean(text)
    ...
    def _madara_chapter_volume(text: str) -> int | None:
        ...
        Supporte : ``"Tomo 3"``, ``"Tome 3"``, ``"Volume 3"``.
    ...
    @staticmethod
    def _ortega_extract_ts_reader(html: str) -> list[str]:
        """Extrait les URLs d'images depuis l'objet JS ``ts_reader``.

        Le thème Madara d'Ortegascans expose les pages dans un objet
        ...
        for pattern in patterns:
            for match in pattern.finditer(html):
                ...
                try:
                    ...
                    continue
                urls = OrtegascansComParser._ortega_walk_ts_reader(parsed)
                if urls:
                    return urls
    ...
    @staticmethod
    def _ortega_walk_ts_reader(payload: Any) -> list[str]:
    ...
    async def search(
        ...
        """Recherche des mangas sur Ortegascans COM.

        Ortegascans COM utilise ``/?s={query}&post_type=wp-manga``
        ...
        """
        self.logger.debug(
            "OrtegascansCom search: {query} (page {page})",
            query=query,
            page=page,
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec recherche OrtegascansCom pour {query!r}: {err}",
                query=query,
                err=exc,
            )
            raise ParseError(
    ...
    def _madara_parse_search_html(
        ...
        *,
        ...
        self.logger.info(
            "OrtegascansCom search: {n} résultat(s)", n=len(results)
        )
        return results

    async def get_manga(self, url_or_id: str) -> Manga:
        """Récupère la fiche complète d'un manga Ortegascans COM.
        ...
        self.logger.debug("OrtegascansCom get_manga: {url}", url=url)
        ...
        try:
            ...
            self.logger.error(
                "Échec get_manga OrtegascansCom pour {url!r}: {err}",
                url=url,
                err=exc,
            )
            raise ParseError(
        ...
        self.logger.info(
            "OrtegascansCom get_manga OK: {title} ({n} chapitres)",
            title=title,
            n=len(chapters),
        )
        return manga
    ...
    async def get_chapters(self, manga: Manga) -> list[Chapter]:
        ...
        """
        self.logger.debug(
            "OrtegascansCom get_chapters: {title}", title=manga.title
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_chapters OrtegascansCom pour {title!r}: {err}",
                title=manga.title,
                err=exc,
            )
            raise ParseError(
    ...
    def _madara_parse_chapters_from_html(
        ...
        *,
        ...
        self.logger.debug(
            "OrtegascansCom chapitres: {n}", n=len(chapters)
        )
        return chapters

    async def get_pages(self, chapter: Chapter) -> list[Page]:
        """Récupère les URLs des pages d'un chapitre Ortegascans COM.
        ...
        """
        self.logger.debug(
            "OrtegascansCom get_pages: {url}", url=str(chapter.url)
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_pages OrtegascansCom pour {url!r}: {err}",
                url=chapter_url,
                err=exc,
            )
            raise ParseError(
        ...
        ts_urls = self._ortega_extract_ts_reader(html)
        if ts_urls:
            for u in ts_urls:
        ...
        self.logger.info(
            "OrtegascansCom get_pages: {n} page(s) extraite(s)", n=len(pages)
        )
        return pages
    ...
    async def health_check(self) -> bool:
        """Vérifie que Ortegascans COM est accessible.
        ...
        for base in self.fallback_domains:
            try:
                ...
                if rendered.ok:
                    self.logger.info(
                        "Health check OrtegascansCom OK: {base} (status={s})",
                        base=base,
                        s=rendered.status,
                    )
                    return True
                ...
                self.logger.debug(
                    "Health check OrtegascansCom échec sur {base}: {err}",
                    base=base,
                    err=exc,
                )
        self.logger.warning("Health check OrtegascansCom KO (tous domaines)")
        return False
