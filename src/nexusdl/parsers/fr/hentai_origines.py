"""Parseur Hentai Origines pour NexusDL.

Hentai Origines (``https://hentai-origines.fr``) est une équipe de
scanlation **francophone** spécialisée dans le **hentai** et les mangas
adultes (18+/NSFW), traduits en français. Le site fait partie de la
« famille Origines » (Mangas Origines) mais se distingue par sa
spécialisation exclusive dans le contenu adulte.

Le site utilise le CMS **FoolSlide** — un moteur PHP historique très
répandu dans la communauté scan FR avant l'avènement de WordPress/Madara.
Cette proximité avec Mangas Origines (même équipe) implique un partage
partiel de l'infrastructure et des sélecteurs CSS.
...
* **Protection** : Cloudflare (AS13335, IP ``104.21.16.85`` /
  ...
  **Referer strict** sur les images (hotlink bloqué) et un **age gate**
  cookie (``adult=1``).
* **Domaines** :
    - ``hentai-origines.fr`` (principal)
    - ``hentaiorigines.fr`` (miroir sans tiret)
    - ``hentai-origines.com`` (miroir historique)
...
* **Contenu adulte** : site 18+/NSFW **exclusivement** (hentai, ecchi,
  doujinshi adulte, manga érotique). Age gate cookie obligatoire.
...
Example:
    ...
    >>> from nexusdl.parsers.fr.hentai_origines import HentaiOriginesParser
    >>>
    >>> parser = HentaiOriginesParser(config, session)
...
_BASE_URL: Final[str] = "https://hentai-origines.fr"
_FALLBACK_DOMAINS: Final[tuple[str, ...]] = (
    "https://hentai-origines.fr",
    "https://hentaiorigines.fr",
    "https://hentai-origines.com",
)
_SITE_ID: Final[str] = "hentai_origines"
_LANGUAGE: Final[Language] = Language.FR
_ADULT: Final[bool] = True  # Site 18+/NSFW exclusif (hentai)

# Cookie age gate (obligatoire pour accéder au contenu).
_AGE_GATE_COOKIE_NAME: Final[str] = "adult"
_AGE_GATE_COOKIE_VALUE: Final[str] = "1"

# Regex de parsing des chapitres (format FoolSlide FR).
# Supporte "Chapitre 47", "Chapter 47", "Ch. 47", "Scan 47", "Vol. 2 Ch. 15".
...
# Ce parser étant 18+/NSFW exclusif, tous les contenus sont au minimum
# EROTICA, et souvent PORNOGRAPHIC.
...
# Marqueurs de contenu explicite (pornographique).
_PORNOGRAPHIC_MARKERS: Final[frozenset[str]] = frozenset(
    {
        "hentai",
        "porn",
        "18+",
...
# Marqueurs de contenu érotique (moins explicite).
_EROTIC_MARKERS: Final[frozenset[str]] = frozenset(
    {
        "ecchi",
        "smut",
        "mature",
...
# ---------------------------------------------------------------------------
# Sélecteurs FoolSlide spécifiques à Hentai Origines
# ---------------------------------------------------------------------------

_HO_SEARCH_ITEM_SELECTOR: Final[str] = (
...
_HO_SEARCH_LINK_SELECTOR: Final[str] = "a"
_HO_SEARCH_COVER_SELECTOR: Final[str] = "img"
_HO_SEARCH_TITLE_SELECTOR: Final[str] = (
    ".manga-title, .title, h3, h4, .manga-name"
)

_HO_MANGA_TITLE_SELECTOR: Final[str] = (
    "h1.manga-title, h1.title, h1, .manga-title, .title"
)
_HO_MANGA_COVER_SELECTOR: Final[str] = (
    ".manga-cover img, .cover img, .thumbnail img, "
    "img.manga-cover, img.cover"
)
_HO_MANGA_DESCRIPTION_SELECTOR: Final[str] = (
    ".manga-summary, .summary, .description, "
    ".synopsis, .manga-description"
)
_HO_MANGA_AUTHOR_SELECTOR: Final[str] = (
    ".manga-author, .author, a[href*='/author/'], "
    "a[href*='/auteur/']"
)
_HO_MANGA_ARTIST_SELECTOR: Final[str] = (
    ".manga-artist, .artist, a[href*='/artist/'], "
    "a[href*='/artiste/']"
)
_HO_MANGA_GENRES_SELECTOR: Final[str] = (
    ".manga-genres a, .genres a, "
    "a[href*='/genre/'], a[href*='/tag/']"
)
_HO_MANGA_STATUS_SELECTOR: Final[str] = (
    ".manga-status, .status"
)

_HO_CHAPTER_SELECTOR: Final[str] = (
    ".chapter-list a, "
...
_HO_PAGE_IMG_SELECTOR: Final[str] = (
    "img.page-image, "
    ".reader-content img, "
    ".page-content img, "
    "div.page-image img, "
...
_HO_PAGES_VAR_NAMES: Final[tuple[str, ...]] = (
    "pages",
    "page_urls",
    "pageUrls",
    "images",
...
class HentaiOriginesParser(
    JsRenderedMixin,
    CloudflareMixin,
    FoolSlideMixin,
    BaseParser,
):
    """Parseur Hentai Origines (FoolSlide + Cloudflare).
    ...
    Attributes:
        ...
        adult: Contenu adulte (``True`` — site 18+/NSFW exclusif).
    ...
    # --- Sélecteurs FoolSlide (standards + ajustements Hentai Origines) ---
    fools_search_item_selector: ClassVar[str] = _HO_SEARCH_ITEM_SELECTOR
    fools_search_link_selector: ClassVar[str] = _HO_SEARCH_LINK_SELECTOR
    fools_search_cover_selector: ClassVar[str] = _HO_SEARCH_COVER_SELECTOR
    fools_search_title_selector: ClassVar[str] = _HO_SEARCH_TITLE_SELECTOR

    fools_manga_title_selector: ClassVar[str] = _HO_MANGA_TITLE_SELECTOR
    fools_manga_cover_selector: ClassVar[str] = _HO_MANGA_COVER_SELECTOR
    fools_manga_description_selector: ClassVar[str] = _HO_MANGA_DESCRIPTION_SELECTOR
    fools_manga_author_selector: ClassVar[str] = _HO_MANGA_AUTHOR_SELECTOR
    fools_manga_artist_selector: ClassVar[str] = _HO_MANGA_ARTIST_SELECTOR
    fools_manga_genres_selector: ClassVar[str] = _HO_MANGA_GENRES_SELECTOR
    fools_manga_status_selector: ClassVar[str] = _HO_MANGA_STATUS_SELECTOR

    fools_chapter_selector: ClassVar[str] = _HO_CHAPTER_SELECTOR
    fools_page_img_selector: ClassVar[str] = _HO_PAGE_IMG_SELECTOR
    fools_pages_var_names: ClassVar[tuple[str, ...]] = _HO_PAGES_VAR_NAMES

    # --- Comportement FoolSlide ---
    fools_requires_js: ClassVar[bool] = False
    fools_api_pages_endpoint: ClassVar[str | None] = None
    fools_default_rating: ClassVar[ContentRating] = ContentRating.PORNOGRAPHIC
    ...
    ho_ajax_endpoint: ClassVar[str] = "/ajax/lecture.php"
    ...
    def __init__(
        ...
        *,
        ...
        """Initialise le parseur Hentai Origines.
        ...
        # Injecte le cookie d'age gate (adult=1).
        self._inject_age_gate_cookie()
    ...
    def _inject_age_gate_cookie(self) -> None:
        """Injecte le cookie d'age gate (``adult=1``).

        Hentai Origines exige un cookie ``adult=1`` pour accéder au
        contenu NSFW. Sans ce cookie, le site redirige vers une page
        d'avertissement 18+.
        """
        target = getattr(self.session, "cookies", None)
        if isinstance(target, dict):
            target[_AGE_GATE_COOKIE_NAME] = _AGE_GATE_COOKIE_VALUE
            self.logger.debug(
                "Cookie age gate 'adult=1' injecté dans la session"
            )
            return
        updater = getattr(self.session, "update_cookies", None)
        if callable(updater):
            try:
                updater({_AGE_GATE_COOKIE_NAME: _AGE_GATE_COOKIE_VALUE})
                self.logger.debug(
                    "Cookie age gate 'adult=1' injecté via update_cookies"
                )
            except Exception as exc:  # noqa: BLE001
                self.logger.debug(
                    "Échec injection cookie age gate : {err}", err=exc
                )
    ...
    def _fools_text(node: Node | None) -> str:
        ...
        if node is None:
            return ""
        return HentaiOriginesParser._fools_clean(
    ...
    def _fools_chapter_number(text: str) -> float | str:
        ...
        """
        cleaned = HentaiOriginesParser._fools_clean(text)
    ...
    def _fools_detect_rating(self, genres: list[str]) -> ContentRating:
        ...
        Hentai Origines est un site 18+/NSFW **exclusif** : tous les
        contenus sont au minimum ``EROTICA``, et la majorité sont
        ``PORNOGRAPHIC``. La méthode affine la classification en fonction
        des genres détectés.
        ...
        Returns:
            Classification détectée (jamais ``SAFE``).
        """
        lowered = {g.lower() for g in genres}

        # Priorité 1 : marqueurs explicites de pornographie.
        if lowered & _PORNOGRAPHIC_MARKERS:
            return ContentRating.PORNOGRAPHIC

        # Priorité 2 : marqueurs érotiques.
        if lowered & _EROTIC_MARKERS:
            return ContentRating.EROTICA

        # Priorité 3 : marqueurs adultes génériques.
        if lowered & _ADULT_GENRE_MARKERS:
            return ContentRating.PORNOGRAPHIC

        # Défaut : site 18+/NSFW exclusif → PORNOGRAPHIC.
        return self.fools_default_rating

    # ------------------------------------------------------------------
    # Helpers Hentai Origines spécifiques
    # ------------------------------------------------------------------

    @staticmethod
    def _ho_extract_chapter_id(url: str) -> str | None:
        """Extrait l'ID de chapitre depuis une URL Hentai Origines.
    ...
    @staticmethod
    def _ho_extract_pages_from_js(html: str) -> list[str]:
    ...
    async def search(
        ...
        """Recherche des mangas hentai sur Hentai Origines.

        Hentai Origines utilise ``/search/?q={query}`` (paramètre
        FoolSlide).
        ...
        """
        self.logger.debug(
            "HentaiOrigines search: {query} (page {page})",
            query=query,
            page=page,
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec recherche HentaiOrigines pour {query!r}: {err}",
                query=query,
                err=exc,
            )
            raise ParseError(
    ...
    def _fools_parse_search_html(
        ...
        *,
        ...
        self.logger.info(
            "HentaiOrigines search: {n} résultat(s)", n=len(results)
        )
        return results

    async def get_manga(self, url_or_id: str) -> Manga:
        """Récupère la fiche complète d'un manga Hentai Origines.
        ...
        self.logger.debug("HentaiOrigines get_manga: {url}", url=url)
        ...
        try:
            ...
            self.logger.error(
                "Échec get_manga HentaiOrigines pour {url!r}: {err}",
                url=url,
                err=exc,
            )
            raise ParseError(
        ...
        self.logger.info(
            "HentaiOrigines get_manga OK: {title} ({n} chapitres)",
            title=title,
            n=len(chapters),
        )
        return manga
    ...
    def _fools_extract_series_slug(url: str) -> str:
        """Extrait un identifiant de série depuis une URL Hentai Origines.
    ...
    async def get_chapters(self, manga: Manga) -> list[Chapter]:
        ...
        """
        self.logger.debug(
            "HentaiOrigines get_chapters: {title}", title=manga.title
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_chapters HentaiOrigines pour {title!r}: {err}",
                title=manga.title,
                err=exc,
            )
            raise ParseError(
    ...
    def _fools_parse_chapters_from_html(
        ...
        *,
        ...
        for node in self._fools_all(tree, self.fools_chapter_selector):
            ...
            chapter_id = self._ho_extract_chapter_id(abs_url)
        ...
        self.logger.debug(
            "HentaiOrigines chapitres: {n}", n=len(chapters)
        )
        return chapters

    async def get_pages(self, chapter: Chapter) -> list[Page]:
        """Récupère les URLs des pages d'un chapitre Hentai Origines.

        Hentai Origines (FoolSlide) expose les pages dans une variable JS
        ...
        """
        self.logger.debug(
            "HentaiOrigines get_pages: {url}", url=str(chapter.url)
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_pages HentaiOrigines pour {url!r}: {err}",
                url=chapter_url,
                err=exc,
            )
            raise ParseError(
        ...
        js_urls = self._ho_extract_pages_from_js(html)
        if js_urls:
            for u in js_urls:
        ...
        if not urls:
            for node in self._fools_all(tree, self.fools_page_img_selector):
                ...
                if not src:
                    continue
                # Ignore les images non-chapitre (logos, bannières, age gate).
                if any(
                    marker in src.lower()
                    for marker in (
                        "logo",
        ...
        if not urls:
            chapter_id = (
                self._ho_extract_chapter_id(chapter_url) or chapter.source_id
            )
            if chapter_id and chapter_id.isdigit():
                urls = await self._ho_fetch_pages_via_ajax(chapter_id)
        ...
        self.logger.info(
            "HentaiOrigines get_pages: {n} page(s) extraite(s)",
            n=len(pages),
        )
        return pages

    async def _ho_fetch_pages_via_ajax(
        self, chapter_id: str
    ) -> list[str]:
        ...
        while page_num <= max_pages:
            endpoint = (
                f"{self.base_url}{self.ho_ajax_endpoint}"
    ...
    async def health_check(self) -> bool:
        """Vérifie que Hentai Origines est accessible.
        ...
        for base in self.fallback_domains:
            try:
                ...
                if rendered.ok:
                    self.logger.info(
                        "Health check HentaiOrigines OK: {base} (status={s})",
                        base=base,
                        s=rendered.status,
                    )
                    return True
                ...
                self.logger.debug(
                    "Health check HentaiOrigines échec sur {base}: {err}",
                    base=base,
                    err=exc,
                )
        self.logger.warning(
            "Health check HentaiOrigines KO (tous domaines)"
        )
        return False
