"""Parseur Scan-Manga pour NexusDL.

Scan-Manga (``https://www.scan-manga.com``) est l'un des plus anciens sites
de scanlation **francophone** encore en activité (créé en 2007). Il utilise
le CMS **FoolSlide** — un moteur PHP historique très répandu dans la
communauté scan FR avant l'avènement de WordPress/Madara.
...
* **Domaines** :
    - ``scan-manga.com`` (principal, préfixe ``www.``)
    - ``scan-manga.net`` (miroir)
    - ``scanmanga.com`` (miroir avec tiret)
...
⚠️ **Note importante** : Scan-Manga utilise l'extension ``.html`` sur
toutes ses URLs (à la fois pour les mangas et les chapitres). C'est une
particularité de leur déploiement FoolSlide.
...
Example:
    ...
    >>> from nexusdl.parsers.fr.scan_manga import ScanMangaParser
    >>>
    >>> parser = ScanMangaParser(config, session)
...
_BASE_URL: Final[str] = "https://www.scan-manga.com"
_FALLBACK_DOMAINS: Final[tuple[str, ...]] = (
    "https://www.scan-manga.com",
    "https://www.scan-manga.net",
    "https://www.scanmanga.com",
)
_SITE_ID: Final[str] = "scan_manga"
...
# ---------------------------------------------------------------------------
# Sélecteurs FoolSlide spécifiques à Scan-Manga
# ---------------------------------------------------------------------------

_SCAN_SEARCH_ITEM_SELECTOR: Final[str] = (
    "div.manga-list div.manga-item, "
    "div.manga-list a, "
    ".list-manga a, "
    "div.manga-item"
)
_SCAN_SEARCH_LINK_SELECTOR: Final[str] = "a"
_SCAN_SEARCH_COVER_SELECTOR: Final[str] = "img"
_SCAN_SEARCH_TITLE_SELECTOR: Final[str] = (
    ".manga-title, .title, h3, h4, .manga-name"
)

_SCAN_MANGA_TITLE_SELECTOR: Final[str] = (
    "h1.manga-title, h1.title, h1, .manga-title, .title"
)
_SCAN_MANGA_COVER_SELECTOR: Final[str] = (
    ".manga-cover img, .cover img, .thumbnail img, "
    "img.manga-cover, img.cover"
)
_SCAN_MANGA_DESCRIPTION_SELECTOR: Final[str] = (
    ".manga-summary, .summary, .description, "
    ".synopsis, .manga-description"
)
_SCAN_MANGA_AUTHOR_SELECTOR: Final[str] = (
    ".manga-author, .author, a[href*='/author/'], "
    "a[href*='/auteur/']"
)
_SCAN_MANGA_ARTIST_SELECTOR: Final[str] = (
    ".manga-artist, .artist, a[href*='/artist/'], "
    "a[href*='/artiste/']"
)
_SCAN_MANGA_GENRES_SELECTOR: Final[str] = (
    ".manga-genres a, .genres a, "
    "a[href*='/genre/'], a[href*='/tag/']"
)
_SCAN_MANGA_STATUS_SELECTOR: Final[str] = (
    ".manga-status, .status"
)

_SCAN_CHAPTER_SELECTOR: Final[str] = (
    ".chapter-list a, "
...
_SCAN_PAGE_IMG_SELECTOR: Final[str] = (
    "img.page-image, "
    ".reader-content img, "
    ".page-content img, "
    "div.page-image img, "
...
_SCAN_PAGES_VAR_NAMES: Final[tuple[str, ...]] = (
    "pages",
    "page_urls",
    "pageUrls",
    "images",
...
class ScanMangaParser(
    JsRenderedMixin,
    CloudflareMixin,
    FoolSlideMixin,
    BaseParser,
):
    """Parseur Scan-Manga (FoolSlide + Cloudflare).
    ...
    # --- Sélecteurs FoolSlide (standards + ajustements Scan-Manga) ---
    fools_search_item_selector: ClassVar[str] = _SCAN_SEARCH_ITEM_SELECTOR
    fools_search_link_selector: ClassVar[str] = _SCAN_SEARCH_LINK_SELECTOR
    fools_search_cover_selector: ClassVar[str] = _SCAN_SEARCH_COVER_SELECTOR
    fools_search_title_selector: ClassVar[str] = _SCAN_SEARCH_TITLE_SELECTOR

    fools_manga_title_selector: ClassVar[str] = _SCAN_MANGA_TITLE_SELECTOR
    fools_manga_cover_selector: ClassVar[str] = _SCAN_MANGA_COVER_SELECTOR
    fools_manga_description_selector: ClassVar[str] = _SCAN_MANGA_DESCRIPTION_SELECTOR
    fools_manga_author_selector: ClassVar[str] = _SCAN_MANGA_AUTHOR_SELECTOR
    fools_manga_artist_selector: ClassVar[str] = _SCAN_MANGA_ARTIST_SELECTOR
    fools_manga_genres_selector: ClassVar[str] = _SCAN_MANGA_GENRES_SELECTOR
    fools_manga_status_selector: ClassVar[str] = _SCAN_MANGA_STATUS_SELECTOR

    fools_chapter_selector: ClassVar[str] = _SCAN_CHAPTER_SELECTOR
    fools_page_img_selector: ClassVar[str] = _SCAN_PAGE_IMG_SELECTOR
    fools_pages_var_names: ClassVar[tuple[str, ...]] = _SCAN_PAGES_VAR_NAMES
    ...
    scan_ajax_endpoint: ClassVar[str] = "/ajax/lecture.php"
    ...
    def __init__(
        ...
        *,
        ...
        """Initialise le parseur Scan-Manga.
    ...
    def _fools_text(node: Node | None) -> str:
        ...
        if node is None:
            return ""
        return ScanMangaParser._fools_clean(
    ...
    def _fools_chapter_number(text: str) -> float | str:
        ...
        """
        cleaned = ScanMangaParser._fools_clean(text)
    ...
    # ------------------------------------------------------------------
    # Helpers Scan-Manga spécifiques
    # ------------------------------------------------------------------

    @staticmethod
    def _scan_extract_chapter_id(url: str) -> str | None:
        """Extrait l'ID de chapitre depuis une URL Scan-Manga.
    ...
    @staticmethod
    def _scan_extract_pages_from_js(html: str) -> list[str]:
    ...
    async def search(
        ...
        """Recherche des mangas sur Scan-Manga.

        Scan-Manga utilise ``/search/?q={query}`` (paramètre FoolSlide).
        ...
        """
        self.logger.debug(
            "ScanManga search: {query} (page {page})",
            query=query,
            page=page,
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec recherche ScanManga pour {query!r}: {err}",
                query=query,
                err=exc,
            )
            raise ParseError(
    ...
    def _fools_parse_search_html(
        ...
        *,
        ...
        self.logger.info(
            "ScanManga search: {n} résultat(s)", n=len(results)
        )
        return results

    async def get_manga(self, url_or_id: str) -> Manga:
        """Récupère la fiche complète d'un manga Scan-Manga.
        ...
        self.logger.debug("ScanManga get_manga: {url}", url=url)
        ...
        try:
            ...
            self.logger.error(
                "Échec get_manga ScanManga pour {url!r}: {err}",
                url=url,
                err=exc,
            )
            raise ParseError(
        ...
        self.logger.info(
            "ScanManga get_manga OK: {title} ({n} chapitres)",
            title=title,
            n=len(chapters),
        )
        return manga
    ...
    def _fools_extract_series_slug(url: str) -> str:
        """Extrait un identifiant de série depuis une URL Scan-Manga.
    ...
    async def get_chapters(self, manga: Manga) -> list[Chapter]:
        ...
        """
        self.logger.debug(
            "ScanManga get_chapters: {title}", title=manga.title
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_chapters ScanManga pour {title!r}: {err}",
                title=manga.title,
                err=exc,
            )
            raise ParseError(
    ...
    def _fools_parse_chapters_from_html(
        ...
        *,
        ...
        for node in self._fools_all(tree, self.fools_chapter_selector):
            ...
            chapter_id = self._scan_extract_chapter_id(abs_url)
        ...
        self.logger.debug(
            "ScanManga chapitres: {n}", n=len(chapters)
        )
        return chapters

    async def get_pages(self, chapter: Chapter) -> list[Page]:
        """Récupère les URLs des pages d'un chapitre Scan-Manga.

        Scan-Manga (FoolSlide) expose les pages dans une variable JS
        ...
        """
        self.logger.debug(
            "ScanManga get_pages: {url}", url=str(chapter.url)
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_pages ScanManga pour {url!r}: {err}",
                url=chapter_url,
                err=exc,
            )
            raise ParseError(
        ...
        js_urls = self._scan_extract_pages_from_js(html)
        if js_urls:
            for u in js_urls:
        ...
        if not urls:
            chapter_id = self._scan_extract_chapter_id(chapter_url) or chapter.source_id
            if chapter_id and chapter_id.isdigit():
                urls = await self._scan_fetch_pages_via_ajax(chapter_id)
        ...
        self.logger.info(
            "ScanManga get_pages: {n} page(s) extraite(s)", n=len(pages)
        )
        return pages

    async def _scan_fetch_pages_via_ajax(
        self, chapter_id: str
    ) -> list[str]:
        ...
        while page_num <= max_pages:
            endpoint = (
                f"{self.base_url}{self.scan_ajax_endpoint}"
    ...
    async def health_check(self) -> bool:
        """Vérifie que Scan-Manga est accessible.
        ...
        for base in self.fallback_domains:
            try:
                ...
                if rendered.ok:
                    self.logger.info(
                        "Health check ScanManga OK: {base} (status={s})",
                        base=base,
                        s=rendered.status,
                    )
                    return True
                ...
                self.logger.debug(
                    "Health check ScanManga échec sur {base}: {err}",
                    base=base,
                    err=exc,
                )
        self.logger.warning("Health check ScanManga KO (tous domaines)")
        return False
