"""Parseur Epsilon Scan pour NexusDL.

Epsilon Scan (``https://epsilonscan.fr``) est une équipe de scanlation
...
Raijin Scans, Poseidon Scans, Phenix Scans, Ortegascans, Karma Scans et
FanDub Scans.
...
* **Domaines** :
    - ``epsilonscan.fr`` (principal)
    - ``epsilon-scan.fr`` (miroir avec tiret)
    - ``epsilonscan.com`` (miroir secondaire)
...
* **Objet JS ``ts_reader``** : le thème Madara d'Epsilon Scan expose les
...
Example:
    ...
    >>> from nexusdl.parsers.fr.epsilonscan import EpsilonScanParser
    >>>
    >>> parser = EpsilonScanParser(config, session)
...
_BASE_URL: Final[str] = "https://epsilonscan.fr"
_FALLBACK_DOMAINS: Final[tuple[str, ...]] = (
    "https://epsilonscan.fr",
    "https://epsilon-scan.fr",
    "https://epsilonscan.com",
)
_SITE_ID: Final[str] = "epsilonscan"
...
# ---------------------------------------------------------------------------
# Sélecteurs Madara spécifiques à Epsilon Scan
# ---------------------------------------------------------------------------

_EPSILON_SEARCH_ITEM_SELECTOR: Final[str] = (
    ".page-item-detail.manga, .page-item-detail, "
    ".c-tabs-item__content, .bs, .bsx, "
    ".manga-item, .item-summary, .row.c-tabs-item"
)
_EPSILON_CHAPTER_SELECTOR: Final[str] = (
...
_EPSILON_PAGE_IMG_SELECTOR: Final[str] = (
...
_EPSILON_PAGES_VAR_NAMES: Final[tuple[str, ...]] = (
    "ts_reader",
    "pages",
    "page_urls",
    "pageUrls",
...
class EpsilonScanParser(
    JsRenderedMixin,
    CloudflareMixin,
    MadaraMixin,
    BaseParser,
):
    """Parseur Epsilon Scan (Madara + Cloudflare).
    ...
    # --- Sélecteurs Madara (standards + ajustements Epsilon Scan) ---
    madara_search_item_selector: ClassVar[str] = _EPSILON_SEARCH_ITEM_SELECTOR
    ...
    madara_chapter_selector: ClassVar[str] = _EPSILON_CHAPTER_SELECTOR
    madara_page_img_selector: ClassVar[str] = _EPSILON_PAGE_IMG_SELECTOR
    madara_pages_var_names: ClassVar[tuple[str, ...]] = _EPSILON_PAGES_VAR_NAMES
    ...
    def __init__(
        ...
        *,
        ...
        """Initialise le parseur Epsilon Scan.
    ...
    def _madara_text(node: Node | None) -> str:
        ...
        if node is None:
            return ""
        return EpsilonScanParser._madara_clean(
    ...
    def _madara_chapter_number(text: str) -> float | str:
        ...
        """
        cleaned = EpsilonScanParser._madara_clean(text)
    ...
    @staticmethod
    def _epsilon_extract_ts_reader(html: str) -> list[str]:
        """Extrait les URLs d'images depuis l'objet JS ``ts_reader``.

        Le thème Madara d'Epsilon Scan expose les pages dans un objet
        ...
        for pattern in patterns:
            for match in pattern.finditer(html):
                ...
                try:
                    ...
                    continue
                urls = EpsilonScanParser._epsilon_walk_ts_reader(parsed)
                if urls:
                    return urls
    ...
    @staticmethod
    def _epsilon_walk_ts_reader(payload: Any) -> list[str]:
    ...
    async def search(
        ...
        """Recherche des mangas sur Epsilon Scan.

        Epsilon Scan utilise ``/?s={query}&post_type=wp-manga`` (paramètre
        ...
        """
        self.logger.debug(
            "EpsilonScan search: {query} (page {page})",
            query=query,
            page=page,
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec recherche EpsilonScan pour {query!r}: {err}",
                query=query,
                err=exc,
            )
            raise ParseError(
    ...
    def _madara_parse_search_html(
        ...
        *,
        ...
        self.logger.info(
            "EpsilonScan search: {n} résultat(s)", n=len(results)
        )
        return results

    async def get_manga(self, url_or_id: str) -> Manga:
        """Récupère la fiche complète d'un manga Epsilon Scan.
        ...
        self.logger.debug("EpsilonScan get_manga: {url}", url=url)
        ...
        try:
            ...
            self.logger.error(
                "Échec get_manga EpsilonScan pour {url!r}: {err}",
                url=url,
                err=exc,
            )
            raise ParseError(
        ...
        self.logger.info(
            "EpsilonScan get_manga OK: {title} ({n} chapitres)",
            title=title,
            n=len(chapters),
        )
        return manga
    ...
    async def get_chapters(self, manga: Manga) -> list[Chapter]:
        ...
        """
        self.logger.debug(
            "EpsilonScan get_chapters: {title}", title=manga.title
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_chapters EpsilonScan pour {title!r}: {err}",
                title=manga.title,
                err=exc,
            )
            raise ParseError(
    ...
    def _madara_parse_chapters_from_html(
        ...
        *,
        ...
        self.logger.debug(
            "EpsilonScan chapitres: {n}", n=len(chapters)
        )
        return chapters

    async def get_pages(self, chapter: Chapter) -> list[Page]:
        """Récupère les URLs des pages d'un chapitre Epsilon Scan.
        ...
        """
        self.logger.debug(
            "EpsilonScan get_pages: {url}", url=str(chapter.url)
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_pages EpsilonScan pour {url!r}: {err}",
                url=chapter_url,
                err=exc,
            )
            raise ParseError(
        ...
        ts_urls = self._epsilon_extract_ts_reader(html)
        if ts_urls:
            for u in ts_urls:
        ...
        self.logger.info(
            "EpsilonScan get_pages: {n} page(s) extraite(s)", n=len(pages)
        )
        return pages
    ...
    async def health_check(self) -> bool:
        """Vérifie que Epsilon Scan est accessible.
        ...
        for base in self.fallback_domains:
            try:
                ...
                if rendered.ok:
                    self.logger.info(
                        "Health check EpsilonScan OK: {base} (status={s})",
                        base=base,
                        s=rendered.status,
                    )
                    return True
                ...
                self.logger.debug(
                    "Health check EpsilonScan échec sur {base}: {err}",
                    base=base,
                    err=exc,
                )
        self.logger.warning("Health check EpsilonScan KO (tous domaines)")
        return False
