"""Parseur SushiScan.fr pour NexusDL.

SushiScan.fr (``https://sushiscan.fr``) est l'**instance sœur** de
SushiScan.net — un site de scanlation **francophone** spécialisé dans les
mangas, manhwas et manhuas traduits en français. Bien que partageant le
même thème WordPress **Madara** (Mangabooth), les deux instances sont
**gérées par des équipes différentes** et ont des catalogues partiellement
distincts.
...
* **Domaines** :
    - ``sushiscan.fr`` (principal)
    - ``sushiscans.fr`` (miroir avec "s")
    - ``sushi-scan.fr`` (miroir avec tiret)
...
:class:`~nexusdl.parsers.fr.sushiscan_net.SushiScanNetParser`. Les deux
sites ont des catalogues partiellement différents — il ne s'agit **pas**
d'un simple miroir. Utiliser l'un ou l'autre dépend de la disponibilité
...
Example:
    ...
    >>> from nexusdl.parsers.fr.sushiscan_fr import SushiScanFrParser
    >>>
    >>> parser = SushiScanFrParser(config, session)
...
_BASE_URL: Final[str] = "https://sushiscan.fr"
_FALLBACK_DOMAINS: Final[tuple[str, ...]] = (
    "https://sushiscan.fr",
    "https://sushiscans.fr",
    "https://sushi-scan.fr",
)
_SITE_ID: Final[str] = "sushiscan_fr"
...
# ---------------------------------------------------------------------------
# Sélecteurs Madara spécifiques à SushiScan.fr
# ---------------------------------------------------------------------------

_SUSHI_FR_SEARCH_ITEM_SELECTOR: Final[str] = (
    ".page-item-detail.manga, .page-item-detail, "
    ".c-tabs-item__content, .bs, .bsx, "
    ".manga-item, .item-summary, .row.c-tabs-item"
)
_SUSHI_FR_CHAPTER_SELECTOR: Final[str] = (
...
_SUSHI_FR_PAGE_IMG_SELECTOR: Final[str] = (
...
_SUSHI_FR_PAGES_VAR_NAMES: Final[tuple[str, ...]] = (
    "ts_reader",
    "pages",
    "page_urls",
    "pageUrls",
...
class SushiScanFrParser(
    JsRenderedMixin,
    CloudflareMixin,
    MadaraMixin,
    BaseParser,
):
    """Parseur SushiScan.fr (Madara + Cloudflare).
    ...
    # --- Sélecteurs Madara (standards + ajustements SushiScan.fr) ---
    madara_search_item_selector: ClassVar[str] = _SUSHI_FR_SEARCH_ITEM_SELECTOR
    ...
    madara_chapter_selector: ClassVar[str] = _SUSHI_FR_CHAPTER_SELECTOR
    madara_page_img_selector: ClassVar[str] = _SUSHI_FR_PAGE_IMG_SELECTOR
    madara_pages_var_names: ClassVar[tuple[str, ...]] = _SUSHI_FR_PAGES_VAR_NAMES
    ...
    def __init__(
        ...
        *,
        ...
        """Initialise le parseur SushiScan.fr.
    ...
    def _madara_text(node: Node | None) -> str:
        ...
        if node is None:
            return ""
        return SushiScanFrParser._madara_clean(
    ...
    def _madara_chapter_number(text: str) -> float | str:
        ...
        """
        cleaned = SushiScanFrParser._madara_clean(text)
    ...
    @staticmethod
    def _sushi_fr_extract_ts_reader(html: str) -> list[str]:
        """Extrait les URLs d'images depuis l'objet JS ``ts_reader``.

        Le thème Madara de SushiScan.fr expose les pages dans un objet
        ...
        for pattern in patterns:
            for match in pattern.finditer(html):
                ...
                try:
                    ...
                    continue
                urls = SushiScanFrParser._sushi_fr_walk_ts_reader(parsed)
                if urls:
                    return urls
    ...
    @staticmethod
    def _sushi_fr_walk_ts_reader(payload: Any) -> list[str]:
    ...
    async def search(
        ...
        """Recherche des mangas sur SushiScan.fr.

        SushiScan.fr utilise ``/?s={query}&post_type=wp-manga`` (paramètre
        ...
        """
        self.logger.debug(
            "SushiScanFr search: {query} (page {page})",
            query=query,
            page=page,
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec recherche SushiScanFr pour {query!r}: {err}",
                query=query,
                err=exc,
            )
            raise ParseError(
    ...
    def _madara_parse_search_html(
        ...
        *,
        ...
        self.logger.info(
            "SushiScanFr search: {n} résultat(s)", n=len(results)
        )
        return results

    async def get_manga(self, url_or_id: str) -> Manga:
        """Récupère la fiche complète d'un manga SushiScan.fr.
        ...
        self.logger.debug("SushiScanFr get_manga: {url}", url=url)
        ...
        try:
            ...
            self.logger.error(
                "Échec get_manga SushiScanFr pour {url!r}: {err}",
                url=url,
                err=exc,
            )
            raise ParseError(
        ...
        self.logger.info(
            "SushiScanFr get_manga OK: {title} ({n} chapitres)",
            title=title,
            n=len(chapters),
        )
        return manga
    ...
    async def get_chapters(self, manga: Manga) -> list[Chapter]:
        ...
        """
        self.logger.debug(
            "SushiScanFr get_chapters: {title}", title=manga.title
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_chapters SushiScanFr pour {title!r}: {err}",
                title=manga.title,
                err=exc,
            )
            raise ParseError(
    ...
    def _madara_parse_chapters_from_html(
        ...
        *,
        ...
        self.logger.debug(
            "SushiScanFr chapitres: {n}", n=len(chapters)
        )
        return chapters

    async def get_pages(self, chapter: Chapter) -> list[Page]:
        """Récupère les URLs des pages d'un chapitre SushiScan.fr.
        ...
        """
        self.logger.debug(
            "SushiScanFr get_pages: {url}", url=str(chapter.url)
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_pages SushiScanFr pour {url!r}: {err}",
                url=chapter_url,
                err=exc,
            )
            raise ParseError(
        ...
        ts_urls = self._sushi_fr_extract_ts_reader(html)
        if ts_urls:
            for u in ts_urls:
        ...
        self.logger.info(
            "SushiScanFr get_pages: {n} page(s) extraite(s)", n=len(pages)
        )
        return pages
    ...
    async def health_check(self) -> bool:
        """Vérifie que SushiScan.fr est accessible.
        ...
        for base in self.fallback_domains:
            try:
                ...
                if rendered.ok:
                    self.logger.info(
                        "Health check SushiScanFr OK: {base} (status={s})",
                        base=base,
                        s=rendered.status,
                    )
                    return True
                ...
                self.logger.debug(
                    "Health check SushiScanFr échec sur {base}: {err}",
                    base=base,
                    err=exc,
                )
        self.logger.warning("Health check SushiScanFr KO (tous domaines)")
        return False
