"""Parseur FanDub Scans pour NexusDL.

FanDub Scans (``https://fandubscans.com``) est une équipe de scanlation
**francophone** spécialisée dans les mangas, manhwas et manhuas traduits
en français. Le site utilise le thème WordPress **Madara** (Mangabooth),
également utilisé par Toonily, ToonGod, Galaxy Scans, CrunchyScan, Urano
Scans, Taisei Scans, SushiScan.net, SushiScan.fr, Shinra Scans, Rimuscan,
Raijin Scans, Poseidon Scans, Phenix Scans, Ortegascans et Karma Scans.

⚠️ **Note sur le nom** : malgré son nom « FanDub », ce site est un site
de **scanlation** (traduction écrite de mangas/manhwas), pas un site de
fansub (sous-titrage vidéo). Le nom est historique et provient d'un
ancien projet multimédia de l'équipe.
...
* **Domaines** :
    - ``fandubscans.com`` (principal)
    - ``fandub-scans.com`` (miroir avec tiret)
    - ``fandubscans.fr`` (miroir francophone)
...
* **Objet JS ``ts_reader``** : le thème Madara de FanDub Scans expose les
...
Example:
    ...
    >>> from nexusdl.parsers.fr.fandub_scans import FanDubScansParser
    >>>
    >>> parser = FanDubScansParser(config, session)
...
_BASE_URL: Final[str] = "https://fandubscans.com"
_FALLBACK_DOMAINS: Final[tuple[str, ...]] = (
    "https://fandubscans.com",
    "https://fandub-scans.com",
    "https://fandubscans.fr",
)
_SITE_ID: Final[str] = "fandub_scans"
...
# ---------------------------------------------------------------------------
# Sélecteurs Madara spécifiques à FanDub Scans
# ---------------------------------------------------------------------------

_FANDUB_SEARCH_ITEM_SELECTOR: Final[str] = (
    ".page-item-detail.manga, .page-item-detail, "
    ".c-tabs-item__content, .bs, .bsx, "
    ".manga-item, .item-summary, .row.c-tabs-item"
)
_FANDUB_CHAPTER_SELECTOR: Final[str] = (
...
_FANDUB_PAGE_IMG_SELECTOR: Final[str] = (
...
_FANDUB_PAGES_VAR_NAMES: Final[tuple[str, ...]] = (
    "ts_reader",
    "pages",
    "page_urls",
    "pageUrls",
...
class FanDubScansParser(
    JsRenderedMixin,
    CloudflareMixin,
    MadaraMixin,
    BaseParser,
):
    """Parseur FanDub Scans (Madara + Cloudflare).
    ...
    # --- Sélecteurs Madara (standards + ajustements FanDub Scans) ---
    madara_search_item_selector: ClassVar[str] = _FANDUB_SEARCH_ITEM_SELECTOR
    ...
    madara_chapter_selector: ClassVar[str] = _FANDUB_CHAPTER_SELECTOR
    madara_page_img_selector: ClassVar[str] = _FANDUB_PAGE_IMG_SELECTOR
    madara_pages_var_names: ClassVar[tuple[str, ...]] = _FANDUB_PAGES_VAR_NAMES
    ...
    def __init__(
        ...
        *,
        ...
        """Initialise le parseur FanDub Scans.
    ...
    def _madara_text(node: Node | None) -> str:
        ...
        if node is None:
            return ""
        return FanDubScansParser._madara_clean(
    ...
    def _madara_chapter_number(text: str) -> float | str:
        ...
        """
        cleaned = FanDubScansParser._madara_clean(text)
    ...
    @staticmethod
    def _fandub_extract_ts_reader(html: str) -> list[str]:
        """Extrait les URLs d'images depuis l'objet JS ``ts_reader``.

        Le thème Madara de FanDub Scans expose les pages dans un objet
        ...
        for pattern in patterns:
            for match in pattern.finditer(html):
                ...
                try:
                    ...
                    continue
                urls = FanDubScansParser._fandub_walk_ts_reader(parsed)
                if urls:
                    return urls
    ...
    @staticmethod
    def _fandub_walk_ts_reader(payload: Any) -> list[str]:
    ...
    async def search(
        ...
        """Recherche des mangas sur FanDub Scans.

        FanDub Scans utilise ``/?s={query}&post_type=wp-manga`` (paramètre
        ...
        """
        self.logger.debug(
            "FanDubScans search: {query} (page {page})",
            query=query,
            page=page,
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec recherche FanDubScans pour {query!r}: {err}",
                query=query,
                err=exc,
            )
            raise ParseError(
    ...
    def _madara_parse_search_html(
        ...
        *,
        ...
        self.logger.info(
            "FanDubScans search: {n} résultat(s)", n=len(results)
        )
        return results

    async def get_manga(self, url_or_id: str) -> Manga:
        """Récupère la fiche complète d'un manga FanDub Scans.
        ...
        self.logger.debug("FanDubScans get_manga: {url}", url=url)
        ...
        try:
            ...
            self.logger.error(
                "Échec get_manga FanDubScans pour {url!r}: {err}",
                url=url,
                err=exc,
            )
            raise ParseError(
        ...
        self.logger.info(
            "FanDubScans get_manga OK: {title} ({n} chapitres)",
            title=title,
            n=len(chapters),
        )
        return manga
    ...
    async def get_chapters(self, manga: Manga) -> list[Chapter]:
        ...
        """
        self.logger.debug(
            "FanDubScans get_chapters: {title}", title=manga.title
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_chapters FanDubScans pour {title!r}: {err}",
                title=manga.title,
                err=exc,
            )
            raise ParseError(
    ...
    def _madara_parse_chapters_from_html(
        ...
        *,
        ...
        self.logger.debug(
            "FanDubScans chapitres: {n}", n=len(chapters)
        )
        return chapters

    async def get_pages(self, chapter: Chapter) -> list[Page]:
        """Récupère les URLs des pages d'un chapitre FanDub Scans.
        ...
        """
        self.logger.debug(
            "FanDubScans get_pages: {url}", url=str(chapter.url)
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_pages FanDubScans pour {url!r}: {err}",
                url=chapter_url,
                err=exc,
            )
            raise ParseError(
        ...
        ts_urls = self._fandub_extract_ts_reader(html)
        if ts_urls:
            for u in ts_urls:
        ...
        self.logger.info(
            "FanDubScans get_pages: {n} page(s) extraite(s)", n=len(pages)
        )
        return pages
    ...
    async def health_check(self) -> bool:
        """Vérifie que FanDub Scans est accessible.
        ...
        for base in self.fallback_domains:
            try:
                ...
                if rendered.ok:
                    self.logger.info(
                        "Health check FanDubScans OK: {base} (status={s})",
                        base=base,
                        s=rendered.status,
                    )
                    return True
                ...
                self.logger.debug(
                    "Health check FanDubScans échec sur {base}: {err}",
                    base=base,
                    err=exc,
                )
        self.logger.warning("Health check FanDubScans KO (tous domaines)")
        return False
