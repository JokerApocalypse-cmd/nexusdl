"""Parseur Shinra Scans pour NexusDL.

Shinra Scans (``https://shinrascans.com``) est une équipe de scanlation
**francophone** spécialisée dans les mangas, manhwas et manhuas. Le site
utilise le thème WordPress **Madara** (Mangabooth), également utilisé par
Toonily, ToonGod, Galaxy Scans, CrunchyScan, Urano Scans, Taisei Scans,
SushiScan.net et SushiScan.fr.
...
* **Domaines** :
    - ``shinrascans.com`` (principal)
    - ``shinra-scans.com`` (miroir avec tiret)
    - ``shinrascans.fr`` (miroir francophone)
...
* **Objet JS ``ts_reader``** : le thème Madara de Shinra Scans expose les
...
Example:
    ...
    >>> from nexusdl.parsers.fr.shinra_scans import ShinraScansParser
    >>>
    >>> parser = ShinraScansParser(config, session)
...
_BASE_URL: Final[str] = "https://shinrascans.com"
_FALLBACK_DOMAINS: Final[tuple[str, ...]] = (
    "https://shinrascans.com",
    "https://shinra-scans.com",
    "https://shinrascans.fr",
)
_SITE_ID: Final[str] = "shinra_scans"
...
# ---------------------------------------------------------------------------
# Sélecteurs Madara spécifiques à Shinra Scans
# ---------------------------------------------------------------------------

_SHINRA_SEARCH_ITEM_SELECTOR: Final[str] = (
    ".page-item-detail.manga, .page-item-detail, "
    ".c-tabs-item__content, .bs, .bsx, "
    ".manga-item, .item-summary, .row.c-tabs-item"
)
_SHINRA_CHAPTER_SELECTOR: Final[str] = (
...
_SHINRA_PAGE_IMG_SELECTOR: Final[str] = (
...
_SHINRA_PAGES_VAR_NAMES: Final[tuple[str, ...]] = (
    "ts_reader",
    "pages",
    "page_urls",
    "pageUrls",
...
class ShinraScansParser(
    JsRenderedMixin,
    CloudflareMixin,
    MadaraMixin,
    BaseParser,
):
    """Parseur Shinra Scans (Madara + Cloudflare).
    ...
    # --- Sélecteurs Madara (standards + ajustements Shinra Scans) ---
    madara_search_item_selector: ClassVar[str] = _SHINRA_SEARCH_ITEM_SELECTOR
    ...
    madara_chapter_selector: ClassVar[str] = _SHINRA_CHAPTER_SELECTOR
    madara_page_img_selector: ClassVar[str] = _SHINRA_PAGE_IMG_SELECTOR
    madara_pages_var_names: ClassVar[tuple[str, ...]] = _SHINRA_PAGES_VAR_NAMES
    ...
    def __init__(
        ...
        *,
        ...
        """Initialise le parseur Shinra Scans.
    ...
    def _madara_text(node: Node | None) -> str:
        ...
        if node is None:
            return ""
        return ShinraScansParser._madara_clean(
    ...
    def _madara_chapter_number(text: str) -> float | str:
        ...
        """
        cleaned = ShinraScansParser._madara_clean(text)
    ...
    @staticmethod
    def _shinra_extract_ts_reader(html: str) -> list[str]:
        """Extrait les URLs d'images depuis l'objet JS ``ts_reader``.

        Le thème Madara de Shinra Scans expose les pages dans un objet
        ...
        for pattern in patterns:
            for match in pattern.finditer(html):
                ...
                try:
                    ...
                    continue
                urls = ShinraScansParser._shinra_walk_ts_reader(parsed)
                if urls:
                    return urls
    ...
    @staticmethod
    def _shinra_walk_ts_reader(payload: Any) -> list[str]:
    ...
    async def search(
        ...
        """Recherche des mangas sur Shinra Scans.

        Shinra Scans utilise ``/?s={query}&post_type=wp-manga`` (paramètre
        ...
        """
        self.logger.debug(
            "ShinraScans search: {query} (page {page})",
            query=query,
            page=page,
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec recherche ShinraScans pour {query!r}: {err}",
                query=query,
                err=exc,
            )
            raise ParseError(
    ...
    def _madara_parse_search_html(
        ...
        *,
        ...
        self.logger.info(
            "ShinraScans search: {n} résultat(s)", n=len(results)
        )
        return results

    async def get_manga(self, url_or_id: str) -> Manga:
        """Récupère la fiche complète d'un manga Shinra Scans.
        ...
        self.logger.debug("ShinraScans get_manga: {url}", url=url)
        ...
        try:
            ...
            self.logger.error(
                "Échec get_manga ShinraScans pour {url!r}: {err}",
                url=url,
                err=exc,
            )
            raise ParseError(
        ...
        self.logger.info(
            "ShinraScans get_manga OK: {title} ({n} chapitres)",
            title=title,
            n=len(chapters),
        )
        return manga
    ...
    async def get_chapters(self, manga: Manga) -> list[Chapter]:
        ...
        """
        self.logger.debug(
            "ShinraScans get_chapters: {title}", title=manga.title
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_chapters ShinraScans pour {title!r}: {err}",
                title=manga.title,
                err=exc,
            )
            raise ParseError(
    ...
    def _madara_parse_chapters_from_html(
        ...
        *,
        ...
        self.logger.debug(
            "ShinraScans chapitres: {n}", n=len(chapters)
        )
        return chapters

    async def get_pages(self, chapter: Chapter) -> list[Page]:
        """Récupère les URLs des pages d'un chapitre Shinra Scans.
        ...
        """
        self.logger.debug(
            "ShinraScans get_pages: {url}", url=str(chapter.url)
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_pages ShinraScans pour {url!r}: {err}",
                url=chapter_url,
                err=exc,
            )
            raise ParseError(
        ...
        ts_urls = self._shinra_extract_ts_reader(html)
        if ts_urls:
            for u in ts_urls:
        ...
        self.logger.info(
            "ShinraScans get_pages: {n} page(s) extraite(s)", n=len(pages)
        )
        return pages
    ...
    async def health_check(self) -> bool:
        """Vérifie que Shinra Scans est accessible.
        ...
        for base in self.fallback_domains:
            try:
                ...
                if rendered.ok:
                    self.logger.info(
                        "Health check ShinraScans OK: {base} (status={s})",
                        base=base,
                        s=rendered.status,
                    )
                    return True
                ...
                self.logger.debug(
                    "Health check ShinraScans échec sur {base}: {err}",
                    base=base,
                    err=exc,
                )
        self.logger.warning("Health check ShinraScans KO (tous domaines)")
        return False
