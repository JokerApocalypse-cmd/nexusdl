"""Parseur Mangas Origines pour NexusDL.

Mangas Origines (``https://mangas-origines.com``) est l'un des plus anciens
et des plus importants sites de scanlation **francophone**, en activité
depuis 2010. Il utilise le CMS **FoolSlide** — un moteur PHP historique très
répandu dans la communauté scan FR avant l'avènement de WordPress/Madara.

⚠️ **Note importante** : ce parser cible l'instance **historique**
``mangas-origines.com``. Il est distinct de
:class:`~nexusdl.parsers.fr.mangas_origines_fr.MangasOriginesFrParser` qui
cible l'instance moderne ``mangas-origines.fr``. Bien que les deux
instances soient gérées par la **même équipe**, leurs catalogues sont
**partiellement distincts** :

* ``mangas-origines.com`` — instance historique (ce parser), catalogue
  plus ancien, certains titres legacy uniquement disponibles ici.
* ``mangas-origines.fr`` — instance moderne (parser séparé), catalogue
  plus récent, plus active.
...
* **Domaines** :
    - ``mangas-origines.com`` (principal, instance historique)
    - ``mangasorigines.com`` (miroir sans tiret)
    - ``mangas-origines.fr`` (instance moderne, fallback)
...
Example:
    ...
    >>> from nexusdl.parsers.fr.mangas_origines import MangasOriginesParser
    >>>
    >>> parser = MangasOriginesParser(config, session)
...
_BASE_URL: Final[str] = "https://mangas-origines.com"
_FALLBACK_DOMAINS: Final[tuple[str, ...]] = (
    "https://mangas-origines.com",
    "https://mangasorigines.com",
    "https://mangas-origines.fr",
)
_SITE_ID: Final[str] = "mangas_origines"
...
# ---------------------------------------------------------------------------
# Sélecteurs FoolSlide spécifiques à Mangas Origines (instance historique)
# ---------------------------------------------------------------------------

_MO_SEARCH_ITEM_SELECTOR: Final[str] = (
    # Sélecteurs FoolSlide historiques (avant refonte du thème).
    "div.group, "
    "div.manga-list div.group, "
    "div.list-item, "
    "div.manga-item, "
    "div.manga-list a, "
    ".list-manga a"
...
_MO_MANGA_GENRES_SELECTOR: Final[str] = (
    ...
    ".manga-info a[href*='/genre/']"
)
_MO_MANGA_STATUS_SELECTOR: Final[str] = (
    ".manga-status, .status, .info .status"
)
...
class MangasOriginesParser(
    JsRenderedMixin,
    CloudflareMixin,
    FoolSlideMixin,
    BaseParser,
):
    """Parseur Mangas Origines (FoolSlide + Cloudflare).
    ...
    def __init__(
        ...
        *,
        ...
        """Initialise le parseur Mangas Origines.
    ...
    def _fools_text(node: Node | None) -> str:
        ...
        if node is None:
            return ""
        return MangasOriginesParser._fools_clean(
    ...
    def _fools_chapter_number(text: str) -> float | str:
        ...
        """
        cleaned = MangasOriginesParser._fools_clean(text)
    ...
    async def search(
        ...
        """Recherche des mangas sur Mangas Origines.
        ...
        """
        self.logger.debug(
            "MangasOrigines search: {query} (page {page})",
            query=query,
            page=page,
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec recherche MangasOrigines pour {query!r}: {err}",
                query=query,
                err=exc,
            )
            raise ParseError(
    ...
    def _fools_parse_search_html(
        ...
        *,
        ...
        self.logger.info(
            "MangasOrigines search: {n} résultat(s)", n=len(results)
        )
        return results

    async def get_manga(self, url_or_id: str) -> Manga:
        """Récupère la fiche complète d'un manga Mangas Origines.
        ...
        self.logger.debug("MangasOrigines get_manga: {url}", url=url)
        ...
        try:
            ...
            self.logger.error(
                "Échec get_manga MangasOrigines pour {url!r}: {err}",
                url=url,
                err=exc,
            )
            raise ParseError(
        ...
        self.logger.info(
            "MangasOrigines get_manga OK: {title} ({n} chapitres)",
            title=title,
            n=len(chapters),
        )
        return manga
    ...
    async def get_chapters(self, manga: Manga) -> list[Chapter]:
        ...
        """
        self.logger.debug(
            "MangasOrigines get_chapters: {title}", title=manga.title
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_chapters MangasOrigines pour {title!r}: {err}",
                title=manga.title,
                err=exc,
            )
            raise ParseError(
    ...
    def _fools_parse_chapters_from_html(
        ...
        *,
        ...
        self.logger.debug(
            "MangasOrigines chapitres: {n}", n=len(chapters)
        )
        return chapters

    async def get_pages(self, chapter: Chapter) -> list[Page]:
        """Récupère les URLs des pages d'un chapitre Mangas Origines.
        ...
        """
        self.logger.debug(
            "MangasOrigines get_pages: {url}", url=str(chapter.url)
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_pages MangasOrigines pour {url!r}: {err}",
                url=chapter_url,
                err=exc,
            )
            raise ParseError(
        ...
        self.logger.info(
            "MangasOrigines get_pages: {n} page(s) extraite(s)",
            n=len(pages),
        )
        return pages
    ...
    async def health_check(self) -> bool:
        """Vérifie que Mangas Origines est accessible.
        ...
        for base in self.fallback_domains:
            try:
                ...
                if rendered.ok:
                    self.logger.info(
                        "Health check MangasOrigines OK: {base} (status={s})",
                        base=base,
                        s=rendered.status,
                    )
                    return True
                ...
                self.logger.debug(
                    "Health check MangasOrigines échec sur {base}: {err}",
                    base=base,
                    err=exc,
                )
        self.logger.warning(
            "Health check MangasOrigines KO (tous domaines)"
        )
        return False
