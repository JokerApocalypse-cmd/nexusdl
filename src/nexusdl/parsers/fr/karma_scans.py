"""Parseur Karma Scans pour NexusDL.

Karma Scans (``https://karmascans.com``) est une équipe de scanlation
...
Raijin Scans, Poseidon Scans, Phenix Scans et Ortegascans.
...
* **Domaines** :
    - ``karmascans.com`` (principal)
    - ``karma-scans.com`` (miroir avec tiret)
    - ``karmascans.fr`` (miroir francophone)
...
* **Objet JS ``ts_reader``** : le thème Madara de Karma Scans expose les
...
Example:
    ...
    >>> from nexusdl.parsers.fr.karma_scans import KarmaScansParser
    >>>
    >>> parser = KarmaScansParser(config, session)
...
_BASE_URL: Final[str] = "https://karmascans.com"
_FALLBACK_DOMAINS: Final[tuple[str, ...]] = (
    "https://karmascans.com",
    "https://karma-scans.com",
    "https://karmascans.fr",
)
_SITE_ID: Final[str] = "karma_scans"
...
# ---------------------------------------------------------------------------
# Sélecteurs Madara spécifiques à Karma Scans
# ---------------------------------------------------------------------------

_KARMA_SEARCH_ITEM_SELECTOR: Final[str] = (
    ".page-item-detail.manga, .page-item-detail, "
    ".c-tabs-item__content, .bs, .bsx, "
    ".manga-item, .item-summary, .row.c-tabs-item"
)
_KARMA_CHAPTER_SELECTOR: Final[str] = (
...
_KARMA_PAGE_IMG_SELECTOR: Final[str] = (
...
_KARMA_PAGES_VAR_NAMES: Final[tuple[str, ...]] = (
    "ts_reader",
    "pages",
    "page_urls",
    "pageUrls",
...
class KarmaScansParser(
    JsRenderedMixin,
    CloudflareMixin,
    MadaraMixin,
    BaseParser,
):
    """Parseur Karma Scans (Madara + Cloudflare).
    ...
    # --- Sélecteurs Madara (standards + ajustements Karma Scans) ---
    madara_search_item_selector: ClassVar[str] = _KARMA_SEARCH_ITEM_SELECTOR
    ...
    madara_chapter_selector: ClassVar[str] = _KARMA_CHAPTER_SELECTOR
    madara_page_img_selector: ClassVar[str] = _KARMA_PAGE_IMG_SELECTOR
    madara_pages_var_names: ClassVar[tuple[str, ...]] = _KARMA_PAGES_VAR_NAMES
    ...
    def __init__(
        ...
        *,
        ...
        """Initialise le parseur Karma Scans.
    ...
    def _madara_text(node: Node | None) -> str:
        ...
        if node is None:
            return ""
        return KarmaScansParser._madara_clean(
    ...
    def _madara_chapter_number(text: str) -> float | str:
        ...
        """
        cleaned = KarmaScansParser._madara_clean(text)
    ...
    @staticmethod
    def _karma_extract_ts_reader(html: str) -> list[str]:
        """Extrait les URLs d'images depuis l'objet JS ``ts_reader``.

        Le thème Madara de Karma Scans expose les pages dans un objet
        ...
        for pattern in patterns:
            for match in pattern.finditer(html):
                ...
                try:
                    ...
                    continue
                urls = KarmaScansParser._karma_walk_ts_reader(parsed)
                if urls:
                    return urls
    ...
    @staticmethod
    def _karma_walk_ts_reader(payload: Any) -> list[str]:
    ...
    async def search(
        ...
        """Recherche des mangas sur Karma Scans.

        Karma Scans utilise ``/?s={query}&post_type=wp-manga`` (paramètre
        ...
        """
        self.logger.debug(
            "KarmaScans search: {query} (page {page})",
            query=query,
            page=page,
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec recherche KarmaScans pour {query!r}: {err}",
                query=query,
                err=exc,
            )
            raise ParseError(
    ...
    def _madara_parse_search_html(
        ...
        *,
        ...
        self.logger.info(
            "KarmaScans search: {n} résultat(s)", n=len(results)
        )
        return results

    async def get_manga(self, url_or_id: str) -> Manga:
        """Récupère la fiche complète d'un manga Karma Scans.
        ...
        self.logger.debug("KarmaScans get_manga: {url}", url=url)
        ...
        try:
            ...
            self.logger.error(
                "Échec get_manga KarmaScans pour {url!r}: {err}",
                url=url,
                err=exc,
            )
            raise ParseError(
        ...
        self.logger.info(
            "KarmaScans get_manga OK: {title} ({n} chapitres)",
            title=title,
            n=len(chapters),
        )
        return manga
    ...
    async def get_chapters(self, manga: Manga) -> list[Chapter]:
        ...
        """
        self.logger.debug(
            "KarmaScans get_chapters: {title}", title=manga.title
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_chapters KarmaScans pour {title!r}: {err}",
                title=manga.title,
                err=exc,
            )
            raise ParseError(
    ...
    def _madara_parse_chapters_from_html(
        ...
        *,
        ...
        self.logger.debug(
            "KarmaScans chapitres: {n}", n=len(chapters)
        )
        return chapters

    async def get_pages(self, chapter: Chapter) -> list[Page]:
        """Récupère les URLs des pages d'un chapitre Karma Scans.
        ...
        """
        self.logger.debug(
            "KarmaScans get_pages: {url}", url=str(chapter.url)
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_pages KarmaScans pour {url!r}: {err}",
                url=chapter_url,
                err=exc,
            )
            raise ParseError(
        ...
        ts_urls = self._karma_extract_ts_reader(html)
        if ts_urls:
            for u in ts_urls:
        ...
        self.logger.info(
            "KarmaScans get_pages: {n} page(s) extraite(s)", n=len(pages)
        )
        return pages
    ...
    async def health_check(self) -> bool:
        """Vérifie que Karma Scans est accessible.
        ...
        for base in self.fallback_domains:
            try:
                ...
                if rendered.ok:
                    self.logger.info(
                        "Health check KarmaScans OK: {base} (status={s})",
                        base=base,
                        s=rendered.status,
                    )
                    return True
                ...
                self.logger.debug(
                    "Health check KarmaScans échec sur {base}: {err}",
                    base=base,
                    err=exc,
                )
        self.logger.warning("Health check KarmaScans KO (tous domaines)")
        return False
