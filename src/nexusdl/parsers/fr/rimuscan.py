"""Parseur Rimuscan pour NexusDL.

Rimuscan (``https://rimuscan.com``) est une équipe de scanlation
...
Scans, Taisei Scans, SushiScan.net, SushiScan.fr et Shinra Scans.
...
* **Domaines** :
    - ``rimuscan.com`` (principal)
    - ``rimu-scan.com`` (miroir avec tiret)
    - ``rimuscan.fr`` (miroir francophone)
...
* **Objet JS ``ts_reader``** : le thème Madara de Rimuscan expose les
...
Example:
    ...
    >>> from nexusdl.parsers.fr.rimuscan import RimuscanParser
    >>>
    >>> parser = RimuscanParser(config, session)
...
_BASE_URL: Final[str] = "https://rimuscan.com"
_FALLBACK_DOMAINS: Final[tuple[str, ...]] = (
    "https://rimuscan.com",
    "https://rimu-scan.com",
    "https://rimuscan.fr",
)
_SITE_ID: Final[str] = "rimuscan"
...
# ---------------------------------------------------------------------------
# Sélecteurs Madara spécifiques à Rimuscan
# ---------------------------------------------------------------------------

_RIMU_SEARCH_ITEM_SELECTOR: Final[str] = (
    ".page-item-detail.manga, .page-item-detail, "
    ".c-tabs-item__content, .bs, .bsx, "
    ".manga-item, .item-summary, .row.c-tabs-item"
)
_RIMU_CHAPTER_SELECTOR: Final[str] = (
...
_RIMU_PAGE_IMG_SELECTOR: Final[str] = (
...
_RIMU_PAGES_VAR_NAMES: Final[tuple[str, ...]] = (
    "ts_reader",
    "pages",
    "page_urls",
    "pageUrls",
...
class RimuscanParser(
    JsRenderedMixin,
    CloudflareMixin,
    MadaraMixin,
    BaseParser,
):
    """Parseur Rimuscan (Madara + Cloudflare).
    ...
    # --- Sélecteurs Madara (standards + ajustements Rimuscan) ---
    madara_search_item_selector: ClassVar[str] = _RIMU_SEARCH_ITEM_SELECTOR
    ...
    madara_chapter_selector: ClassVar[str] = _RIMU_CHAPTER_SELECTOR
    madara_page_img_selector: ClassVar[str] = _RIMU_PAGE_IMG_SELECTOR
    madara_pages_var_names: ClassVar[tuple[str, ...]] = _RIMU_PAGES_VAR_NAMES
    ...
    def __init__(
        ...
        *,
        ...
        """Initialise le parseur Rimuscan.
    ...
    def _madara_text(node: Node | None) -> str:
        ...
        if node is None:
            return ""
        return RimuscanParser._madara_clean(
    ...
    def _madara_chapter_number(text: str) -> float | str:
        ...
        """
        cleaned = RimuscanParser._madara_clean(text)
    ...
    @staticmethod
    def _rimu_extract_ts_reader(html: str) -> list[str]:
        """Extrait les URLs d'images depuis l'objet JS ``ts_reader``.

        Le thème Madara de Rimuscan expose les pages dans un objet
        ...
        for pattern in patterns:
            for match in pattern.finditer(html):
                ...
                try:
                    ...
                    continue
                urls = RimuscanParser._rimu_walk_ts_reader(parsed)
                if urls:
                    return urls
    ...
    @staticmethod
    def _rimu_walk_ts_reader(payload: Any) -> list[str]:
    ...
    async def search(
        ...
        """Recherche des mangas sur Rimuscan.

        Rimuscan utilise ``/?s={query}&post_type=wp-manga`` (paramètre
        ...
        """
        self.logger.debug(
            "Rimuscan search: {query} (page {page})",
            query=query,
            page=page,
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec recherche Rimuscan pour {query!r}: {err}",
                query=query,
                err=exc,
            )
            raise ParseError(
    ...
    def _madara_parse_search_html(
        ...
        *,
        ...
        self.logger.info(
            "Rimuscan search: {n} résultat(s)", n=len(results)
        )
        return results

    async def get_manga(self, url_or_id: str) -> Manga:
        """Récupère la fiche complète d'un manga Rimuscan.
        ...
        self.logger.debug("Rimuscan get_manga: {url}", url=url)
        ...
        try:
            ...
            self.logger.error(
                "Échec get_manga Rimuscan pour {url!r}: {err}",
                url=url,
                err=exc,
            )
            raise ParseError(
        ...
        self.logger.info(
            "Rimuscan get_manga OK: {title} ({n} chapitres)",
            title=title,
            n=len(chapters),
        )
        return manga
    ...
    async def get_chapters(self, manga: Manga) -> list[Chapter]:
        ...
        """
        self.logger.debug(
            "Rimuscan get_chapters: {title}", title=manga.title
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_chapters Rimuscan pour {title!r}: {err}",
                title=manga.title,
                err=exc,
            )
            raise ParseError(
    ...
    def _madara_parse_chapters_from_html(
        ...
        *,
        ...
        self.logger.debug(
            "Rimuscan chapitres: {n}", n=len(chapters)
        )
        return chapters

    async def get_pages(self, chapter: Chapter) -> list[Page]:
        """Récupère les URLs des pages d'un chapitre Rimuscan.
        ...
        """
        self.logger.debug(
            "Rimuscan get_pages: {url}", url=str(chapter.url)
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_pages Rimuscan pour {url!r}: {err}",
                url=chapter_url,
                err=exc,
            )
            raise ParseError(
        ...
        ts_urls = self._rimu_extract_ts_reader(html)
        if ts_urls:
            for u in ts_urls:
        ...
        self.logger.info(
            "Rimuscan get_pages: {n} page(s) extraite(s)", n=len(pages)
        )
        return pages
    ...
    async def health_check(self) -> bool:
        """Vérifie que Rimuscan est accessible.
        ...
        for base in self.fallback_domains:
            try:
                ...
                if rendered.ok:
                    self.logger.info(
                        "Health check Rimuscan OK: {base} (status={s})",
                        base=base,
                        s=rendered.status,
                    )
                    return True
                ...
                self.logger.debug(
                    "Health check Rimuscan échec sur {base}: {err}",
                    base=base,
                    err=exc,
                )
        self.logger.warning("Health check Rimuscan KO (tous domaines)")
        return False
