"""Parseur Phenix Scans pour NexusDL.

Phenix Scans (``https://phenixscans.fr``) est une équipe de scanlation
...
Raijin Scans et Poseidon Scans.
...
* **Domaines** :
    - ``phenixscans.fr`` (principal)
    - ``phenix-scans.fr`` (miroir avec tiret)
    - ``phenixscans.com`` (miroir secondaire)
...
* **Objet JS ``ts_reader``** : le thème Madara de Phenix Scans expose les
...
Example:
    ...
    >>> from nexusdl.parsers.fr.phenix_scans import PhenixScansParser
    >>>
    >>> parser = PhenixScansParser(config, session)
...
_BASE_URL: Final[str] = "https://phenixscans.fr"
_FALLBACK_DOMAINS: Final[tuple[str, ...]] = (
    "https://phenixscans.fr",
    "https://phenix-scans.fr",
    "https://phenixscans.com",
)
_SITE_ID: Final[str] = "phenix_scans"
...
# ---------------------------------------------------------------------------
# Sélecteurs Madara spécifiques à Phenix Scans
# ---------------------------------------------------------------------------

_PHENIX_SEARCH_ITEM_SELECTOR: Final[str] = (
    ".page-item-detail.manga, .page-item-detail, "
    ".c-tabs-item__content, .bs, .bsx, "
    ".manga-item, .item-summary, .row.c-tabs-item"
)
_PHENIX_CHAPTER_SELECTOR: Final[str] = (
...
_PHENIX_PAGE_IMG_SELECTOR: Final[str] = (
...
_PHENIX_PAGES_VAR_NAMES: Final[tuple[str, ...]] = (
    "ts_reader",
    "pages",
    "page_urls",
    "pageUrls",
...
class PhenixScansParser(
    JsRenderedMixin,
    CloudflareMixin,
    MadaraMixin,
    BaseParser,
):
    """Parseur Phenix Scans (Madara + Cloudflare).
    ...
    # --- Sélecteurs Madara (standards + ajustements Phenix Scans) ---
    madara_search_item_selector: ClassVar[str] = _PHENIX_SEARCH_ITEM_SELECTOR
    ...
    madara_chapter_selector: ClassVar[str] = _PHENIX_CHAPTER_SELECTOR
    madara_page_img_selector: ClassVar[str] = _PHENIX_PAGE_IMG_SELECTOR
    madara_pages_var_names: ClassVar[tuple[str, ...]] = _PHENIX_PAGES_VAR_NAMES
    ...
    def __init__(
        ...
        *,
        ...
        """Initialise le parseur Phenix Scans.
    ...
    def _madara_text(node: Node | None) -> str:
        ...
        if node is None:
            return ""
        return PhenixScansParser._madara_clean(
    ...
    def _madara_chapter_number(text: str) -> float | str:
        ...
        """
        cleaned = PhenixScansParser._madara_clean(text)
    ...
    @staticmethod
    def _phenix_extract_ts_reader(html: str) -> list[str]:
        """Extrait les URLs d'images depuis l'objet JS ``ts_reader``.

        Le thème Madara de Phenix Scans expose les pages dans un objet
        ...
        for pattern in patterns:
            for match in pattern.finditer(html):
                ...
                try:
                    ...
                    continue
                urls = PhenixScansParser._phenix_walk_ts_reader(parsed)
                if urls:
                    return urls
    ...
    @staticmethod
    def _phenix_walk_ts_reader(payload: Any) -> list[str]:
    ...
    async def search(
        ...
        """Recherche des mangas sur Phenix Scans.

        Phenix Scans utilise ``/?s={query}&post_type=wp-manga`` (paramètre
        ...
        """
        self.logger.debug(
            "PhenixScans search: {query} (page {page})",
            query=query,
            page=page,
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec recherche PhenixScans pour {query!r}: {err}",
                query=query,
                err=exc,
            )
            raise ParseError(
    ...
    def _madara_parse_search_html(
        ...
        *,
        ...
        self.logger.info(
            "PhenixScans search: {n} résultat(s)", n=len(results)
        )
        return results

    async def get_manga(self, url_or_id: str) -> Manga:
        """Récupère la fiche complète d'un manga Phenix Scans.
        ...
        self.logger.debug("PhenixScans get_manga: {url}", url=url)
        ...
        try:
            ...
            self.logger.error(
                "Échec get_manga PhenixScans pour {url!r}: {err}",
                url=url,
                err=exc,
            )
            raise ParseError(
        ...
        self.logger.info(
            "PhenixScans get_manga OK: {title} ({n} chapitres)",
            title=title,
            n=len(chapters),
        )
        return manga
    ...
    async def get_chapters(self, manga: Manga) -> list[Chapter]:
        ...
        """
        self.logger.debug(
            "PhenixScans get_chapters: {title}", title=manga.title
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_chapters PhenixScans pour {title!r}: {err}",
                title=manga.title,
                err=exc,
            )
            raise ParseError(
    ...
    def _madara_parse_chapters_from_html(
        ...
        *,
        ...
        self.logger.debug(
            "PhenixScans chapitres: {n}", n=len(chapters)
        )
        return chapters

    async def get_pages(self, chapter: Chapter) -> list[Page]:
        """Récupère les URLs des pages d'un chapitre Phenix Scans.
        ...
        """
        self.logger.debug(
            "PhenixScans get_pages: {url}", url=str(chapter.url)
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_pages PhenixScans pour {url!r}: {err}",
                url=chapter_url,
                err=exc,
            )
            raise ParseError(
        ...
        ts_urls = self._phenix_extract_ts_reader(html)
        if ts_urls:
            for u in ts_urls:
        ...
        self.logger.info(
            "PhenixScans get_pages: {n} page(s) extraite(s)", n=len(pages)
        )
        return pages
    ...
    async def health_check(self) -> bool:
        """Vérifie que Phenix Scans est accessible.
        ...
        for base in self.fallback_domains:
            try:
                ...
                if rendered.ok:
                    self.logger.info(
                        "Health check PhenixScans OK: {base} (status={s})",
                        base=base,
                        s=rendered.status,
                    )
                    return True
                ...
                self.logger.debug(
                    "Health check PhenixScans échec sur {base}: {err}",
                    base=base,
                    err=exc,
                )
        self.logger.warning("Health check PhenixScans KO (tous domaines)")
        return False
