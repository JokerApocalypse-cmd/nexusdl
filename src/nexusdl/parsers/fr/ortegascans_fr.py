"""Parseur Ortegascans FR pour NexusDL.

Ortegascans FR (``https://ortegascans.fr``) est l'instance **francophone**
d'Ortegascans — un site de scanlation multi-langue (ES/EN/FR). Il utilise
le thème WordPress **Madara** (Mangabooth), également utilisé par Toonily,
ToonGod, Galaxy Scans, CrunchyScan, Urano Scans, Taisei Scans,
SushiScan.net, SushiScan.fr, Shinra Scans, Rimuscan, Raijin Scans,
Poseidon Scans et Phenix Scans.

⚠️ **Note importante** : Ortegascans existe en plusieurs variantes
linguistiques :

* ``ortegascans.com`` — instance principale (espagnol).
* ``ortegascans.fr`` — instance française (ce parser).
...
* **Domaines** :
    - ``ortegascans.fr`` (principal, instance FR)
    - ``ortega-scans.fr`` (miroir avec tiret)
    - ``ortegascans.com`` (instance ES, fallback)
...
Example:
    ...
    >>> from nexusdl.parsers.fr.ortegascans_fr import OrtegascansFrParser
    >>>
    >>> parser = OrtegascansFrParser(config, session)
...
# Statuts → énumération NexusDL (français + anglais + espagnol).
...
# ---------------------------------------------------------------------------
# Sélecteurs Madara spécifiques à Ortegascans FR
# ---------------------------------------------------------------------------
...
class OrtegascansFrParser(
    JsRenderedMixin,
    CloudflareMixin,
    MadaraMixin,
    BaseParser,
):
    """Parseur Ortegascans FR (Madara + Cloudflare).
    ...
    # --- Sélecteurs Madara (standards + ajustements Ortegascans FR) ---
    ...
    def __init__(
        ...
        *,
        ...
        """Initialise le parseur Ortegascans FR.
    ...
    def _madara_text(node: Node | None) -> str:
        ...
        if node is None:
            return ""
        return OrtegascansFrParser._madara_clean(
    ...
    def _madara_parse_status(self, raw: str | None) -> MangaStatus:
        ...
        Args:
            raw: Libellé brut (français, anglais ou espagnol).
    ...
    def _madara_parse_relative_date(raw: str | None) -> datetime | None:
        """Parse une date relative (français, anglais ou espagnol).

        Supporte : ``"il y a 3 jours"``, ``"3 days ago"``,
        ``"hace 3 días"``.
    ...
    def _madara_chapter_number(text: str) -> float | str:
        ...
        Supporte : ``"Chapitre 47"``, ``"Chapter 47"``, ``"Capítulo 47"``.
        ...
        """
        cleaned = OrtegascansFrParser._madara_clean(text)
    ...
    def _madara_chapter_volume(text: str) -> int | None:
        ...
        Supporte : ``"Tome 3"``, ``"Volume 3"``, ``"Tomo 3"``.
        ...
        for pattern in patterns:
            for match in pattern.finditer(html):
                ...
                try:
                    ...
                    continue
                urls = OrtegascansFrParser._ortega_walk_ts_reader(parsed)
                if urls:
                    return urls
    ...
    async def search(
        ...
        """Recherche des mangas sur Ortegascans FR.

        Ortegascans FR utilise ``/?s={query}&post_type=wp-manga``
        ...
        """
        self.logger.debug(
            "OrtegascansFr search: {query} (page {page})",
            query=query,
            page=page,
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec recherche OrtegascansFr pour {query!r}: {err}",
                query=query,
                err=exc,
            )
            raise ParseError(
    ...
    def _madara_parse_search_html(
        ...
        *,
        ...
        self.logger.info(
            "OrtegascansFr search: {n} résultat(s)", n=len(results)
        )
        return results

    async def get_manga(self, url_or_id: str) -> Manga:
        """Récupère la fiche complète d'un manga Ortegascans FR.
        ...
        self.logger.debug("OrtegascansFr get_manga: {url}", url=url)
        ...
        try:
            ...
            self.logger.error(
                "Échec get_manga OrtegascansFr pour {url!r}: {err}",
                url=url,
                err=exc,
            )
            raise ParseError(
        ...
        self.logger.info(
            "OrtegascansFr get_manga OK: {title} ({n} chapitres)",
            title=title,
            n=len(chapters),
        )
        return manga
    ...
    async def get_chapters(self, manga: Manga) -> list[Chapter]:
        ...
        """
        self.logger.debug(
            "OrtegascansFr get_chapters: {title}", title=manga.title
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_chapters OrtegascansFr pour {title!r}: {err}",
                title=manga.title,
                err=exc,
            )
            raise ParseError(
    ...
    def _madara_parse_chapters_from_html(
        ...
        *,
        ...
        self.logger.debug(
            "OrtegascansFr chapitres: {n}", n=len(chapters)
        )
        return chapters

    async def get_pages(self, chapter: Chapter) -> list[Page]:
        """Récupère les URLs des pages d'un chapitre Ortegascans FR.
        ...
        """
        self.logger.debug(
            "OrtegascansFr get_pages: {url}", url=str(chapter.url)
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_pages OrtegascansFr pour {url!r}: {err}",
                url=chapter_url,
                err=exc,
            )
            raise ParseError(
        ...
        self.logger.info(
            "OrtegascansFr get_pages: {n} page(s) extraite(s)", n=len(pages)
        )
        return pages
    ...
    async def health_check(self) -> bool:
        """Vérifie que Ortegascans FR est accessible.
        ...
        for base in self.fallback_domains:
            try:
                ...
                if rendered.ok:
                    self.logger.info(
                        "Health check OrtegascansFr OK: {base} (status={s})",
                        base=base,
                        s=rendered.status,
                    )
                    return True
                ...
                self.logger.debug(
                    "Health check OrtegascansFr échec sur {base}: {err}",
                    base=base,
                    err=exc,
                )
        self.logger.warning("Health check OrtegascansFr KO (tous domaines)")
        return False
