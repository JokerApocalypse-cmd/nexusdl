"""Parseur Scylla Scans pour NexusDL.

Scylla Scans (``https://scyllascans.org``) était un groupe de scanlation
anglophone de premier plan, spécialisé dans les mangas, manhwas et manhuas
de qualité. Le groupe a annoncé sa dissolution le 4 novembre 2025[reference:0].

Le site a utilisé le thème WordPress **Madara** (MangaBooth), comme la
plupart des sites de scanlation modernes. Il a également changé plusieurs
fois de domaine :

* ``scyllascans.org`` (domaine d'origine, aujourd'hui expiré et en vente
  sur GoDaddy)[reference:1].
* ``scyllacomics.xyz`` (dernier domaine connu, actuellement parké)[reference:2].

Caractéristiques techniques
---------------------------

* **Moteur** : WordPress Madara (thème premium pour sites manga).
* **Langue** : Anglais (``en``).
* **Protection** : Cloudflare (AS13335) — contournement nécessaire
  (Playwright / FlareSolverr).
* **Domaines** : ``scyllascans.org`` (historique), ``scyllacomics.xyz``
  (dernier connu).
* **Structure des URLs** (format Madara standard) :
    - Recherche : ``/?s={query}&post_type=wp-manga``.
    - Manga : ``/manga/{slug}/`` ou ``/webtoon/{slug}/``.
...
Example:
    ...
    >>> from nexusdl.parsers.en.scyllascans import ScyllaScansParser
    >>>
    >>> parser = ScyllaScansParser(config, session)
...
_BASE_URL: Final[str] = "https://scyllascans.org"
_FALLBACK_DOMAINS: Final[tuple[str, ...]] = (
    "https://scyllascans.org",
    "https://scyllacomics.xyz",
)
_SITE_ID: Final[str] = "scyllascans"
...
# ---------------------------------------------------------------------------
# Sélecteurs Madara spécifiques à Scylla Scans
# ---------------------------------------------------------------------------

_SCYLLA_SEARCH_ITEM_SELECTOR: Final[str] = (
    ".page-item-detail.manga, .page-item-detail, "
    ".c-tabs-item__content, .bs, .bsx"
)
_SCYLLA_CHAPTER_SELECTOR: Final[str] = (
    "div.listing-chapters_wrap > ul li, "
    ".listing-chapters_wrap li, "
    ".eplister li:not(.ch-price-side)"
)
_SCYLLA_PAGE_IMG_SELECTOR: Final[str] = (
...
class ScyllaScansParser(
    ...
    """Parseur Scylla Scans (Madara + Cloudflare).
    ...
    # --- Sélecteurs Madara (standards + ajustements Scylla) ---
    madara_search_item_selector: ClassVar[str] = _SCYLLA_SEARCH_ITEM_SELECTOR
    ...
    madara_chapter_selector: ClassVar[str] = _SCYLLA_CHAPTER_SELECTOR
    madara_page_img_selector: ClassVar[str] = _SCYLLA_PAGE_IMG_SELECTOR
    ...
    def __init__(
        ...
        *,
        ...
        """Initialise le parseur Scylla Scans.
    ...
    def _madara_text(node: Node | None) -> str:
        ...
        if node is None:
            return ""
        return ScyllaScansParser._madara_clean(
    ...
    def _madara_chapter_number(text: str) -> float | str:
        ...
        """
        cleaned = ScyllaScansParser._madara_clean(text)
    ...
    async def search(
        ...
        """Recherche des mangas sur Scylla Scans.

        Scylla Scans utilise ``/?s={query}&post_type=wp-manga`` (paramètre
        ...
        """
        self.logger.debug(
            "ScyllaScans search: {query} (page {page})",
            query=query,
            page=page,
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec recherche ScyllaScans pour {query!r}: {err}",
                query=query,
                err=exc,
            )
            raise ParseError(
    ...
    def _madara_parse_search_html(
        ...
        *,
        ...
        self.logger.info(
            "ScyllaScans search: {n} résultat(s)", n=len(results)
        )
        return results

    async def get_manga(self, url_or_id: str) -> Manga:
        """Récupère la fiche complète d'un manga Scylla Scans.
        ...
        self.logger.debug("ScyllaScans get_manga: {url}", url=url)

        try:
            response = await self.cf_get(url)
        except Exception as exc:  # noqa: BLE001
            self.logger.error(
                "Échec get_manga ScyllaScans pour {url!r}: {err}",
                url=url,
                err=exc,
            )
            raise ParseError(
        ...
        self.logger.info(
            "ScyllaScans get_manga OK: {title} ({n} chapitres)",
            title=title,
            n=len(chapters),
        )
        return manga
    ...
    async def get_chapters(self, manga: Manga) -> list[Chapter]:
        ...
        """
        self.logger.debug(
            "ScyllaScans get_chapters: {title}", title=manga.title
        )
        try:
            html = await self.cf_get_html(str(manga.url))
        except Exception as exc:  # noqa: BLE001
            self.logger.error(
                "Échec get_chapters ScyllaScans pour {title!r}: {err}",
                title=manga.title,
                err=exc,
            )
            raise ParseError(
    ...
    def _madara_parse_chapters_from_html(
        ...
        *,
        ...
        for node in self._madara_all(tree, self.madara_chapter_selector):
            ...
            # Extrait l'ID depuis l'URL : /manga/{slug}/{chapter-slug}/
            source_id = (
                abs_url.rstrip("/").rsplit("/", 1)[-1] or label
            )
            chapters.append(
                Chapter(
                    id=f"{self.config.id}:{source_id}",
        ...
        self.logger.debug(
            "ScyllaScans chapitres: {n}", n=len(chapters)
        )
        return chapters

    async def get_pages(self, chapter: Chapter) -> list[Page]:
        """Récupère les URLs des pages d'un chapitre Scylla Scans.
        ...
        """
        self.logger.debug(
            "ScyllaScans get_pages: {url}", url=str(chapter.url)
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_pages ScyllaScans pour {url!r}: {err}",
                url=chapter_url,
                err=exc,
            )
            raise ParseError(
        ...
        urls: list[str] = []
        for node in self._madara_all(
            tree, self.madara_page_img_selector
        ):
            src = (
    ...
    async def health_check(self) -> bool:
        """Vérifie que Scylla Scans est accessible.

        Teste le domaine principal puis les domaines de fallback. Note : le
        site est signalé comme inactif depuis novembre 2025, le health check
        peut donc échouer sur tous les domaines.
        ...
        for base in self.fallback_domains:
            try:
                ...
                if response.status_code < 500:
                    self.logger.info(
                        "Health check ScyllaScans OK: {base} (status={s})",
                        base=base,
                ...
                self.logger.debug(
                    "Health check ScyllaScans échec sur {base}: {err}",
                    base=base,
                    err=exc,
                )
        self.logger.warning(
            "Health check ScyllaScans KO (tous domaines) — le site est "
            "probablement inactif depuis novembre 2025"
        )
        return False
