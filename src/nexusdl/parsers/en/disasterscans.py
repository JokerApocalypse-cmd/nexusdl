"""Parseur Disaster Scans pour NexusDL.

Disaster Scans (``https://disasterscans.com``) était un groupe de scanlation
anglophone de premier plan, spécialisé dans les manhwa/manhua coréens et
chinois. Le groupe a cessé son activité en 2024 et son catalogue a été
partiellement repris par d'autres équipes.

Caractéristiques techniques
---------------------------

* **Moteur** : WordPress **Madara** (thème premium Mangabooth), le même
  moteur que Toonily, ToonGod, Scylla Scans, Galaxy Scans, etc.
* **Langue** : Anglais (``en``).
* **Protection** : Cloudflare (AS13335, IP ``104.21.16.85`` /
  ``172.67.172.115``) avec challenge de niveau modéré.
* **Domaines** :
    - ``disasterscans.com`` (principal historique)
    - ``disasterscans.net`` (miroir)
...
* **Objet JS ``ts_reader``** : le thème Madara de Disaster Scans expose
  également les pages dans un objet JavaScript ``ts_reader`` — méthode
  d'extraction principale utilisée par les extensions Tachiyomi/Mihon.
...
Example:
    ...
    >>> from nexusdl.parsers.en.disasterscans import DisasterScansParser
    >>>
    >>> parser = DisasterScansParser(config, session)
    >>> results = await parser.search("tomb raider king")
...
_BASE_URL: Final[str] = "https://disasterscans.com"
_FALLBACK_DOMAINS: Final[tuple[str, ...]] = (
    "https://disasterscans.com",
    "https://disasterscans.net",
)
_SITE_ID: Final[str] = "disasterscans"
_LANGUAGE: Final[Language] = Language.EN
_ADULT: Final[bool] = False

# Regex de parsing des chapitres (format Madara : "Chapter 255").
...
# ---------------------------------------------------------------------------
# Sélecteurs Madara spécifiques à Disaster Scans
# ---------------------------------------------------------------------------

_DS_SEARCH_ITEM_SELECTOR: Final[str] = (
    ".page-item-detail.manga, .page-item-detail, "
    ".c-tabs-item__content, .bs, .bsx, "
    ".manga-item, .item-summary, .row.c-tabs-item"
)
_DS_CHAPTER_SELECTOR: Final[str] = (
...
_DS_PAGE_IMG_SELECTOR: Final[str] = (
...
_DS_PAGES_VAR_NAMES: Final[tuple[str, ...]] = (
    "ts_reader",
    "pages",
    "page_urls",
    "pageUrls",
...
class DisasterScansParser(
    ...
    """Parseur Disaster Scans (Madara + Cloudflare).
    ...
    # --- Sélecteurs Madara (standards + ajustements Disaster Scans) ---
    madara_search_item_selector: ClassVar[str] = _DS_SEARCH_ITEM_SELECTOR
    ...
    madara_chapter_selector: ClassVar[str] = _DS_CHAPTER_SELECTOR
    madara_page_img_selector: ClassVar[str] = _DS_PAGE_IMG_SELECTOR
    madara_pages_var_names: ClassVar[tuple[str, ...]] = _DS_PAGES_VAR_NAMES
    ...
    def __init__(
        ...
        *,
        ...
        """Initialise le parseur Disaster Scans.
    ...
    def _madara_text(node: Node | None) -> str:
        ...
        if node is None:
            return ""
        return DisasterScansParser._madara_clean(
    ...
    def _madara_chapter_number(text: str) -> float | str:
        ...
        Args:
            text: Libellé (ex. ``"Chapter 255"``).

        Returns:
            ``float`` si un numéro est trouvé, sinon la chaîne nettoyée.
        """
        cleaned = DisasterScansParser._madara_clean(text)
    ...
    @staticmethod
    def _ds_extract_ts_reader(html: str) -> list[str]:
        """Extrait les URLs d'images depuis l'objet JS ``ts_reader``.

        Le thème Madara de Disaster Scans expose les pages dans un objet
        ...
        for pattern in patterns:
            for match in pattern.finditer(html):
                ...
                try:
                    ...
                    continue
                urls = DisasterScansParser._ds_walk_ts_reader(parsed)
                if urls:
                    return urls
    ...
    @staticmethod
    def _ds_walk_ts_reader(payload: Any) -> list[str]:
    ...
    async def search(
        ...
        """Recherche des mangas sur Disaster Scans.

        Disaster Scans utilise ``/?s={query}&post_type=wp-manga`` (paramètre
        ...
        """
        self.logger.debug(
            "DisasterScans search: {query} (page {page})",
            query=query,
            page=page,
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec recherche DisasterScans pour {query!r}: {err}",
                query=query,
                err=exc,
            )
            raise ParseError(
    ...
    def _madara_parse_search_html(
        ...
        *,
        ...
        self.logger.info(
            "DisasterScans search: {n} résultat(s)", n=len(results)
        )
        return results

    async def get_manga(self, url_or_id: str) -> Manga:
        """Récupère la fiche complète d'un manga Disaster Scans.
        ...
        self.logger.debug("DisasterScans get_manga: {url}", url=url)
        ...
        try:
            ...
            self.logger.error(
                "Échec get_manga DisasterScans pour {url!r}: {err}",
                url=url,
                err=exc,
            )
            raise ParseError(
        ...
        self.logger.info(
            "DisasterScans get_manga OK: {title} ({n} chapitres)",
            title=title,
            n=len(chapters),
        )
        return manga
    ...
    async def get_chapters(self, manga: Manga) -> list[Chapter]:
        ...
        """
        self.logger.debug(
            "DisasterScans get_chapters: {title}", title=manga.title
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_chapters DisasterScans pour {title!r}: {err}",
                title=manga.title,
                err=exc,
            )
            raise ParseError(
    ...
    def _madara_parse_chapters_from_html(
        ...
        *,
        ...
        self.logger.debug(
            "DisasterScans chapitres: {n}", n=len(chapters)
        )
        return chapters

    async def get_pages(self, chapter: Chapter) -> list[Page]:
        """Récupère les URLs des pages d'un chapitre Disaster Scans.
        ...
        """
        self.logger.debug(
            "DisasterScans get_pages: {url}", url=str(chapter.url)
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_pages DisasterScans pour {url!r}: {err}",
                url=chapter_url,
                err=exc,
            )
            raise ParseError(
        ...
        ts_urls = self._ds_extract_ts_reader(html)
        if ts_urls:
            for u in ts_urls:
        ...
        self.logger.info(
            "DisasterScans get_pages: {n} page(s) extraite(s)", n=len(pages)
        )
        return pages
    ...
    async def health_check(self) -> bool:
        """Vérifie que Disaster Scans est accessible.

        Teste le domaine principal puis les domaines de fallback. Note : le
        site est signalé comme inactif depuis 2024, le health check peut
        donc échouer sur tous les domaines.
        ...
        for base in self.fallback_domains:
            try:
                ...
                if rendered.ok:
                    self.logger.info(
                        "Health check DisasterScans OK: {base} (status={s})",
                        base=base,
                        s=rendered.status,
                    )
                    return True
                ...
                self.logger.debug(
                    "Health check DisasterScans échec sur {base}: {err}",
                    base=base,
                    err=exc,
                )
        self.logger.warning(
            "Health check DisasterScans KO (tous domaines) — le site est "
            "probablement inactif depuis 2024"
        )
        return False
