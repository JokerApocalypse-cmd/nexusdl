"""Parseur Toonily pour NexusDL.

Toonily (``https://toonily.com``) est un site de scanlation anglophone
spécialisé dans les **manhwa** et **webtoons** coréens, utilisant le thème
WordPress **Madara** — le même moteur que de nombreux sites manga
(``mangabooth.com``). Le site héberge du contenu généraliste et mature
(``toonily-mature=1`` cookie).
...
* **Langue** : Anglais (``en``).
* **Protection** : Cloudflare (IP ``172.67.72.131`` / ``172.67.72.131``,
  AS13335 Cloudflare). Contournement nécessaire (FlareSolverr ou
  Playwright).
* **Domaines** : ``toonily.com`` (principal), ``toonily.me``,
  ``toonily.net``, ``toonily.to``, ``toonily.pro``.
* **Structure des URLs** :
    - Recherche : ``/search/{query}`` (mots séparés par des tirets).
    - Manga : ``/webtoon/{slug}/`` ou ``/serie/{slug}/``.
    - Chapitre : ``/webtoon/{slug}/chapter-{num}/``.
* **Cookie mature** : ``toonily-mature=1`` requis pour accéder au contenu
  adulte (le parseur l'injecte par défaut).
* **Sélecteurs Madara** : ``.page-item-detail.manga``,
  ``div.listing-chapters_wrap > ul li``,
  ``div.reading-content div.page-break.no-gaps img``.
* **Chapitres** : certains peuvent être verrouillés (premium) — le parseur
  les filtre via l'absence de lien ``a`` actif.
...
Example:
    ...
    >>> from nexusdl.parsers.en.toonily import ToonilyParser
    >>>
    >>> parser = ToonilyParser(config, session)
    >>> results = await parser.search("nano machine")
...
_BASE_URL: Final[str] = "https://toonily.com"
...
_SITE_ID: Final[str] = "toonily"
_LANGUAGE: Final[Language] = Language.EN
_ADULT: Final[bool] = True  # Site mature (toonily-mature=1)

# Cookie mature à injecter dans la session.
_MATURE_COOKIE: Final[str] = "toonily-mature=1"
...
_STATUS_MAP: Final[dict[str, MangaStatus]] = {
    ...
    "end": MangaStatus.COMPLETED,  # Toonily utilise "End"
...
# ---------------------------------------------------------------------------
# Sélecteurs Madara spécifiques à Toonily
# ---------------------------------------------------------------------------

_TOONILY_SEARCH_ITEM_SELECTOR: Final[str] = (
    ".page-item-detail.manga, .page-item-detail, .c-tabs-item__content, .bs"
)
_TOONILY_CHAPTER_SELECTOR: Final[str] = (
    "div.listing-chapters_wrap > ul li, "
    ".listing-chapters_wrap li, "
    ".eplister li:not(.ch-price-side)"
)
_TOONILY_PAGE_IMG_SELECTOR: Final[str] = (
    "div.reading-content div.page-break.no-gaps img, "
    ".reading-content img, "
    ".page-break img"
)
...
class ToonilyParser(
    ...
    """Parseur Toonily (Madara + Cloudflare).
    ...
    madara_search_path: ClassVar[str] = "/search"
    madara_search_query_param: ClassVar[str] = "q"
    madara_search_method: ClassVar[str] = "GET"
    madara_series_path: ClassVar[str] = "/webtoon"
    madara_read_path: ClassVar[str] = "/webtoon"

    # --- Sélecteurs Madara (standards + ajustements Toonily) ---
    madara_search_item_selector: ClassVar[str] = _TOONILY_SEARCH_ITEM_SELECTOR
    madara_search_link_selector: ClassVar[str] = ".item-summary .post-title a, a"
    madara_search_cover_selector: ClassVar[str] = "img"
    madara_search_title_selector: ClassVar[str] = (
        ".item-summary .post-title a, .post-title, h3, h4"
    )
    ...
    madara_manga_status_selector: ClassVar[str] = (
        ".status, .manga-status, .post-status, .post-content_item .summary-content"
    )
    madara_manga_year_selector: ClassVar[str] = (
        ".year, .manga-year, .post-year, .post-content_item .summary-content"
    )

    madara_chapter_selector: ClassVar[str] = _TOONILY_CHAPTER_SELECTOR
    madara_page_img_selector: ClassVar[str] = _TOONILY_PAGE_IMG_SELECTOR
    ...
    def __init__(
        ...
        *,
        ...
        # Injecte le cookie mature dans la session si possible.
        self._inject_mature_cookie()

    def _inject_mature_cookie(self) -> None:
        """Injecte le cookie ``toonily-mature=1`` dans la session HTTP.

        Ce cookie est requis par Toonily pour afficher le contenu mature
        (genres adultes, images explicites). Sans lui, certains mangas
        peuvent être filtrés ou des pages bloquées.
        """
        target = getattr(self.session, "cookies", None)
        if isinstance(target, dict):
            target.update({"toonily-mature": "1"})
            self.logger.debug(
                "Cookie mature injecté dans la session HTTP (%s)", _MATURE_COOKIE
            )
            return
        updater = getattr(self.session, "update_cookies", None)
        if callable(updater):
            try:
                updater({"toonily-mature": "1"})
                self.logger.debug(
                    "Cookie mature injecté via update_cookies (%s)", _MATURE_COOKIE
                )
            except Exception as exc:  # noqa: BLE001
                self.logger.debug(
                    "Échec injection cookie mature: {err}", err=exc
                )
    ...
    def _madara_text(node: Node | None) -> str:
        ...
        if node is None:
            return ""
        return ToonilyParser._madara_clean(node.text(deep=True, separator=" "))
    ...
    def _madara_chapter_number(text: str) -> float | str:
        ...
        """
        cleaned = ToonilyParser._madara_clean(text)
    ...
    async def search(
        ...
        """Recherche des mangas sur Toonily.

        Toonily utilise ``/search/{query}`` avec les espaces remplacés par
        des tirets (pas de paramètre ``?s=``).
        ...
        """
        self.logger.debug(
            "Toonily search: {query} (page {page})",
            query=query,
            page=page,
        )

        # Toonily : /search/{query-with-dashes}
        ...
        try:
            ...
            self.logger.error(
                "Échec recherche Toonily pour {query!r}: {err}",
                query=query,
                err=exc,
            )
            raise ParseError(
    ...
    def _madara_parse_search_html(
        ...
        *,
        ...
        self.logger.info(
            "Toonily search: {n} résultat(s)", n=len(results)
        )
        return results

    async def get_manga(self, url_or_id: str) -> Manga:
        """Récupère la fiche complète d'un manga Toonily.
        ...
        self.logger.debug("Toonily get_manga: {url}", url=url)

        try:
            response = await self.cf_get(url)
        except Exception as exc:  # noqa: BLE001
            self.logger.error(
                "Échec get_manga Toonily pour {url!r}: {err}",
                url=url,
                err=exc,
            )
            raise ParseError(
        ...
        status_node = self._madara_first(tree, self.madara_manga_status_selector)
        ...
        self.logger.info(
            "Toonily get_manga OK: {title} ({n} chapitres)",
            title=title,
            n=len(chapters),
        )
        return manga
    ...
    async def get_chapters(self, manga: Manga) -> list[Chapter]:
        ...
        """
        self.logger.debug(
            "Toonily get_chapters: {title}", title=manga.title
        )
        try:
            html = await self.cf_get_html(str(manga.url))
        except Exception as exc:  # noqa: BLE001
            self.logger.error(
                "Échec get_chapters Toonily pour {title!r}: {err}",
                title=manga.title,
                err=exc,
            )
            raise ParseError(
    ...
    async def get_pages(self, chapter: Chapter) -> list[Page]:
        ...
        """
        self.logger.debug(
            "Toonily get_pages: {url}", url=str(chapter.url)
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_pages Toonily pour {url!r}: {err}",
                url=chapter_url,
                err=exc,
            )
            raise ParseError(
    ...
    async def health_check(self) -> bool:
        """Vérifie que Toonily est accessible.
        ...
        for base in self.fallback_domains:
            try:
                ...
                if response.status_code < 500:
                    self.logger.info(
                        "Health check Toonily OK: {base} (status={s})",
                        base=base,
                ...
                self.logger.debug(
                    "Health check Toonily échec sur {base}: {err}",
                    base=base,
                    err=exc,
                )
        self.logger.warning("Health check Toonily KO (tous domaines)")
        return False
