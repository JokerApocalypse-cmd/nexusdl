"""Parseur CrunchyScan pour NexusDL.

CrunchyScan (``https://crunchyscan.org``) est une plateforme francophone de
scantrad agrégant plusieurs milliers de mangas, manhwas et webtoons traduits
en français. Le site est accessible sans inscription et utilise un lecteur
web avec défilement vertical adapté aux webtoons.

Caractéristiques techniques
---------------------------

* **Moteur** : WordPress **Madara** (thème premium Mangabooth), le même
  moteur que Toonily, ToonGod, Scylla Scans, Galaxy Scans, Disaster Scans.
* **Langue** : Français (``fr``).
* **Protection** : Cloudflare (AS13335, IP ``104.21.16.85``) avec challenge
  **entre chaque chapitre** avant lecture. Le site utilise également un
  **captcha interceptor** intégré aux chapitres (signalé par la communauté
  Tachiyomi/Mihon).
* **Domaines** :
    - ``crunchyscan.org`` (domaine actuel, depuis 2026)
    - ``crunchyscan.fr`` (domaine historique, bloqué en France)
    - ``crunchyscan.st`` (miroir)
* **Structure des URLs** :
    - Catalogue : ``/liste-manga/`` ou ``/manga/``.
    - Manga : ``/liste-manga/{slug}/`` ou ``/manga/{slug}/``.
    - Chapitre : ``/lecture-en-ligne/{slug}/read/{chapter-slug}``.
* **Contenu adulte** : 18+/NSFW = Oui (signalé dans la source Tachiyomi).
...
Example:
    ...
    >>> from nexusdl.parsers.en.crunchyscan_org import CrunchyScanParser
    >>>
    >>> parser = CrunchyScanParser(config, session)
...
_BASE_URL: Final[str] = "https://crunchyscan.org"
_FALLBACK_DOMAINS: Final[tuple[str, ...]] = (
    "https://crunchyscan.org",
    "https://crunchyscan.fr",
    "https://crunchyscan.st",
)
_SITE_ID: Final[str] = "crunchyscan_org"
_LANGUAGE: Final[Language] = Language.FR
_ADULT: Final[bool] = True  # 18+/NSFW = Oui (signalé par la communauté)

# Regex de parsing des chapitres (format Madara FR : "Chapitre 47").
_CHAPTER_NUMBER_RE: Final[re.Pattern[str]] = re.compile(
    r"(?:chapitre|chapter|ch\.?|episode|ep\.?|ep)\s*(\d+(?:[.,]\d+)?)",
    re.IGNORECASE,
)
...
# ---------------------------------------------------------------------------
# Sélecteurs Madara spécifiques à CrunchyScan
# ---------------------------------------------------------------------------

_CS_SEARCH_ITEM_SELECTOR: Final[str] = (
    ".page-item-detail.manga, .page-item-detail, "
    ".c-tabs-item__content, .bs, .bsx, "
    ".manga-item, .item-summary, .row.c-tabs-item"
)
_CS_CHAPTER_SELECTOR: Final[str] = (
...
_CS_PAGE_IMG_SELECTOR: Final[str] = (
...
_CS_PAGES_VAR_NAMES: Final[tuple[str, ...]] = (
    "ts_reader",
    "pages",
    "page_urls",
    "pageUrls",
...
class CrunchyScanParser(
    ...
    """Parseur CrunchyScan (Madara + Cloudflare).
    ...
    Attributes:
        ...
        adult: Contenu adulte (``True`` — 18+/NSFW).
    ...
    # --- Sélecteurs Madara (standards + ajustements CrunchyScan) ---
    madara_search_item_selector: ClassVar[str] = _CS_SEARCH_ITEM_SELECTOR
    ...
    madara_chapter_selector: ClassVar[str] = _CS_CHAPTER_SELECTOR
    madara_page_img_selector: ClassVar[str] = _CS_PAGE_IMG_SELECTOR
    madara_pages_var_names: ClassVar[tuple[str, ...]] = _CS_PAGES_VAR_NAMES
    ...
    def __init__(
        ...
        *,
        ...
        """Initialise le parseur CrunchyScan.
    ...
    def _madara_text(node: Node | None) -> str:
        ...
        if node is None:
            return ""
        return CrunchyScanParser._madara_clean(
    ...
    def _madara_parse_relative_date(raw: str | None) -> datetime | None:
        ...
        Args:
            raw: Chaîne de date relative (ex. ``"il y a 3 jours"``).
    ...
    def _madara_chapter_number(text: str) -> float | str:
        ...
        Gère les numéros décimaux (``12.5``) ainsi que les labels spéciaux
        (``Oneshot``, ``Extra``).

        Args:
            text: Libellé (ex. ``"Chapitre 47"``).

        Returns:
            ``float`` si un numéro est trouvé, sinon la chaîne nettoyée.
        """
        cleaned = CrunchyScanParser._madara_clean(text)
    ...
    def _madara_detect_rating(self, genres: list[str]) -> ContentRating:
        ...
        CrunchyScan est un site 18+/NSFW (signalé par la communauté). La
        classification est détectée automatiquement à partir des genres.
    ...
    @staticmethod
    def _cs_extract_ts_reader(html: str) -> list[str]:
        """Extrait les URLs d'images depuis l'objet JS ``ts_reader``.

        Le thème Madara de CrunchyScan expose les pages dans un objet
        ...
        for pattern in patterns:
            for match in pattern.finditer(html):
                ...
                try:
                    ...
                    continue
                urls = CrunchyScanParser._cs_walk_ts_reader(parsed)
                if urls:
                    return urls
    ...
    @staticmethod
    def _cs_walk_ts_reader(payload: Any) -> list[str]:
    ...
    async def search(
        ...
        """Recherche des mangas sur CrunchyScan.

        CrunchyScan utilise ``/?s={query}&post_type=wp-manga`` (paramètre
        ...
        """
        self.logger.debug(
            "CrunchyScan search: {query} (page {page})",
            query=query,
            page=page,
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec recherche CrunchyScan pour {query!r}: {err}",
                query=query,
                err=exc,
            )
            raise ParseError(
    ...
    def _madara_parse_search_html(
        ...
        *,
        ...
        self.logger.info(
            "CrunchyScan search: {n} résultat(s)", n=len(results)
        )
        return results

    async def get_manga(self, url_or_id: str) -> Manga:
        """Récupère la fiche complète d'un manga CrunchyScan.
        ...
        self.logger.debug("CrunchyScan get_manga: {url}", url=url)
        ...
        try:
            ...
            self.logger.error(
                "Échec get_manga CrunchyScan pour {url!r}: {err}",
                url=url,
                err=exc,
            )
            raise ParseError(
        ...
        self.logger.info(
            "CrunchyScan get_manga OK: {title} ({n} chapitres)",
            title=title,
            n=len(chapters),
        )
        return manga
    ...
    async def get_chapters(self, manga: Manga) -> list[Chapter]:
        ...
        """
        self.logger.debug(
            "CrunchyScan get_chapters: {title}", title=manga.title
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_chapters CrunchyScan pour {title!r}: {err}",
                title=manga.title,
                err=exc,
            )
            raise ParseError(
    ...
    def _madara_parse_chapters_from_html(
        ...
        *,
        ...
        self.logger.debug(
            "CrunchyScan chapitres: {n}", n=len(chapters)
        )
        return chapters

    async def get_pages(self, chapter: Chapter) -> list[Page]:
        """Récupère les URLs des pages d'un chapitre CrunchyScan.

        Le site utilise Cloudflare (challenge entre chaque chapitre) et du
        ...
        """
        self.logger.debug(
            "CrunchyScan get_pages: {url}", url=str(chapter.url)
        )
        ...
        try:
            ...
            self.logger.error(
                "Échec get_pages CrunchyScan pour {url!r}: {err}",
                url=chapter_url,
                err=exc,
            )
            raise ParseError(
        ...
        ts_urls = self._cs_extract_ts_reader(html)
        if ts_urls:
            for u in ts_urls:
        ...
        self.logger.info(
            "CrunchyScan get_pages: {n} page(s) extraite(s)", n=len(pages)
        )
        return pages
    ...
    async def health_check(self) -> bool:
        """Vérifie que CrunchyScan est accessible.
        ...
        for base in self.fallback_domains:
            try:
                ...
                if rendered.ok:
                    self.logger.info(
                        "Health check CrunchyScan OK: {base} (status={s})",
                        base=base,
                        s=rendered.status,
                    )
                    return True
                ...
                self.logger.debug(
                    "Health check CrunchyScan échec sur {base}: {err}",
                    base=base,
                    err=exc,
                )
        self.logger.warning("Health check CrunchyScan KO (tous domaines)")
        return False
