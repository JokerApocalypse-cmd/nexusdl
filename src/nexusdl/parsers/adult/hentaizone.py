"""Parser complet pour HentaiZone (https://hentaizone.com).

HentaiZone est un site adulte français de scantrad hentai et mangas ecchi
traduits en français. Basé sur le thème WordPress **Madara** (MangaBooth),
avec les spécificités suivantes :

    - Protection Cloudflare active avec challenge JS.
    - Chapitres « sponsor » : accès anticipé nécessitant un compte premium.
      Détectés et marqués ``[Sponsor]`` dans les titres.
    - Images servies via CDN protégé par Referer (vérification stricte).
    - Lecteur avec lazy-loading : ``data-src`` / ``data-lazy-src``.
    - Certains chapitres sont réservés aux membres (``[Membre]``).
    - Contenu 18+ : ``adult = True``, activé uniquement si
      ``registry.include_adult: true``.

Fonctionnalités intégrées
=========================

    - **Recherche** : pagination Madara standard, déduplication URL.
    - **Manga** : parsing complet (titre, description, auteur, genres,
      statut, année, couverture, chapitres).
    - **Chapitres** : tri décroissant, détection sponsor/membre/futur,
      filtrage des URLs manquantes.
    - **Pages** : extraction lazy-load, filtrage trackers, gestion blob:.
    - **Cloudflare** : bypass via Playwright, retry avec backoff,
      rotation automatique de miroir en échec.
    - **Téléchargement** : retry, validation taille/MIME, Referer strict
      pour le CDN, écriture atomique via ``.part``.
    - **Health check** : distingue domaine actif / redirigé / suspendu.

Différences avec les autres parsers Madara
==========================================

    - **Détection sponsor/membre** : HentaiZone ajoute des marqueurs
      ``[Sponsor]`` et ``[Membre]`` dans les titres de chapitres.
    - **Referer strict** : le CDN d'images exige un Referer précis,
      sinon renvoie un placeholder ou un 403.
    - **Catalogue mixte** : le parser accepte à la fois les hentai purs
      et les mangas ecchi traduits (avec ``suggestive`` rating possible).
...
Example:
    Utilisation directe::
        ...
        async with HttpSession(site_config) as session:
            parser = HentaiZoneParser(site_config, session)
            results = await parser.search("one piece")
            manga = await parser.get_manga(results[0].url)
            pages = await parser.get_pages(manga.chapters[0])
...
#:
#: Historique :
#:   - ``hentaizone.com``  : domaine principal actuel
#:   - ``hentaizone.fr``   : redirige vers .com
#:   - ``hentai-zone.com`` : ancien domaine
MIRROR_DOMAINS: Final[tuple[str, ...]] = (
    "https://hentaizone.com",
    "https://hentaizone.fr",
)
...
#: Sélecteurs CSS (thème Madara + personnalisations HentaiZone).
...
PLAYWRIGHT_TIMEOUT: Final[float] = 50.0
...
#: Détection des chapitres sponsor (accès anticipé payant).
SPONSOR_PATTERN: Final[re.Pattern[str]] = re.compile(
    r"\b(sponsor|sponsoris[ée]|premium|anticip[ée]|early[- ]access|💎)\b",
    re.IGNORECASE,
)

#: Détection des chapitres réservés aux membres.
MEMBER_PATTERN: Final[re.Pattern[str]] = re.compile(
    r"\b(membre|members?|subscriber|vip|🔒)\b",
    re.IGNORECASE,
)

#: Détection des chapitres « à venir ».
...
def _is_tracking_url(url: str) -> bool:
    """Détecte une URL de tracking / publicité (inclut régies adultes).
...
class HentaiZoneParser(BaseParser):
    """Parser complet pour HentaiZone (https://hentaizone.com).

    Implémentation autonome — aucune dépendance à un mixin. Toute la
    logique Madara est contenue dans cette classe, avec des
    personnalisations spécifiques HentaiZone (sponsor, membre, Referer
    CDN strict).

    Attributes:
        site_id: Identifiant du parser (``"hentaizone"``).
    ...
    # --- Identité ---------------------------------------------------------
    site_id: ClassVar[str] = "hentaizone"
    ...
    def __init__(
        ...
        *,
        ...
        """Initialise le parser HentaiZone.
        ...
        Args:
            ...
            playwright_pool: Pool Playwright. **Recommandé** — sans lui,
                les challenges Cloudflare ne peuvent pas être résolus.
        """
        ...
        logger.bind(site=self.site_id).debug(
            "Parser HentaiZone initialisé (playwright={}, mirrors={})",
    ...
    async def __aenter__(self) -> HentaiZoneParser:
    ...
    async def close(self) -> None:
        ...
        if self._closed:
            return
        self._closed = True
        logger.bind(site=self.site_id).debug("Parser HentaiZone fermé")
    ...
    async def search(
        ...
        *,
        ...
        """Recherche des mangas sur HentaiZone.
    ...
    async def get_chapters(self, manga: Manga) -> list[Chapter]:
        ...
        Enrichit chaque chapitre :
            - ``[Sponsor]`` si accès anticipé payant.
            - ``[Membre]`` si réservé aux membres.
        ...
        logger.bind(site=self.site_id).info(
            "{} chapitre(s) pour '{}' ({} sponsor, {} membre, {} à venir)",
            len(valid),
            manga.title,
            sum(1 for ch in valid if ch.title and ch.title.startswith("[Sponsor]")),
            sum(1 for ch in valid if ch.title and ch.title.startswith("[Membre]")),
    ...
    async def get_pages(self, chapter: Chapter) -> list[Page]:
        ...
        if chapter.title and (
            chapter.title.startswith("[Sponsor]")
            or chapter.title.startswith("[Membre]")
        ):
            logger.bind(site=self.site_id).warning(
                "Chapitre restreint : ch.{} ({})",
                chapter.number,
                "sponsor" if "Sponsor" in chapter.title else "membre",
            )
    ...
    async def download_page(
        ...
        *,
        ...
        Le CDN d'HentaiZone exige un Referer strict. Le parser utilise
        ``Referer: {base_url}/`` pour éviter les placeholders 1×1.
    ...
    async def _download_page_once(
        ...
        try:
            # Referer strict exigé par le CDN HentaiZone
            referer = f"{self._active_base}/"
    ...
    async def health_check(self) -> bool:
        """Vérifie que HentaiZone est accessible.
        ...
        try:
            if self._playwright_pool is not None:
                ...
                if _is_cloudflare_challenge(html):
                    logger.bind(site=self.site_id).warning(
                        "HentaiZone : challenge CF non résolu"
                    )
                    return False
        ...
        logger.bind(site=self.site_id).error(
            "HentaiZone : aucun domaine accessible. Site possiblement "
    ...
    def _parse_chapters_html(
        ...
        Détecte les chapitres sponsor, membre et « à venir ».
        ...
        for item in tree.css(self.selectors["chapter_item"]):
            try:
                ...
                # Détection des marqueurs (ordre de priorité : sponsor > membre > futur)
                is_sponsor = bool(SPONSOR_PATTERN.search(full_text))
                is_member = bool(MEMBER_PATTERN.search(full_text)) and not is_sponsor
                is_future = bool(FUTURE_PATTERN.search(full_text))

                display_title = chapter_title
                if is_sponsor:
                    display_title = f"[Sponsor] {display_title}"
                elif is_member:
                    display_title = f"[Membre] {display_title}"
                if is_future:
    ...
    def describe(self) -> dict[str, Any]:
        ...
        return {
            ...
            "supports_sponsor_detection": True,
            "supports_member_detection": True,
