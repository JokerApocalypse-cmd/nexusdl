"""Parser complet pour X-Manga sur le domaine principal (https://x-manga.org).

X-Manga est un site adulte français de scans hentai traduits, basé sur
le thème WordPress Madara (MangaBooth). Il est accessible via deux
domaines interchangeables :

    - ``x-manga.net`` (historique, miroir)
    - ``x-manga.org`` (principal actuel, domaine de ce parser)
...
=========================
    ...
    - **Cloudflare** : bypass via Playwright, détection de challenge,
      retry avec backoff, rotation automatique de miroir en échec.
...
Différences avec ``x_manga_net.py``
===================================

    - ``base_url`` = ``https://x-manga.org`` (au lieu de ``.net``).
    - ``mirror_domains`` = ``(.org, .net)`` (ordre inversé).
    - ``REDIRECT_MARKERS`` = ``(scan.hentai.menu, x-manga.net)``.
    - ``site_id`` = ``"x_manga_org"`` (au lieu de ``"x_manga_net"``).

Les deux parsers partagent la même logique — le contenu servi est
identique. Le choix du parser dépend uniquement du domaine dans l'URL
...
Example:
    Utilisation directe::
        ...
        async with HttpSession(site_config) as session:
            parser = XMangaOrgParser(site_config, session)
...
#: Le ``.org`` est le domaine canonique ; le ``.net`` est un fallback.
...
#: Marqueurs de domaines historiques à réécrire automatiquement vers
#: le domaine canonique (`.org`).
#:
#: Historique :
#:   - ``scan.hentai.menu`` : ancien domaine, redirigeait vers x-manga.net
#:   - ``x-manga.net``      : ancien domaine principal, maintenant miroir
...
class XMangaOrgParser(BaseParser):
    """Parser complet pour X-Manga (https://x-manga.org).
    ...
    Attributes:
        site_id: Identifiant du parser (``"x_manga_org"``).
        language: Langue du contenu (``"fr"``).
        adult: Contenu 18+ (``True``).
        base_url: URL canonique du parser.
        mirror_domains: Liste des miroirs (``.org`` puis ``.net``).
    ...
    def __init__(
        ...
        *,
        ...
        """Initialise le parser X-Manga (.org).
        ...
        logger.bind(site=self.site_id).debug(
            "Parser X-Manga (.org) initialisé (playwright={}, mirrors={})",
    ...
    async def __aenter__(self) -> XMangaOrgParser:
    ...
    async def close(self) -> None:
        ...
        if self._closed:
            return
        self._closed = True
        logger.bind(site=self.site_id).debug("Parser X-Manga (.org) fermé")
    ...
    async def search(
        ...
        *,
        ...
        """Recherche des mangas sur X-Manga (.org).
    ...
    async def health_check(self) -> bool:
        """Vérifie que X-Manga (.org) est accessible.
        ...
        try:
            if self._playwright_pool is not None:
                ...
                if _is_cloudflare_challenge(html):
                    logger.bind(site=self.site_id).warning(
                        "X-Manga (.org) : challenge CF non résolu"
                    )
                    return False
        ...
        logger.bind(site=self.site_id).error(
            "X-Manga (.org) : aucun domaine accessible. Site possiblement "
    ...
    def normalize_url(self, url: str) -> str:
        ...
        Réécrit :
            - ``x-manga.net`` → ``x-manga.org``
            - ``scan.hentai.menu`` → ``x-manga.org``
        ...
        # Réécriture des marqueurs historiques vers le domaine canonique
